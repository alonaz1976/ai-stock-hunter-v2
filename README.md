# AI Stock Hunter V6.0.1 — Scanner Stop & State Hotfix

V6.0.1 keeps the V6.0.0 model/UX upgrade and adds a focused scanner-control hotfix built on the stable V5.9.9 background-scanner baseline. The goal is simpler decisions, cleaner visual hierarchy, and materially better evidence handling rather than adding more raw indicators.
...
Replace **all four files together**: `app.py`, `quant_engine.py`, `requirements.txt`, and `README.md`. App and engine versions must both show **6.0.1**.
...
## V6.0.1 scanner hotfix

- Adds a real **Stop Scan** control with cooperative server-side cancellation.
- Explicit scanner states: `RUNNING → STOPPING → STOPPED` plus `COMPLETED` / `FAILED`.
- Auto-refresh runs only while a scan is RUNNING or STOPPING. After completion/stop/failure, the page performs one final rerun and becomes static, eliminating the endlessly blinking Streamlit stop/rerun indicator.
- A stop request waits only for the current provider/data call to return, then cancels before the next ticker. The previous Last Completed Scan remains available.

Deployment: replace all four files together so app and engine both show **6.0.1**.
