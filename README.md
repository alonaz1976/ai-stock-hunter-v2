# AI Stock Hunter V6.3.0 — Persistent Feedback + Historical Replay
...
- **Optional automatic GitHub persistence.** When Streamlit secrets contain a `[feedback_persistence]` GitHub token/repository/path, V6.3.0 automatically restores the database on a fresh host and syncs it after completed Scanner runs, newly evaluated outcome windows and Historical Replay runs. The token is never shown or exported.
...
- **Replay candidate rule is research-only.** A sampled daily row is a replay candidate when Entry Score >=55 and either Early Score >=50 or Quant Score >=60. This benchmark is for learning indicator behavior; it is not a reconstruction of the full live Trade Trigger.
