# AI Stock Hunter — V6.3.9.40 Timing Audit Cleanup

## V6.3.9.40 — truthful TimingDataSource / TimingDataReason semantics

- Removes misleading `TimingDataSource=UNAVAILABLE` when move-consumed timing is not actually required for the current decision.
- `WAIT/WATCH` with no measured consumed move now uses `NOT REQUIRED`; if another safety layer already blocks entry (Exit / No-Chase / Extension), it uses `NOT REQUIRED — ALREADY BLOCKED` while preserving the visible safety warning.
- `INVALIDATED`, `EXTENDED — DO NOT CHASE`, `TOO LATE / CHASE`, and `RETEST ONLY / LATE` with no consumed measurement use `NOT REQUIRED — ALREADY BLOCKED` with a stage-specific reason.
- ARMED / CONFIRMED / TRIGGERED still require complete timing data exactly as before; genuinely missing required timing remains `UNAVAILABLE` / `TIMING DATA INCOMPLETE`.
- Decision diagnostics updated to **V3.12**; Trade Priority version is `V3.12-TIMING-AUDIT-CLEANUP`.
- Cached V3.11 snapshots are recomputed under V3.12 after redeploy.
- No changes to Decision weights, Q/E/EV thresholds, 80% Move Consumed hard gate, No-Chase logic, valuation, Trade Plan, or Feedback scoring.
- `app.py` and `quant_engine.py` synchronized to **V6.3.9.40**.

---

# AI Stock Hunter — V6.3.9.38 Feedback Startup NameError Hotfix

## V6.3.9.38 — empty-feedback startup safety

- Fixes a startup `NameError` in the Feedback Excel path when the server has snapshots/outcomes missing or no completed scan yet.
- `integrity_summary` and `movement_summary` are now initialized before the `sn.empty / oc.empty` branches.
- When no outcomes exist, Feedback Excel exports empty integrity/movement sheets instead of referencing undefined variables.
- Decision Intelligence remains **V3.10**; Timing Guard Wiring from V6.3.9.37 is unchanged.
- No changes to scan logic, rankings, Q/E/EV semantics, thresholds, No-Chase, valuation, trade plan, Feedback scoring, or Timing Data Completeness rules.
- `app.py` and `quant_engine.py` synchronized to **V6.3.9.38**.

---

# AI Stock Hunter — V6.3.9.37 Timing Guard Wiring Fix

## V6.3.9.37 — final-stage timing completeness wired before Decision Ranking and Excel export

- Fixes the V6.3.9.36 wiring gap where a final `ARMED`/`TRIGGERED` row could still reach Decision Intelligence as `FRESH / QUALIFIED` while `MoveConsumedBeforeTriggerPct` was missing.
- Adds `_wire_timing_completeness_v63937()` before Ranking V3.10. The guard evaluates the **final** TradeStage, not only the upstream Entry-engine state.
- Final ARMED/CONFIRMED/TRIGGERED + missing Move Consumed attempts one causal fallback: `SetupOriginPrice <= TriggerAnchorPrice <= Price`.
- If recovery fails: `TimingDataComplete=False`, `TimingQualification=TIMING DATA INCOMPLETE`, Q# is blocked, visible action becomes `TIMING DATA INCOMPLETE — NOT QUALIFIED FOR ENTRY`, and Emerging receives the lowest freshness tier.
- Scanner runtime calls the wiring guard before Decision Intelligence. Cached snapshots are forcibly migrated because the current `DecisionRankVersion` is **V3.10**.
- Top Scanner Excel download now runs the current migration/guard before export, so a stale runtime snapshot cannot export old V3.8/V3.9 decision semantics under a new filename.
- Scanner table / Early Entry Snapshot / Decision Intelligence export include `TimingDataComplete`, `TimingDataSource`, `TimingDataReason`.
- Decision diagnostics: `V3.10`; Trade Priority: `V3.10-TIMING-GUARD-WIRED`.
- `app.py` and `quant_engine.py` synchronized to **V6.3.9.37**.
- No changes to Decision weights, Q/E thresholds, Evidence thresholds, the 80% hard gate, No-Chase thresholds or valuation logic.

# AI Stock Hunter — V6.3.9.36 Timing Data Completeness Guard

## V6.3.9.36 — causal timing fallback + incomplete-timing qualification guard
- Added `TimingDataComplete`, `TimingDataSource`, and `TimingDataReason` to Scanner/Analyze audit output.
- When the primary session move / move-before-trigger ratio is unavailable, the Entry engine now attempts a conservative **Setup Origin → Trigger Anchor** fallback, accepted only when `origin <= trigger <= current price`.
- ARMED / CONFIRMED / TRIGGERED setups with a causal Trigger Anchor but no measurable/recoverable `MoveConsumedBeforeTriggerPct` are labeled **TIMING DATA INCOMPLETE** instead of `FRESH / QUALIFIED`.
- Incomplete timing cannot become `CurrentOpportunityQualified` or Q#. The visible recommendation becomes **TIMING DATA INCOMPLETE — NOT QUALIFIED FOR ENTRY**.
- Low-sample setups may remain in the research-only Emerging lane, but incomplete timing receives the lowest Emerging freshness tier so it cannot outrank fully measured FRESH setups.
- The existing 80% Move Consumed hard gate, No-Chase rules, Decision-score weights, Q/E numeric thresholds, Evidence rules, trade-plan geometry, valuation, and Feedback rules are unchanged.
- Decision diagnostics: `V3.9`; Trade Priority: `V3.9-TIMING-COMPLETENESS-FRESHNESS-FIRST`.
- `app.py` and `quant_engine.py` are synchronized to **V6.3.9.36**.
