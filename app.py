...
APP_VERSION = "6.3.9.16"
APP_BUILD_ID = "V63915-FRESHNESS-EVIDENCE-LATE-GUARDS-20260922-A"
...
# -----------------------------------------------------------------------------
# V6.3.9.16 Decision Ranking Engine V3.2 — sample-aware evidence qualification
#
...
def _apply_decision_ranking_v63916(df,scan_mode='Production 151'):
    """Ranking V3.2: de-duplicated families plus sample-aware qualification guards.
    ...
    def row_score(r):
        ...
        return pd.Series({
            ...
            'DecisionRankVersion':'V3.2',
        })

    rank_fields=z.apply(row_score,axis=1)
    for c in rank_fields.columns:z[c]=rank_fields[c]
    z=z.sort_values(['DecisionRankScore','DecisionRankEligible','SetupQualityScore','EntryTimingScore'],ascending=[False,False,False,False]).reset_index(drop=True)
    z['GlobalRank']=np.arange(1,len(z)+1)
    z['TradePriorityRank']=z['GlobalRank']
...
def _ensure_decision_ranking_v63916(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.2 and evidence guards into cached snapshots.
    ...
    the Scanner is rerun under V6.3.9.16.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    return _apply_decision_ranking_v63916(z,scan_mode)
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.2. Optimized mode contributes only
            # a bounded OOS modifier; overlapping decision families are not reweighted twice.
            res=_apply_decision_ranking_v63916(res,config.get('scan_mode','Production 151'))
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.15 migration: an older cached last_completed scan may survive a code
    # redeploy and therefore have no Ranking V3.2 columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3').all()
    )
    if not _rank_version_ok:
        full=_ensure_decision_ranking_v63916(full,str((last.get('config') or {}).get('scan_mode','Production 151')))
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    f"<div class='top5-head'><div class='top5-name'>#{rank} {ticker}</div>"
                ...
                _qual_html=f" • <b>QUAL {_e6395(_qual)}</b>"
                st.markdown(f"<div class='top5-rankline'><b>SETUP {_setup_txt}</b> • <b>EVIDENCE {_ev_txt}</b> • Ranking V3.2 • de-duplicated{_qual_html}{_bridge_html}{_price_meta}</div>",unsafe_allow_html=True)
                ...
                if not _ev_trade:
                    ...
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>Evidence guard:</b> {_e6395(_evq)} • {_ev_reason} • not eligible for CONFIRMED/ACTIONABLE or Top Opportunity.</div>",unsafe_allow_html=True)
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Ranking V3.2 {safe(r.get('DecisionRankScore'),1)}/100 • {r.get('DecisionRankReason','—')}")
        ...
        st.caption("Ranking V3.2: Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%, with each positive family counted once. Evidence, late/move-consumed and session freshness now also act as qualification guards/caps; stale or unproven setups cannot qualify as Top Opportunities. Market Cycle, Catalyst, Valuation, TOP and Opportunity remain context/research only.")
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Ranking V3.2.")
    ...
    primary_show=st.selectbox("Show",["ALL","TOP OPPORTUNITIES","ACTIONABLE NOW","WATCHLIST","RESEARCH"],key="show_primary_v63915")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.2 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.2 or Entry.")
    st.caption("Ranking V3.2 uses each decision family once: Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%. One Late/Chase/Exit risk family is applied, then evidence/timing/session-freshness qualification caps protect the Top ranks. Hourly stays an Entry diagnostic, not a second weight; Market Cycle, Catalyst, Valuation, legacy TOP and Opportunity are research/context only.")
