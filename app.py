...
APP_VERSION = "6.3.7"
APP_BUILD_ID = "V637-FOUR-SCORES-REGULAR-SIGNATURE-LAB-20260919-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • PERSISTENT FEEDBACK • HISTORICAL REPLAY • LARGE UNIVERSE</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Regular +2/+3/+5/+8% Signature Lab • Explosive Radar • Persistent Outcome Learning</div></div>""",unsafe_allow_html=True)
        ...
        try:
            ...
            for h in horizons:
                ...
                for target in targets:
                    ...
                    rows.append((d,str(ticker),str(market),str(timeframe),int(h),float(target),mask,hit,float(setup.get('setup_score_v2',0) or 0)))
        except Exception:
            continue
...
def _regular_signature_worker_v637(runtime,jid,cfg):
    try:
        ...
        names=_regular_signature_indicator_names_v637();cols=['Date','Ticker','Market','Timeframe','Horizon','Target','Mask','Hit','SetupScore'];raw=pd.DataFrame(records,columns=cols)
        if not raw.empty:
        ...
        if not raw.empty:
            for _,g in raw.groupby(['Market','Target','Lead','Timeframe'],sort=True):
                ii,cc,mm=_regular_signature_analyze_bucket_v637(g,names);inds.extend(ii);combs.extend(cc);summary.append(mm)
        ind_df=pd.DataFrame(inds);combo_df=pd.DataFrame(combs);summary_df=pd.DataFrame(summary)
        ...
        if not current_df.empty and not combo_df.empty:
            ...
            for _,cr in current_df.iterrows():
                ...
                for (target,lead),gg in good[good.Timeframe.eq(tf) & good.Market.eq(str(cr.Market))].groupby(['Target %','Lead Window']):
                    ...
                    mg=pd.DataFrame(matched);best=mg.sort_values(['Validation Lift x','Validation N'],ascending=[False,False]).iloc[0];cnt=len(mg);lift=float(best.get('Validation Lift x',np.nan));hr=float(best.get('Validation Hit Rate %',np.nan));score=float(np.clip(35+10*min(cnt,4)+20*max(0,(lift-1)) if np.isfinite(lift) else 35+10*min(cnt,4),0,100))
                    radar.append({'Ticker':cr.Ticker,'Market':cr.Market,'Target %':target,'Lead Window':lead,'Timeframe':tf,'Regular Signature Score':round(score,1),'OOS Positive Matches':cnt,'Best OOS Hit Rate %':hr,'Best OOS Lift x':lift,'Best Signature':best.Combination,'Setup Score':cr.get('Setup Score',np.nan),'Freshness':cr.get('Freshness',''),'Distribution Risk':cr.get('Distribution Risk',False),'Active Indicators':cr.get('Active Indicators','')})
        radar_df=pd.DataFrame(radar)
        if not radar_df.empty:radar_df=radar_df.sort_values(['Regular Signature Score','Best OOS Lift x','OOS Positive Matches'],ascending=[False,False,False]).reset_index(drop=True)
        ...
        meta={'Tab':'Regular Pre-Move Signature Lab','Version':APP_VERSION,'Scope':scope,'RequestedStocks':len(tickers),'DailyHistory':history,'HourlyHistory':hour_history if use_hourly else 'OFF','Targets':'2% / 3% / 5% / 8%','LeadWindows':'1H / 2H / 1D / 2D / 3D / 5D','DiscoverySplit':'70% chronological','ValidationSplit':'30% untouched OOS','SampleEvery':sample_every,'Meaning':'At each causal snapshot, tests whether the target is reached within the stated future window. No future feature is used.','ResearchOnly':'YES — does not change Production Entry automatically'}
        excel=_feedback_workbook_v629({'Signature Summary':summary_df,'Top OOS Signatures':top_df,'Combination Signatures':combo_df,'Indicator Signatures':ind_df,'Current Regular Radar':radar_df,'Skipped':pd.DataFrame(skipped)},meta)
        _lab_update_v603(runtime,jid,total+3,total+3,'Completed','Completed');_lab_publish_v603(runtime,jid,{'summary':summary_df,'top':top_df,'combinations':combo_df,'indicators':ind_df,'radar':radar_df,'skipped':pd.DataFrame(skipped),'excel':excel})
    except _LabCancelled:_lab_publish_v603(runtime,jid,None,'stopped')
    except Exception as ex:_lab_fail_v603(runtime,jid,ex)
...
with tab_feedback:
    ...
    st.caption("V6.3.7 research-only lab. It asks a simple causal question at every snapshot: which indicators are already present BEFORE an ordinary rise? It tests 1H/2H hourly windows and 1D/2D/3D/5D daily windows, then keeps only signatures that survive the untouched final 30% OOS validation.")
    ...
    if rspay:
        rss=rspay.get('summary',pd.DataFrame());rst=rspay.get('top',pd.DataFrame());rsr=rspay.get('radar',pd.DataFrame())
        ...
        if isinstance(rsr,pd.DataFrame) and not rsr.empty:
            ...
            st.caption("This is a research match to historically OOS-positive signatures, not a live trade trigger. Analyze/Scanner still controls timing and risk.")
