...
APP_VERSION = "6.0.0"
APP_BUILD_ID = "V600-PREMIUM-DIRECTIONAL-EXIT-FEEDBACK-20260916-A"
...
def scanner_excel_bytes(df,scan_meta=None):
    ...
    with pd.ExcelWriter(bio,engine="openpyxl") as writer:
        ...
        exp_cols=[c for c in ["Rank","Ticker","Signal","TradeStage","MovementStage","ExitPressure","ExitStage","VolumeContext","TimeAdjustedRVOL","ExplosiveScore","ExplosiveStage","HourlyConfirm","P5_5D","P10_5D","P15_5D","P15_5D_N","EntryScore","Prediction","BacktestQuality"] if c in out_df.columns]
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                m1.metric("Prediction",safe(r.get('Prediction'),1)); m2.metric("Reliability",safe(r.get('Reliability'),1)); m3.metric("Entry timing",safe(r.get('EntryScore'),1)); m4.metric("Hourly",safe(r.get('HourlyConfirm'),1))
                st.caption(f"Move {safe(r.get('MoveScore'),1)} • Explosive {safe(r.get('ExplosiveScore'),1)} • Volume {r.get('VolumeContext','N/A')} • Time-adjusted RVOL {safe(r.get('TimeAdjustedRVOL'))}x • Global #{int(r.get('GlobalRank',0) or 0)} • {r.get('Market','—')} #{int(r.get('MarketRank',0) or 0)} • Sector #{int(r.get('SectorRank',0) or 0)}")
                n=int(r.get('BacktestN',0) or 0)
                bt_txt=f"{safe(r.get('EmpiricalHitRate'),1)}% ({n} signals)" if n>=12 else f"LOW SAMPLE — {n} signals"
                st.caption(f"Backtest {bt_txt} • Lift {safe(r.get('SignalLift'))}x • RSI {safe(r.get('RSI14'),1)} • Data quality {r.get('DataQuality','OK')}{' • Split adjusted' if bool(r.get('SplitAdjusted',False)) else ''}")
...
with tab1:
    c1,c2,c3,c4=st.columns([1.35,1,1,1]); ticker=c1.text_input("Ticker","1196.HK"); hist=c2.selectbox("History",["3mo","6mo","1y"],1); horizon=c3.selectbox("Forecast days",[3,5,7,10],1); target=c4.selectbox("Target %",[3,4,5,6,8,10,15,20],3)
    with st.expander("Advanced research settings",expanded=False):
        threshold_mode=st.radio("Backtest Signal Threshold",["AUTO","MANUAL"],horizontal=True,help="Used only for historical backtest/calibration. It is not the live Trade Trigger.")
        manual_threshold=st.slider("Manual backtest threshold",60,80,66,disabled=threshold_mode!="MANUAL")
        st.caption("AUTO selects from historical evidence. Live decisions use Opportunity + Entry + Hourly + Exit Pressure, not this threshold.")
    ...
    if st.button(f"Run V{APP_VERSION} Dynamic analysis"):
        ...
        with st.spinner("Running Quant + Early Prediction + Entry Engine..."):
            ...
            if d is None or len(d)<35: st.error("Not enough market data.")
                ...
                h=confirmed_intraday_bars(fetch_ohlcv(t,"1mo","1h")); m15raw=confirmed_intraday_bars(fetch_ohlcv(t,"1mo","15m"))
                _,h,m15raw,ca_report=normalize_cross_timeframes(d,h,m15raw)
                hfeat=compute_features(h,True) if h is not None and len(h)>=30 else None
                m15feat=compute_features(m15raw,True) if m15raw is not None and len(m15raw)>=30 else None
                hs=he=np.nan
                ...
                auto_threshold,auto_threshold_table=_auto_backtest_threshold(f,int(horizon),float(target)/100)
                buy_threshold=int(manual_threshold) if threshold_mode=="MANUAL" else int(auto_threshold)
                bt=backtest_signal(f,int(horizon),float(target)/100,buy_threshold,0)
                bt=normalize_backtest_confidence(bt)
                # Precision layer uses confirmed closed bars only and split-normalized prices.
                ef=m15feat if m15feat is not None else (hfeat if hfeat is not None else f)
                ent=entry_timing(ef,qscore,escore)
                live_f=f.copy()
                if hfeat is not None and len(hfeat):
                    try:
                        _rv=float(hfeat.iloc[-1].get('time_adjusted_rvol',np.nan))
                        if np.isfinite(_rv): live_f.loc[live_f.index[-1],'robust_volume_ratio']=_rv; live_f.loc[live_f.index[-1],'volume_ratio']=_rv
                    except Exception:pass
                latest_live=score_latest(live_f,0)
                sig='RESEARCH SETUP'
                ex=explosive_latest(live_f,hfeat) if callable(explosive_latest) else {}
                timing=signal_timing_latest(live_f,hfeat) if callable(signal_timing_latest) else {}
                row=pd.DataFrame([{'Ticker':t,'Prediction':dyn.get('final_prediction',0),'EntryScore':ent.get('entry_score',0),'MoveScore':timing.get('move_score',0),'ExplosiveScore':ex.get('score',0),'Accel1D':timing.get('accel_1d',0),'Accel2D':timing.get('accel_2d',0),'Accel3D':timing.get('accel_3d',0),'HourlyConfirm':timing.get('hourly_confirmation',ex.get('hourly_confirmation',0)),'CalibrationConfidence':bt.get('confidence',0),'BacktestN':bt.get('n',0),'SignalLift':1.0,'ExitPressure':timing.get('exit_pressure',latest_live.get('exit_pressure',0)),'ExitStage':timing.get('exit_stage',latest_live.get('exit_stage','CLEAR')),'TimingStage':timing.get('timing_stage','—'),'PlanValid':ent.get('plan_valid',False),'EntryLow':ent.get('zone_low'),'EntryHigh':ent.get('zone_high'),'BreakoutTrigger':ent.get('trigger'),'Invalidation':ent.get('invalidation'),'Target1':ent.get('target1'),'Target2':ent.get('target2'),'Price':latest_live.get('price'),'VolumeContext':latest_live.get('volume_context','N/A'),'TimeAdjustedRVOL':(hfeat.iloc[-1].get('time_adjusted_rvol',np.nan) if hfeat is not None and len(hfeat) else latest_live.get('robust_volume_ratio',np.nan)),'RSI14':latest_live.get('rsi14',np.nan),'SplitAdjusted':ca_report.get('split_adjusted',False),'DataQuality':ca_report.get('data_quality','OK')}])
                decision_row=add_market_and_opportunity(row).iloc[0]
                phase=str(decision_row.get('MarketPhase','UNKNOWN')); cur=latest_live.get('price',np.nan); ch=latest_live.get('daily_change_pct',np.nan)
                currency='HKD' if t.endswith('.HK') else ('ILA' if t.endswith('.TA') else 'USD')
                st.markdown("<div class='section'>Decision cockpit</div>",unsafe_allow_html=True)
                p1,p2,p3,p4=st.columns([1.35,1,1,1])
                p1.metric("Last price",f"{cur:,.3f} {currency}" if np.isfinite(cur) else '—',delta=f"{ch:+.2f}%" if np.isfinite(ch) else None)
                p2.metric("TOP SCORE",f"{decision_row.get('TopScore',0):.1f}")
                p3.metric("Opportunity",f"{decision_row.get('OpportunityScore',0):.1f}")
                p4.metric("Exit Pressure",f"{decision_row.get('ExitPressure',0):.0f}",delta=decision_row.get('ExitStage','CLEAR'))
                st.caption(f"{phase} • {decision_row.get('Market','—')} • analyzed {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} • market data may be delayed")
                ...
                if ca_report.get('data_quality')!='OK': st.error("DATA QUALITY CHECK: cross-timeframe price scale mismatch remains. Trade levels are suppressed until the mismatch is resolved.")

                st.markdown(f"### Trade: :{'green' if decision_row.get('TradeStage')=='TRADE TRIGGER' else 'orange'}[{decision_row.get('TradeStage','—')}]  ·  Movement: {decision_row.get('MovementStage','—')}")
                st.caption("Why not Trade Trigger? "+str(decision_row.get('WhyNotTradeTrigger','—')))
                q1,q2,q3,q4=st.columns(4); q1.metric("Prediction",f"{dyn.get('final_prediction',np.nan):.1f}"); q2.metric("Reliability",f"{decision_row.get('Reliability',0):.1f}"); q3.metric("Entry timing",f"{ent.get('entry_score',0):.1f}"); q4.metric("Hourly",f"{timing.get('hourly_confirmation',np.nan):.1f}" if np.isfinite(timing.get('hourly_confirmation',np.nan)) else '—')
                r1,r2,r3,r4=st.columns(4); r1.metric("Move",f"{timing.get('move_score',np.nan):.1f}"); r2.metric("Explosive",f"{ex.get('score',np.nan):.1f}"); r3.metric("Volume context",latest_live.get('volume_context','N/A')); r4.metric("Time-adjusted RVOL",f"{decision_row.get('TimeAdjustedRVOL',np.nan):.2f}x" if np.isfinite(decision_row.get('TimeAdjustedRVOL',np.nan)) else '—')
                st.caption(f"Exit engine: {decision_row.get('ExitStage','CLEAR')} • high volume only supports the bullish score when price/flow context confirms accumulation. Distribution volume feeds Exit Pressure instead.")

                bt_value,bt_sample_label=backtest_display(bt)
                b1,b2,b3=st.columns(3); b1.metric("Backtest evidence",bt_value); b2.metric("Sample",f"{int(bt.get('n',0))} signals"); b3.metric("Backtest threshold",f"{buy_threshold} ({threshold_mode})")
                if int(bt.get('n',0) or 0)<12: st.caption("LOW SAMPLE: hit-rate percentage is intentionally de-emphasized and its influence on Reliability is capped.")
                ...
                if bool(ent.get('plan_valid',False)) and ca_report.get('data_quality')=='OK':
                    z1,z2,z3,z4=st.columns(4); z1.metric("Entry zone",f"{ent['zone_low']:.3f} – {ent['zone_high']:.3f}"); z2.metric("Trade trigger",f"> {ent['trigger']:.3f}"); z3.metric("Invalidation",f"< {ent['invalidation']:.3f}"); z4.metric("Targets",f"{ent['target1']:.3f} / {ent['target2']:.3f}")
                    st.caption(f"Entry status: {ent.get('status','—')} • Plan geometry validated: EntryLow ≤ EntryHigh, invalidation below entry, trigger at/above current price.")
                else:
                    st.warning("Trade plan suppressed: "+str(ent.get('plan_reason','data-quality validation failed')))
                ...
                analyze_summary=pd.DataFrame([{
                    'Ticker':t,'Version':APP_VERSION,'History':hist,'ForecastDays':horizon,'TargetPct':target,'BacktestThreshold':buy_threshold,'ThresholdMode':threshold_mode,'Price':latest_live.get('price'),'TopScore':decision_row.get('TopScore'),'OpportunityScore':decision_row.get('OpportunityScore'),'TradeStage':decision_row.get('TradeStage'),'MovementStage':decision_row.get('MovementStage'),'ExitPressure':decision_row.get('ExitPressure'),'ExitStage':decision_row.get('ExitStage'),'VolumeContext':latest_live.get('volume_context'),'TimeAdjustedRVOL':decision_row.get('TimeAdjustedRVOL'),'WhyNotTradeTrigger':decision_row.get('WhyNotTradeTrigger'),'SplitAdjusted':ca_report.get('split_adjusted'),'DataQuality':ca_report.get('data_quality'),
                ...
                analyze_sheets={'Summary':analyze_summary,'Auto Threshold Evidence':auto_threshold_table,'Quant Components':comp,'Early Components':ec,'Static vs Dynamic':compare_df,'Daily Features':f.reset_index()}
                ...
                # Chart comes LAST: simple first, advanced only when requested.
                st.markdown("<div class='section'>📊 Price chart</div>",unsafe_allow_html=True)
                chart_period=st.radio("Range",["5D","1M","3M","6M","1Y","MAX"],horizontal=True,index=2,key=f"simple_period_{t}")
                chart_tf="1D" if chart_period in ("3M","6M","1Y","MAX") else "1H"
                chart_layers=["EMA20","EMA50","Volume"]; chart_rsi=False; chart_macd=False
                with st.expander("Advanced chart",expanded=False):
                    chart_tf=st.selectbox("Timeframe",["15m","1H","1D","1W"],2,key=f"adv_tf_{t}")
                    chart_layers=st.multiselect("Layers",["EMA9","EMA20","EMA50","VWAP","Volume"],default=["EMA20","EMA50","Volume"],key=f"adv_layers_{t}")
                    ac1,ac2=st.columns(2); chart_rsi=ac1.checkbox("RSI",False,key=f"adv_rsi_{t}"); chart_macd=ac2.checkbox("MACD",False,key=f"adv_macd_{t}")
                ...
                if cd is not None and len(cd)>0:
                    ...
                    st.plotly_chart(chart(cf,f"{t} • {chart_tf} • {chart_period}",chart_layers,chart_rsi,chart_macd,trade_plan=ent if ca_report.get('data_quality')=='OK' else None,current_price=latest_live.get('price')),use_container_width=True,config=CHART_CONFIG,key=f"main_chart_{t}_{chart_tf}_{chart_period}")
                    st.caption("Simple mode: candles + last price + EMA20/EMA50 + volume. Entry/Trigger/Invalidation/Targets are overlaid only when the trade-plan data gate passes.")
                    if chart_capped: st.caption("Intraday display history is capped by the data provider. This affects the chart only, not model calculations.")
                else:
                    st.warning("Selected chart data was unavailable, so the daily analysis chart is shown instead.")
                    st.plotly_chart(chart(f,f"{t} • 1D • Analysis history",["EMA20","EMA50","Volume"],False,False,trade_plan=ent if ca_report.get('data_quality')=='OK' else None,current_price=latest_live.get('price')),use_container_width=True,config=CHART_CONFIG,key=f"fallback_chart_{t}")
...
with tab2:
    ...
    with st.expander("Advanced scanner settings",expanded=False):
        th=st.number_input("Backtest Signal Threshold",60,80,66,help="Historical evidence filter only; not a live buy threshold.")
    ...
    if st.button("Start / Restart Scan",key="scanner_start_v600"):
        ...
        if ok:st.success("Scan started server-side. Global Rank will be calculated across the complete 150-stock universe.")
        else:st.warning(msg)
...
with tab4:
    ...
    if st.button(f"Run V{APP_VERSION} Walk-Forward Validation", key="run_multi_validation"):
        ...
        if len(tickers)<3:
            ...
            bar=st.progress(0, text=f"Starting V{APP_VERSION} walk-forward validation...")
            ...
            if overlap_frames:
                ...
                focus=['Early: Directional Early RVOL','Quant: Directional volume','Quant: Liquidity','Quant: ADX trend strength']
    ...
    if st.button("Run Entry Validation on Validation 50",key="run_entry_validation"):
        ...
        if not rows:
            ...
            if skipped:
                with st.expander(f"Skipped stocks ({len(skipped)})"):
                    st.dataframe(pd.DataFrame(skipped,columns=['Ticker','Reason']),use_container_width=True,hide_index=True)
            st.caption("This test validates the V6 continuous Entry Score formula without changing production weights. Intraday 15m/1H history is provider-limited, so the multi-year walk-forward comparison uses daily historical features for a consistent apples-to-apples test.")
...
with tab6:
    ...
    st.caption("V6 direction-aware Explosive research. Tests bullish volume context and the fingerprint on chronological unseen blocks; it does not rewrite production weights.")
...
with tab7:
    ...
    if st.button("Run Research",key="run_60_research"):
        if not all(callable(x) for x in [historical_signal_timeline,threshold_optimization,adaptive_target_horizon_v2,atr_target_validation,pre_move_study,topk_daily_validation]):
            ...
            prog=st.progress(0.0,text="Starting multi-market research…")
...
with tab8:
    ...
    st.caption("Every completed live scan is snapshotted. Due outcomes are checked at 1D / 3D / 5D so we can audit false positives, false negatives, stage conversion and whether ranks/thresholds actually add value. Small samples never auto-retrain the production model.")
    if st.button("Refresh due outcomes now",key="feedback_refresh_v600"):
        with st.spinner("Evaluating due historical outcomes..."):
            n=_feedback_evaluate_due_v600(max_snapshots=80)
        st.success(f"Evaluated {n} due outcome windows.")
    sn,oc=_feedback_frames_v600()
    f1,f2,f3=st.columns(3); f1.metric("Snapshots",len(sn)); f2.metric("Evaluated windows",len(oc)); f3.metric("DB",str(_feedback_db_path_v600().name))
    if sn.empty:
        st.info("No live scan snapshots yet. Run Scanner once; the completed scan will be recorded automatically.")
    elif oc.empty:
        st.info("Snapshots are being collected. 1D/3D/5D outcome rows will appear as enough future trading data becomes available.")
        ...
        merged['End Return %']=100*pd.to_numeric(merged.end_return,errors='coerce'); merged['MFE %']=100*pd.to_numeric(merged.max_favorable,errors='coerce'); merged['MAE %']=100*pd.to_numeric(merged.max_adverse,errors='coerce')
        st.markdown("#### Live-signal audit")
        stage=merged.groupby(['horizon','trade_stage'],dropna=False).agg(Signals=('snapshot_id','count'),Avg_End_Return=('End Return %','mean'),Avg_MFE=('MFE %','mean'),Avg_MAE=('MAE %','mean'),Target1_Hit=('target1_hit','mean'),Invalidation_Hit=('invalidation_hit','mean')).reset_index()
        stage['Target1 Hit %']=100*stage.pop('Target1_Hit'); stage['Invalidation Hit %']=100*stage.pop('Invalidation_Hit')
        ...
        st.markdown("#### Recent evaluated snapshots")
        showcols=['ticker','market','horizon','trade_stage','movement_stage','top_score','opportunity','entry_score','hourly','exit_pressure','global_rank','End Return %','MFE %','MAE %','first_event']
        st.dataframe(merged[[c for c in showcols if c in merged]].head(100),use_container_width=True,hide_index=True)
        st.session_state['excel_feedback']=workbook_bytes({'Snapshots':sn,'Outcomes':oc,'Stage Summary':stage},{'Tab':'Feedback','Version':APP_VERSION})
        tab_download('excel_feedback','⬇️ Download Feedback audit to Excel',f'AI_Stock_Hunter_V{APP_VERSION}_Feedback')
    st.caption("Persistence note: the included SQLite tracker persists on the running Streamlit host. A redeploy/host reset may clear local storage; connect an external database later for permanent multi-deploy history.")
