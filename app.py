...
APP_VERSION = "6.0.5"
APP_BUILD_ID = "V605-STALE-QUOTE-HARD-GATE-20260917-A"
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        safe_anchor=live_snapshot.get('price',np.nan) if bool(live_snapshot.get('fresh')) and not bool(live_snapshot.get('scale_suspect')) else np.nan
        ...
        # V6.0.5: only a verified current-session quote may replace the Daily reference.
        # During OPEN market with no trustworthy intraday quote, keep the last official
        # close only as a non-live fallback and block LIVE confirmation.
...
with tab1:
    ...
    if snap and snap.get('payload'):
        ...
        price_fresh=bool(live_snapshot.get('fresh'));live_px=float(live_snapshot.get('price',np.nan)) if np.isfinite(float(live_snapshot.get('price',np.nan))) else np.nan;fallback_px=float(live_snapshot.get('fallback_price',np.nan)) if np.isfinite(float(live_snapshot.get('fallback_price',np.nan))) else np.nan
        if price_fresh and np.isfinite(live_px):cur=live_px;price_label='Current-session price'
        ...
        st.markdown("<div class='section'>Decision cockpit</div>",unsafe_allow_html=True);p1,p2,p3,p4=st.columns([1.35,1,1,1]);p1.metric(price_label,f"{cur:,.3f} {currency}" if np.isfinite(cur) else 'UNAVAILABLE',delta=f"{ch:+.2f}%" if price_fresh and np.isfinite(ch) else None);p2.metric("TOP SCORE",f"{decision.get('TopScore',0):.1f}");p3.metric("Opportunity",f"{decision.get('OpportunityScore',0):.1f}");p4.metric("Exit Pressure",f"{decision.get('ExitPressure',0):.0f}",delta=decision.get('ExitStage','CLEAR'))
        ...
        st.caption(f"{phase} • {decision.get('Market','—')} • {quote_source} @ {quote_stamp}{age_txt} • provider data may be delayed")
        if phase=='OPEN' and not price_fresh:
            ...
            st.error("LIVE PRICE UNAVAILABLE — no trustworthy current-session intraday bar. The number above is explicitly the last official close, not an online price. Live day-change, LIVE TRIGGER and Actionable Now are blocked."+stale_note)
