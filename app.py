...
APP_VERSION = "6.1.6"
APP_BUILD_ID = "V616-CAUSAL-TRIGGER-SETUP-ORIGIN-20260919-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • CAUSAL TRIGGER TIME • SETUP ORIGIN AUDIT • CHASE GUARD</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Causal Trigger Timing • Setup-Origin Audit • Retest Planning • OOS Evidence</div></div>""",unsafe_allow_html=True)
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        if phase_now=='AFTER-MARKET' and np.isfinite(_regular_close):
            regular_ent=entry_timing(f,qscore,escore,hourly_feat=hfeat,m15_feat=m15feat,current_price=_regular_close,market_regime=_market_regime_v610(t),previous_close=live_snapshot.get('prev_close',np.nan))
