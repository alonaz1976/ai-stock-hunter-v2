...
APP_VERSION = "6.1.0"
APP_BUILD_ID = "V610-ENTRY-TRIGGER-V3-20260917-A"
...
def _aggregate_entry_validation(rows):
    ...
    # V6.1 daily causal proxy for Entry Trigger V3. Hourly confirmation is
...
def _feedback_evaluate_due_v600(max_snapshots=30):
    ...
    try:
        ...
        # V6.1 migration: revisit old daily-ambiguous outcomes and resolve event order
...
def _entry_payload_v603(rows,audit,skipped,cfg,partial=False):
    summary,gate,market_gate=_aggregate_entry_validation(rows);sheets={'Entry Summary':summary,'Entry Trigger V3 Gate':gate,'Entry Gate by Market':market_gate,'Raw Observations':pd.DataFrame(rows),'Stock Audit':pd.DataFrame(audit)}
    if skipped:sheets['Skipped']=pd.DataFrame(skipped,columns=['Ticker','Reason'])
    excel=workbook_bytes(sheets,{'Tab':'Entry Validation','Market':cfg.get('market','CUSTOM'),'History':cfg['history'],'TargetPct':cfg['target'],'HorizonDays':cfg['horizon'],'MinTrainEvents':cfg['min_events'],'Status':'PARTIAL / STOPPED' if partial else 'COMPLETED','Note':'Quant is supporting evidence, not a standalone live buy threshold'})
    return {'summary':summary,'gate':gate,'market_gate':market_gate,'audit':pd.DataFrame(audit),'skipped':pd.DataFrame(skipped,columns=['Ticker','Reason']) if skipped else pd.DataFrame(),'excel':excel,'partial':partial,'observations':len(rows)//2}
...
with tab5:
    ...
    if snap and snap.get('payload'):
        ...
        if market_gate is not None and not market_gate.empty:
            ...
            st.caption("Market rows are diagnostic only. V6.1 does not auto-create market-specific thresholds from small samples.")
        if audit is not None and not audit.empty:st.markdown("#### Stock / fold audit");st.dataframe(audit,use_container_width=True,hide_index=True)
        if sk is not None and not sk.empty:
            with st.expander(f"Skipped stocks ({len(sk)})"):st.dataframe(sk,use_container_width=True,hide_index=True)
...
with tab8:
    ...
    if sn.empty:
        ...
        st.dataframe(stage,use_container_width=True,hide_index=True);st.markdown("#### Recent evaluated snapshots");showcols=['ticker','market','horizon','trade_stage','movement_stage','top_score','opportunity','entry_score','hourly','exit_pressure','global_rank','End Return %','MFE %','MAE %','first_event','Clean Outcome'];st.dataframe(merged[[c for c in showcols if c in merged]].head(100),use_container_width=True,hide_index=True)
