...
APP_VERSION = "6.1.4"
APP_BUILD_ID = "V614-AH-EXCEL-TZ-HOTFIX-20260919-B"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • AH-AWARE • TRIGGER ANCHOR V2 • CHASE GUARD • HOURLY OOS</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Trigger Audit • Retest Planning • OOS Evidence</div></div>""",unsafe_allow_html=True)
...
def add_market_and_opportunity(df):
    ...
    z['ActionableNow']=(((z['MarketPhase'].eq('OPEN')) & z['LiveStage'].eq('CONFIRMED ENTRY — LIVE')) | ((z['MarketPhase'].eq('PRE-MARKET')) & z['PMConfirmation'].eq('CONFIRMED') & z['TradeStage'].eq('CONFIRMED ENTRY'))) & (pd.to_numeric(z['ExitPressure'],errors='coerce').fillna(0)<55) & _plan_ok & _data_ok & _price_fresh
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        ent['after_hours_price']=_ah_price; ent['after_hours_move_pct']=_ah_change; ent['after_hours_volume_strength']=ah_snapshot.get('AHVolumeStrength',np.nan); ent['after_hours_fresh']=bool(ah_snapshot.get('AHFresh',False)); ent['total_move_including_ah_pct']=_total_extended
        ent['regular_session_entry_state']=ent.get('trigger_state',ent.get('status','WAIT'))
...
with tab_analyze:
    ...
    if snap and snap.get('payload'):
        ...
        if phase=='AFTER-MARKET':
            ...
            st.caption(f"Regular session move {f'{_ahr:+.2f}%' if np.isfinite(_ahr) else '—'} • After-hours move {f'{_ahc:+.2f}%' if np.isfinite(_ahc) else '—'} • Total vs prior close {f'{_aht:+.2f}%' if np.isfinite(_aht) else '—'} • AH volume strength {f'{_ahv:.2f}x' if np.isfinite(_ahv) else '—'} • AH quote {_ahts}{f' • age {_ahage:.0f}m' if np.isfinite(_ahage) else ''}")
        ...
        elif pay.get('excel_error'):st.warning("Analyze completed, but Excel export failed: "+str(pay.get('excel_error'))+". The analysis above is still valid.")
