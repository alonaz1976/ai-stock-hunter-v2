...
# -----------------------------------------------------------------------------
# V6.3.9.35 Decision Ranking Engine V3.8 — freshness-first + No-Chase consistency
#
...
def _apply_decision_ranking_v63919(df,scan_mode='Production 151'):
    """Ranking V3.8: de-duplicated score with freshness-first Emerging semantics and explicit No-Chase consistency.
    ...
    def row_score(r):
        ...
        # only; the Emerging Score formula and Ranking V3.8 weights are unchanged.
    ...
    # V6.3.9.35: Emerging is a NOW ranking, so freshness must outrank raw setup
    ...
    def _priority_class33(r):
        ...
        safety_block=(stage=='INVALIDATED' or hard_consumed or (not no_chase) or (not extension_ok)
                      ...
                      or any(k in timing for k in ('TOO LATE','RETEST ONLY','VERY LATE','DO NOT CHASE'))
    ...
    z['TradePriorityVersion']='V3.8-FRESHNESS-FIRST-NOCHASE-CONSISTENCY'
...
def _ensure_decision_ranking_v63919(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.8 and decision lanes into cached snapshots.
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.8. Optimized mode contributes only
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # redeploy and therefore have no Ranking V3.8 decision-lane columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3.8').all()
    )
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            with st.container(border=True):
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Decision Score {safe(r.get('DecisionRankScore'),1)}/100 • model V3.8 • {r.get('DecisionRankReason','—')}")
        ...
        table=table.rename(columns={
            'Rank':'View Rank',
            'DecisionRankScore':'Decision Score V3.8','DecisionRankCore':'Decision Core V3.8','DecisionScoreRank':'Decision Score Rank (research)','GlobalRank':'Current Global Rank',
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Decision Score V3.8.")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.8 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.8 or Entry.")
