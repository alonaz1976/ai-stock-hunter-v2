...
APP_VERSION = "6.1.4"
APP_BUILD_ID = "V614-TRIGGER-ANCHOR-AUDIT-20260918-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • TRIGGER ANCHOR V2 • CHASE GUARD • HOURLY OOS • FEEDBACK</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Trigger Audit • Retest Planning • OOS Evidence</div></div>""",unsafe_allow_html=True)
...
def _candidate_scope_v613(rec,market):
    ...
    source_ok=(source in ('ALL 151','CUSTOM','IMPORTED')) or (source.startswith('US') and market in ('NASDAQ','NYSE','US')) or ('HONG KONG' in source and market=='HONG KONG') or ('TEL AVIV' in source and market=='TEL AVIV')
    order=[]
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        ent=entry_timing(f,qscore,escore,hourly_feat=hfeat,m15_feat=m15feat,current_price=current_entry_price,market_regime=_market_regime_v610(t),previous_close=live_snapshot.get('prev_close',np.nan))
        if str(ca_report.get('data_quality','OK'))!='OK':
...
with tab_scan:
    ...
    scan_mode=st.radio("Scan mode",["Production 151","Optimized 151","Discovery"],horizontal=True,key="scan_mode_v613")
    universe_mode=st.radio("Display market",["ALL","US","HONG KONG","TEL AVIV","Custom"],horizontal=True,key="scanner_market_v613")
    ...
    if universe_mode=="Custom":
        custom=st.text_area("Custom tickers",DEFAULT_TICKERS,height=100,key="custom_scanner_v613");scan_universe=custom;prefilter_top=min(80,max(10,len([x for x in custom.split(',') if x.strip()])))
    ...
    show_mode=st.selectbox("Show",["ALL","CONFIRMED ENTRY","OPTIMIZED CONFIRMED","OPTIMIZED ARMED","ARMED","EXTENDED / RETEST","TOP OPPORTUNITIES","ACTIONABLE NOW","WATCHLIST"],key="show_v613")
...
with tab_feedback:
    ...
    stage=pd.DataFrame();disagreement=pd.DataFrame();recent_summary=pd.DataFrame();recent_features=pd.DataFrame();merged=pd.DataFrame()
