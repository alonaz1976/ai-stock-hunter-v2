...
APP_VERSION = "6.0.6"
APP_BUILD_ID = "V606-CURRENT-QUOTE-FALLBACK-20260917-A"
...
def _v544_scan_universe_dynamic(tickers,daily_period='6mo',use_hourly=True,hourly_period='1mo',horizon=5,target_pct=.03,min_turnover=0,buy_threshold=66,prefilter_top=30,progress_callback=None):
    ...
    for deep_i,(pre,ticker,f,ds) in enumerate(stage,1):
        ...
        try:
            ...
            liveq=None
            # Keep the full 150-stock scan light: only invoke the richer current-quote
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        # V6.0.6: display the best current/delayed quote found by the fallback chain,
...
with tab1:
    ...
    if snap and snap.get('payload'):
        ...
        if phase=='OPEN' and not display_current:
            ...
            st.error("CURRENT PRICE UNAVAILABLE — no trustworthy current-session quote from the fallback chain. The number above is the last official close only. LIVE TRIGGER and Actionable Now are blocked."+stale_note)
        elif phase=='OPEN' and display_current and not trade_fresh:
            st.warning("CURRENT/DELAYED PRICE FOUND — shown for price reference, but the feed is not timestamp-fresh enough for LIVE TRIGGER or Actionable Now.")
