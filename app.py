...
APP_VERSION = "6.3.1"
APP_BUILD_ID = "V631-HIGH-CONFIDENCE-CAUSAL-META-REPLAY-20260919-A"
...
def _feedback_replay_apply_meta_v631(df):
    ...
    Every probability for date D uses clean outcomes from dates strictly before D.
    It is therefore an OOS-style research probability rather than a fit to the row's
    own future result.  Same-day cross-sectional outcomes cannot leak into each other.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    ...
    clean_mask=z['CleanOutcome'].isin(['WIN','LOSS'])
    history_idx=[]
    for d,idxs in z.groupby('_date',sort=True).groups.items():
        train=z.loc[history_idx] if history_idx else pd.DataFrame()
        train=train[train['CleanOutcome'].isin(['WIN','LOSS'])].copy() if not train.empty else train
        if not train.empty:train['_win']=train['CleanOutcome'].eq('WIN').astype(float)
        for idx in list(idxs):
            r=z.loc[idx];n=len(train)
            # Weak neutral prior until enough strictly earlier resolved observations exist.
            if n:
                gw=float(train['_win'].mean());global_p=(float(train['_win'].sum())+8*.50)/(n+8)
            else:gw=np.nan;global_p=.50
            estimates=[];supports=[]
            if n:
                specs=[
                    ('FamilySignature',str(r.get('FamilySignature',''))),
                    ('FamilyCount',int(r.get('IndependentFamilyCount',0) or 0)),
                    ('FreshnessState',str(r.get('FreshnessState',''))),
                    ('ScoreBucket',int(float(r.get('SetupScoreV2',0))//10)),
                    ('CorePattern',f"{int(bool(r.get('CoreMomentum',False)))}{int(bool(r.get('CoreVolumeFlow',False)))}"),
                ]
                tt=train.copy();tt['ScoreBucket']=(pd.to_numeric(tt['SetupScoreV2'],errors='coerce').fillna(0)//10).astype(int);tt['CorePattern']=tt.apply(lambda q:f"{int(bool(q.get('CoreMomentum',False)))}{int(bool(q.get('CoreVolumeFlow',False)))}",axis=1)
                for col,val in specs:
                    if col not in tt:continue
                    g=tt[tt[col].astype(str)==str(val)]
                    nn=len(g)
                    if nn:
                        pp=(float(g['_win'].sum())+12*global_p)/(nn+12);estimates.append(pp);supports.append(nn)
            if estimates:
                ww=np.asarray([min(80,max(5,x)) for x in supports],float);p=float((np.dot(ww,np.asarray(estimates))+20*global_p)/(ww.sum()+20))
                support=int(max(supports))
            else:p=float(global_p);support=int(n)
            z.at[idx,'MetaProbability']=100*p;z.at[idx,'MetaSample']=support;z.at[idx,'MetaLift']=p/gw if np.isfinite(gw) and gw>0 else np.nan
            hc=bool(r.get('CandidateV2',False) and float(r.get('SetupScoreV2',0))>=72 and int(r.get('IndependentFamilyCount',0))>=4 and str(r.get('FreshnessState',''))=='FRESH' and float(r.get('MoveConsumedPct',999))<45 and not bool(r.get('DistributionRisk',False)) and support>=50 and p>=.65)
            z.at[idx,'HighConfidence']=hc
        # Only after all rows on this date were scored do their known replay outcomes become history.
        history_idx.extend([int(i) for i in idxs if bool(clean_mask.loc[i])])
    return z.drop(columns=['_date'],errors='ignore')
...
def _feedback_workbook_v629(sheets,meta=None):
    ...
    try:
        ...
        for sheet,status_header in [('Indicator Summary','סטטוס'),('Stock Summary','Model Status'),('Replay Indicators','Status'),('Replay Stocks','Model Status')]:
...
def _historical_replay_worker_v630(runtime,jid,cfg):
    """V6.3.1 causal daily replay with V2 candidate and OOS-style meta gate."""
    try:
        ...
        for n,t in enumerate(tickers,1):
            _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,n-1,total,t,'Historical Replay V2 • independent families')
            try:
        ...
        excel=_feedback_workbook_v629({'Replay Success':head,'Replay Gate Comparison':gates,'Replay Meta Calibration':cal,'Replay Indicators':inds,'Replay Stocks':stocks,'Replay Events':df,'Skipped':pd.DataFrame(skipped)},{'Tab':'Historical Replay V2','Version':APP_VERSION,'Scope':cfg.get('scope'),'History':history,'TargetPct':100*target,'HorizonDays':h,'SampleEverySessions':step,'RequestedStocks':total,'SuccessfulStocks':successful,'ResearchOnly':'YES — strict-date causal meta probability; daily replay never mixed with Live Feedback'})
        _lab_publish_v603(runtime,jid,{'events':df,'headline':head,'indicators':inds,'stocks':stocks,'gates':gates,'calibration':cal,'skipped':pd.DataFrame(skipped),'excel':excel,'successful':successful})
    except _LabCancelled:_lab_publish_v603(runtime,jid,None,'stopped')
    except Exception as e:_lab_fail_v603(runtime,jid,e)
...
with tab_feedback:
    ...
    st.markdown("#### 🕰️ Historical Replay V2 — causal High Confidence research")
    st.caption("Replay V2 reconstructs DAILY independent signal families, Freshness, Already-Moved/Extension and Distribution using only information available at each historical date. A strict-date Meta Probability uses only earlier resolved dates. Historical 15m/1h confirmations are never fabricated, and Replay remains separate from Live Feedback.")
    ...
    if not replay_events.empty:
        replay_excel=_feedback_workbook_v629({'Replay Success':replay_head,'Replay Gate Comparison':replay_gates,'Replay Meta Calibration':replay_cal,'Replay Indicators':replay_ind,'Replay Stocks':replay_stocks,'Replay Events':replay_events},{'Tab':'Historical Replay V2','Version':APP_VERSION,'ResearchOnly':'YES — strict-date causal meta probability; separate from Live Feedback'})
