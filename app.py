...
APP_VERSION = "6.3.9.39"
APP_BUILD_ID = "V63934-FEEDBACK-INTEGRITY-RANK-UNIFICATION-20260926-A"
...
# -----------------------------------------------------------------------------
# V6.3.9.39 Decision Ranking Engine V3.11 — timing semantics cleanup
#
...
def _apply_decision_ranking_v63919(df,scan_mode='Production 151'):
    """Ranking V3.11: V3.10 logic plus stage-aware TimingQualification semantics.
    ...
    def timing_qualification(r,late,chase,exit_risk):
        ...
        if _semantic_stage=='INVALIDATED':
            ...
            if str(timing_data_source or '').upper() in ('','UNAVAILABLE','NAN'):
                timing_data_source='NOT REQUIRED'
            timing_data_reason='Timing qualification is not applicable after the setup is invalidated'
        elif (_semantic_stage in ('WAIT','WATCH')) and (not np.isfinite(consumed)) and status=='FRESH / QUALIFIED':
    ...
    def row_score(r):
        ...
        guard_eligible=bool(ev_eligible and timing_eligible and timing_data_complete and data_ok and extension_ok and not invalid)
        ...
        current_opportunity_ok=bool(current_setup_ok and current_entry_ok and current_rank_ok and current_stage_ok and timing_data_complete)
        ...
        if validated_opportunity:
            ...
            lane='EVIDENCE VALIDATED / WAIT' if (timing_eligible and timing_data_complete and data_ok and extension_ok and not invalid) else 'EVIDENCE VALIDATED / BLOCKED'
        elif emerging_ok:
            lane='EMERGING SETUP'
        else:
    ...
    z['TradePriorityVersion']='V3.11-TIMING-SEMANTICS'
...
def _ensure_decision_ranking_v63919(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.11 and final-stage timing guards into cached snapshots.
...
def _feedback_movement_summary_v63934(primary):
    ...
    for label,mask in masks:
        ...
        rows.append({'Feature / Cohort':label,'Independent Event N':n,'Hit +3% %':100*h3,'Lift +3% x':h3/base3 if base3>0 else np.nan,'Hit +5% %':100*h5,'Lift +5% x':h5/base5 if base5>0 else np.nan,'Avg MFE %':float(g['_mfe'].mean()),'Avg MAE %':float(g['_mae'].mean()),'Avg End Return %':float(g['_end'].mean()),'Evidence':'LOW SAMPLE' if n<12 else 'OK'})
    return pd.DataFrame(rows)
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.11. Optimized mode contributes only
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # redeploy and therefore have no Ranking V3.11 decision-lane columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3.11').all()
    )
    ...
    if isinstance(full,pd.DataFrame) and not full.empty:
        ...
        st.caption(f"Decision lanes: VALIDATED OPPORTUNITIES {_validated} • EMERGING {_emerging} • EVIDENCE VALIDATED {_evidence_validated} • ACTIONABLE NOW {_action}. Q# = qualified now • E# = emerging now • EV# = historical Evidence rank only (not current entry priority).")
        ...
        for _,r in topcards.iterrows():
            with st.container(border=True):
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Decision Score {safe(r.get('DecisionRankScore'),1)}/100 • model V3.11 • {r.get('DecisionRankReason','—')}")
...
def _render_scanner_top_excel_v624(slot, show_mode, market_filter):
    ...
    with slot.container():
        if not last or not isinstance(last.get('result'),pd.DataFrame):
            ...
            return
        full=_attach_regular_signature_overlay_v638(_attach_pre_move_overlay_v628(last.get('result')));full=_ensure_decision_ranking_v63919(full,str((last.get('config') or {}).get('scan_mode','Production 151')));res=_filter_scanner_results_v599(full,show_mode,market_filter);meta=last.get('meta') or {}
        if res is None or res.empty:
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Decision Score V3.11.")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.11 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.11 or Entry.")
...
with tab_feedback:
    ...
    stage=pd.DataFrame();disagreement=pd.DataFrame();recent_summary=pd.DataFrame();recent_features=pd.DataFrame();early_entry_validation=pd.DataFrame();merged=pd.DataFrame();primary=pd.DataFrame();headline=pd.DataFrame();indicator_summary=pd.DataFrame();stock_summary=pd.DataFrame();integrity_summary=pd.DataFrame();movement_summary=pd.DataFrame()
    ...
    if sn.empty:
        ...
        integrity_summary=_feedback_integrity_summary_v63934(sn,primary);movement_summary=_feedback_movement_summary_v63934(primary)
