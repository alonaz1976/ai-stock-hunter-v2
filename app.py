...
def _decision_intelligence_enrich_v6394(df):
    ...
    for _,r in z.iterrows():
        ...
        try:_out=float(r.get('BearishVolumeEvidence',np.nan))
        except Exception:_out=np.nan
        if not np.isfinite(_out):
            try:_out=float(r.get('ExitPressure',0) or 0)
            except Exception:_out=0.0
        _out=_clip100(_out)
        vals.append({'MoneyFlowScore':mf,'MoneyFlowLabel':mfl,'MoneyFlowEvidence':mfe,'OutflowPressure':round(_out,1),'OutflowLabel':_outflow_label_v63921(_out),'MarketCycleScore':cs,'MarketCycleStage':cst,'MarketCycleReason':csr,'DecisionIntelligenceResearchOnly':True})
...
def _ensure_decision_ranking_v63919(df, scan_mode='Production 151'):
    ...
    the Scanner is rerun under V6.3.9.21.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
...
def _filter_scanner_results_v599(df,show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.21: when a market filter is active, E#/Q#/EV# are local to the
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.21 migration: an older cached last_completed scan may survive a code
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            with st.container(border=True):
                ...
                _outflow_cls='good' if _outflow<20 else ('warn' if _outflow<60 else 'bad')
                st.markdown(
                    f"<div class='top5-status'><b>{_px_txt}{_ccy_html}</b>{_scan_badge}{_bridge_badge} • {_e6395(_phase)}{_session_extra} • "
                    f"Entry <span class='top5-pill {trade_cls}'>{_e6395(trade_stage)}</span> • Outflow <span class='{_outflow_cls}'><b>{_outflow:.0f}/100 {_e6395(_outflow_label)}</b></span> • Exit <span class='{exit_cls}'><b>{_e6395(exit_stage)}</b></span></div>",
                    unsafe_allow_html=True,
                )
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Money Flow: {r.get('MoneyFlowEvidence','—')} • Outflow {_outflow:.0f}/100 {_outflow_label} (bearish-volume/distribution pressure; not a cash amount) • Cycle: {r.get('MarketCycleReason','—')}")
