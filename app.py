...
APP_VERSION = "6.3.9.3"
APP_BUILD_ID = "V6393-VALUATION-OVERLAY-20260922-A"
...
def _scanner_progress_percent(stage,i,total):
    ...
    if stage=='valuation':
        return int(np.clip(90+9*max(0,i)/max(1,total),90,99))
    return 99
...
with tab_scan:
    ...
    valuation_overlay=st.checkbox("Enable valuation overlay (research-only)",value=True,key="valuation_overlay_v6393",help="Fetches cached fundamentals and compares valuation multiples with peers. First run can take longer; results never change Entry/ARMED/CONFIRMED.")
    st.caption("Four layers: Valuation = peer-relative fundamentals research, Regular market-specific radar = +2/+3/+5/+8% research, Pre-Move = +5%/3D research radar, Live Entry = current trade timing. Research evidence never bypasses live risk/timing gates.")
    ...
    with b1:
        ...
        if st.button("▶ Start Scan",key="scanner_start_v612",use_container_width=True,disabled=disabled):
            ...
            config={'tickers':ts,'history':sh,'horizon':int(ho),'target':float(ta)/100,'threshold':66,'prefilter_top':int(prefilter_top),'universe_mode':universe_mode,'scan_mode':scan_mode,'optimizer_model':model if scan_mode!="Production 151" else None,'valuation_overlay':bool(valuation_overlay)}
