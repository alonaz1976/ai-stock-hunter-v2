...
APP_VERSION = "6.3.9.27"
APP_BUILD_ID = "V63927-L1-ORDER-BOOK-PRESSURE-20260924-A"
...
fetch_order_book_pressure = getattr(qe, "fetch_order_book_pressure", lambda ticker,ttl_seconds=20.0: {"state":"NO DATA","imbalance_pct":np.nan,"buy_pressure_pct":np.nan,"sell_pressure_pct":np.nan,"bid":np.nan,"ask":np.nan,"bid_size":np.nan,"ask_size":np.nan,"spread_pct":np.nan,"source":"UNAVAILABLE","timestamp_label":"—","age_minutes":np.nan,"market_state":"UNKNOWN","available":False,"timing_eligible":False,"level":"L1 TOP OF BOOK"})

@st.cache_data(ttl=20, show_spinner=False)
def _order_book_snapshot_v63927(ticker, market_phase):
    """Live L1 order-book timing snapshot for visible cards only.

    Closed/non-regular sessions intentionally return N/A. The indicator is a
    live timing confirmation only and never modifies core scores or ranking.
    """
    phase=str(market_phase or 'UNKNOWN').upper()
    if phase!='OPEN':
        return {'state':'N/A','imbalance_pct':np.nan,'buy_pressure_pct':np.nan,'sell_pressure_pct':np.nan,
                'bid':np.nan,'ask':np.nan,'bid_size':np.nan,'ask_size':np.nan,'spread_pct':np.nan,
                'source':f'{phase} — live book not used','timestamp_label':'—','age_minutes':np.nan,
                'market_state':phase,'available':False,'timing_eligible':False,'level':'L1 TOP OF BOOK'}
    try:
        return dict(fetch_order_book_pressure(str(ticker).upper().strip(),20.0) or {})
    except Exception:
        return {'state':'NO DATA','imbalance_pct':np.nan,'buy_pressure_pct':np.nan,'sell_pressure_pct':np.nan,
                'bid':np.nan,'ask':np.nan,'bid_size':np.nan,'ask_size':np.nan,'spread_pct':np.nan,
                'source':'UNAVAILABLE','timestamp_label':'—','age_minutes':np.nan,'market_state':'UNKNOWN',
                'available':False,'timing_eligible':False,'level':'L1 TOP OF BOOK'}

def _order_book_ui_v63927(ob):
    ob=ob if isinstance(ob,dict) else {}
    state=str(ob.get('state','NO DATA') or 'NO DATA').upper()
    eligible=bool(ob.get('timing_eligible',False))
    imb=pd.to_numeric(pd.Series([ob.get('imbalance_pct',np.nan)]),errors='coerce').iloc[0]
    if state=='BULLISH':cls='good';confirm='CONFIRM' if eligible else 'NOT LIVE'
    elif state=='BEARISH':cls='bad';confirm='CAUTION' if eligible else 'NOT LIVE'
    elif state=='NEUTRAL':cls='warn';confirm='NEUTRAL' if eligible else 'NOT LIVE'
    else:cls='muted';confirm='NO LIVE DATA'
    if np.isfinite(imb):
        side=f"Buy +{imb:.0f}%" if imb>0 else (f"Sell {abs(imb):.0f}%" if imb<0 else 'Balanced 0%')
    else:
        side='L1 unavailable'
    return state,cls,confirm,side
...
def _reconstruct_completed_daily_from_intraday_v63924(ticker, daily, market, intraday=None):
    ...
    try:
        ...
        fresh=_session_freshness_v63915(daily,m)
        expected=fresh.get('expected');latest=fresh.get('latest')
        if fresh.get('ok') or m!='TEL AVIV' or expected is None:
            return daily,meta
...
def _apply_decision_ranking_v63919(df,scan_mode='Production 151'):
    ...
    def evidence_qualification(r,evidence):
        ...
        if n < 8:return state,0.0,50.0,False,p
        if n < 12:return state,0.0,58.0,False,p
        if 'WEAK — GUARD' in state:return state,0.0,42.0,False,p
        if 'WEAK LIFT' in state:return state,0.0,50.0,False,p
        if 'LOW RELIABILITY' in state:return state,0.0,52.0,False,p
        return state,0.0,48.0,False,p
    ...
    def row_score(r):
        ...
        entry_hard_block=any(k in _entry_guard_text for k in ('TOO LATE','DO NOT CHASE','INVALIDATED'))
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            with st.container(border=True):
                ...
                _ob=_order_book_snapshot_v63927(_logical_ticker,_phase)
                _ob_state,_ob_cls,_ob_confirm,_ob_side=_order_book_ui_v63927(_ob)
                _session_extra=''
                ...
                st.markdown(
                    ...
                    f"<div class='top5-mini'><span class='lbl'>Order Book</span><span class='val {_ob_cls}'>{_e6395(_ob_state)}</span><span class='sub'>{_e6395(_ob_confirm)} • {_e6395(_ob_side)} • L1</span></div>"
                ...
                if not _ev_trade:
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>UNVALIDATED</b> • Evidence {_ev_n}/12 • {_e6395(_evq)}</div>",unsafe_allow_html=True)
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    _obb=_num6395(_ob.get('bid',np.nan));_oba=_num6395(_ob.get('ask',np.nan));_obbs=_num6395(_ob.get('bid_size',np.nan));_obas=_num6395(_ob.get('ask_size',np.nan));_obspr=_num6395(_ob.get('spread_pct',np.nan))
                    st.caption(f"Order Book Pressure (L1): {_ob_state} • {_ob_confirm} • {_ob_side} • Bid {f'{_obb:.4f}' if np.isfinite(_obb) else '—'} × {f'{_obbs:.0f}' if np.isfinite(_obbs) else '—'} • Ask {f'{_oba:.4f}' if np.isfinite(_oba) else '—'} × {f'{_obas:.0f}' if np.isfinite(_obas) else '—'} • Spread {f'{_obspr:.3f}%' if np.isfinite(_obspr) else '—'} • {_ob.get('source','UNAVAILABLE')} @ {_ob.get('timestamp_label','—')} • live-timing only, no score/rank change")
...
with tab_analyze:
    ...
    required_engine_api=["fetch_ohlcv","compute_features","score_latest","backtest_signal","entry_timing","score_row","early_score_row","fetch_order_book_pressure"];missing_engine_api=[name for name in required_engine_api if not hasattr(qe,name)];engine_ver=getattr(qe,"ENGINE_VERSION",None);engine_build=getattr(qe,"ENGINE_BUILD_ID",None)
    ...
    if snap and snap.get('payload'):
        ...
        _an_ob=_order_book_snapshot_v63927(t,phase);_an_ob_state,_an_ob_cls,_an_ob_confirm,_an_ob_side=_order_book_ui_v63927(_an_ob)
        ...
        st.markdown("<div class='top5-grid'>"+f"<div class='top5-mini'><span class='lbl'>Setup</span><span class='val'>{_an_setup:.0f}/100</span><span class='sub'>setup strength</span></div>"+f"<div class='top5-mini'><span class='lbl'>Entry</span><span class='val'>{_an_entry_score:.0f}/100</span><span class='sub'>Gates {int(ent.get('confirmed_conditions',0) or 0)}/{int(ent.get('total_conditions',6) or 6)}</span></div>"+f"<div class='top5-mini'><span class='lbl'>Hourly</span><span class='val'>{_an_hour_txt}</span><span class='sub'>timing</span></div>"+f"<div class='top5-mini'><span class='lbl'>Order Book</span><span class='val {_an_ob_cls}'>{_e6395(_an_ob_state)}</span><span class='sub'>{_e6395(_an_ob_confirm)} • {_e6395(_an_ob_side)} • L1</span></div>"+f"<div class='top5-mini'><span class='lbl'>Inflow</span><span class='val {_an_in_cls}'>{_an_in:.0f}/100</span><span class='sub'>{_e6395(_an_inlab)}</span></div>"+f"<div class='top5-mini'><span class='lbl'>Outflow</span><span class='val {_an_out_cls}'>{_an_out:.0f}/100</span><span class='sub'>{_e6395(_an_outlab)}</span></div>"+f"<div class='top5-mini'><span class='lbl'>Net Flow</span><span class='val {_an_net_cls}'>{_an_net:+.0f}</span><span class='sub'>{_e6395(_an_netlab)}</span></div>"+f"<div class='top5-mini'><span class='lbl'>Consumed</span><span class='val {_an_cons_cls}'>{_an_cons_txt}</span><span class='sub {_an_cons_cls}'>{_an_timing}</span></div>"+f"<div class='top5-mini'><span class='lbl'>Chase</span><span class='val {_an_chase_cls}'>{_e6395(_an_chase)}</span><span class='sub'>{_an_cr:.0f}/100</span></div>"+f"<div class='top5-mini'><span class='lbl'>R:R</span><span class='val'>{_an_rr1_txt}</span><span class='sub'>{_an_rr2_txt}</span></div>"+f"<div class='top5-mini'><span class='lbl'>Evidence</span><span class='val'>{_an_evprog}</span><span class='sub'>{_e6395(_an_evq)}</span></div></div>",unsafe_allow_html=True)
        if not bool(decision.get('EvidenceTradeGrade',decision.get('EvidenceGuardOK',False))):st.markdown(f"<div class='top5-alert'>⚠️ <b>UNVALIDATED</b> • Evidence {_an_evn}/12 • {_e6395(_an_evq)}</div>",unsafe_allow_html=True)
        ...
        with st.expander("Details / Why?",expanded=False):
            ...
            _aobb=_num_v6394(_an_ob.get('bid'),np.nan);_aoba=_num_v6394(_an_ob.get('ask'),np.nan);_aobbs=_num_v6394(_an_ob.get('bid_size'),np.nan);_aobas=_num_v6394(_an_ob.get('ask_size'),np.nan);_aobspr=_num_v6394(_an_ob.get('spread_pct'),np.nan)
            st.caption(f"Order Book Pressure (L1): {_an_ob_state} • {_an_ob_confirm} • {_an_ob_side} • Bid {f'{_aobb:.4f}' if np.isfinite(_aobb) else '—'} × {f'{_aobbs:.0f}' if np.isfinite(_aobbs) else '—'} • Ask {f'{_aoba:.4f}' if np.isfinite(_aoba) else '—'} × {f'{_aobas:.0f}' if np.isfinite(_aobas) else '—'} • Spread {f'{_aobspr:.3f}%' if np.isfinite(_aobspr) else '—'} • {_an_ob.get('source','UNAVAILABLE')} @ {_an_ob.get('timestamp_label','—')} • live-timing only, no score/rank change")
            _lrvol=decision.get('LiveIntradayRVOL',np.nan);_drvol=decision.get('DailyRobustRVOL',np.nan)
            st.caption(f"RVOL live {float(_lrvol):.2f}x" if np.isfinite(float(_lrvol)) else 'RVOL live —')
            st.caption(f"Daily Robust RVOL {float(_drvol):.2f}x • Reliability {decision.get('Reliability',0):.1f} • Evidence {decision.get('EvidenceState','UNPROVEN')}" if np.isfinite(float(_drvol)) else f"Daily Robust RVOL — • Reliability {decision.get('Reliability',0):.1f} • Evidence {decision.get('EvidenceState','UNPROVEN')}")
...
with tab_feedback:
    ...
    _fb_sn0,_fb_oc0=_feedback_frames_v600();_fb_imp=_feedback_improvement_tables_v63924(_fb_sn0,_fb_oc0,APP_VERSION);_fb_progress=_feedback_horizon_progress_v63926(_fb_sn0,_fb_oc0)
