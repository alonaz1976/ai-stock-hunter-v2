...
APP_VERSION = "6.1.2"
APP_BUILD_ID = "V612-OOS-OPTIMIZED-DISCOVERY-MARKET-STATUS-20260918-B"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • OOS OPTIMIZER • OPTIMIZED DISCOVERY • INSTITUTIONAL FLOW • FEEDBACK</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • OOS Learning • Optimized Discovery • Live Feedback</div></div>""",unsafe_allow_html=True)
...
DISCOVERY_UNIVERSE=','.join(dict.fromkeys((BROAD_200+','+HK_50+','+TASE_50).split(',')))
...
def _model_scope_for_ticker_v612(model,ticker):
    ...
    source=str(model.get('source_universe','ALL 151'))
    if source!='ALL 151':return None
...
def _optimized_score_from_row_v612(row,model,ticker=None):
    ...
    if not scope:return np.nan,np.nan,'NO MODEL'
...
def _apply_optimized_model_v612(df,model):
    ...
    def ost(r):
        match=float(r.get('OptimizedMatchPct',0) or 0); hourly=bool(r.get('HourlyEntryCheck',False));nochase=bool(r.get('NoChaseCheck',True));regime=bool(r.get('MarketRegimeCheck',True));xp=float(r.get('ExitPressure',0) or 0)
        if match>=100 and hourly and nochase and regime and xp<55:return 'OPTIMIZED CONFIRMED'
        if match>=100 and nochase and regime:return 'OPTIMIZED ARMED'
...
def _feedback_evaluate_due_v600(max_snapshots=30):
    ...
    try:
        ...
        for _,r in snap.iterrows():
            ...
            due=[h for h in (1,2,3,5) if (int(r.id),h) not in done_set and (datetime.utcnow().date()-scan_date).days>=h]
            if not due:continue
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        phase_now=_market_phase(_market_for_ticker_v612(t))
            ...
            try:
                ...
                try:
                    ...
                    if hraw is not None and len(hraw)>=80 and callable(hourly_lift_study):
                        hf=compute_features(hraw,True);hs=hourly_lift_study(hf,targets=(int(cfg.get('hourly_target',3)),),horizon_bars=tuple(cfg.get('hourly_bars',(1,2,4))),min_signals=10)
...
with tab_scan:
    ...
    scan_mode=st.radio("Scan mode",["Production 151","Optimized 151","Discovery"],horizontal=True,key="scan_mode_v612")
    model=_load_shadow_model_v612()
    if scan_mode!="Production 151":
        if model and model.get('scopes'):
            all_scope=model.get('scopes',{}).get('ALL',{});st.caption(f"Shadow model: {all_scope.get('status','RESEARCH ONLY')} • Fold4 OOS lift {all_scope.get('fold4_lift','—')} • source {model.get('source_universe','—')} • generated {model.get('created','—')}")
            if all_scope.get('status')!='SHADOW ELIGIBLE':st.warning("The latest learned model is research-only. Optimized results are shown for comparison and are not promoted to Production.")
        else:st.error("No learned V6.1.2 shadow model exists yet. Run Optimizer first; Production 151 remains available.")
    universe_mode=st.radio("Display market",["ALL","US","HONG KONG","TEL AVIV","Custom"],horizontal=True,key="scanner_market_v612")
    if scan_mode=="Production 151":scan_universe=VALIDATION_151;prefilter_top=151
    elif scan_mode=="Optimized 151":scan_universe=VALIDATION_151;prefilter_top=60
    else:scan_universe=DISCOVERY_UNIVERSE;prefilter_top=80
    if universe_mode=="Custom":
        custom=st.text_area("Custom tickers",DEFAULT_TICKERS,height=100,key="custom_scanner_v612");scan_universe=custom;prefilter_top=min(80,max(10,len([x for x in custom.split(',') if x.strip()])))
    c1,c2,c3=st.columns(3);sh=c1.selectbox("History",["3mo","6mo","1y"],1,key="sh_v612");ho=c2.selectbox("Forecast horizon",[1,2,3,5,7,10],2,key="ho_v612");ta=c3.selectbox("Target %",[1,2,3,4,5,6,8,10],2,key="ta_v612")
    show_mode=st.selectbox("Show",["ALL","CONFIRMED ENTRY","OPTIMIZED CONFIRMED","OPTIMIZED ARMED","ARMED","TOP OPPORTUNITIES","ACTIONABLE NOW","WATCHLIST"],key="show_v612")
    scanner_status=_scanner_status_v601();scanner_busy=scanner_status in ('running','stopping');b1,b2=st.columns([2,1])
    with b1:
        disabled=scanner_busy or (scan_mode!="Production 151" and not model)
    ...
    if scan_mode=="Discovery":st.caption(f"Discovery universe: {DISCOVERY_COUNT} symbols in this build. Stage 1 is daily/optimized; only the best {prefilter_top} receive hourly/15m deep analysis.")
    if _scanner_status_v601() in ('running','stopping'):_scanner_live_fragment_v599(show_mode,"ALL 151")
    else:
        market_filter={'ALL':'ALL 151','US':'US ALL','HONG KONG':'HONG KONG 50','TEL AVIV':'TEL AVIV 50','Custom':'ALL 151'}[universe_mode]
        _render_scanner_results_v599(show_mode,market_filter)
...
with tab_analyze:
    ...
    if snap and snap.get('payload'):
        ...
        st.caption(f"Institutional Flow {float(decision.get('InstitutionalFlowScore',np.nan)):.0f} ({decision.get('InstitutionalFlowLabel','—')}) • Optimized {float(decision.get('OptimizedScore',np.nan)):.1f} / Match {float(decision.get('OptimizedMatchPct',np.nan)):.0f}% • {decision.get('OptimizedStage','NO OPTIMIZER')}" if np.isfinite(float(decision.get('InstitutionalFlowScore',np.nan))) else 'Institutional Flow unavailable')
...
with tab_optimizer:
    ...
    st.caption("Learns entry weights from history with nested out-of-sample selection. Folds 2–3 select the shadow mix; Fold 4 remains untouched until the final OOS test. Production is never auto-changed.")
    o_market=st.radio("Optimization universe",["ALL 151","US 51","HONG KONG 50","TEL AVIV 50"],horizontal=True,key="o_market_v612")
    ...
    with c1:
        if st.button("▶ Run Entry Optimization",key="entry612_start",use_container_width=True,disabled=busy):
            uni={'ALL 151':VALIDATION_151,'US 51':US_51,'HONG KONG 50':HK_50,'TEL AVIV 50':TASE_50}[o_market]
    ...
    if snap and snap.get('payload'):
        pay=snap['payload'];opt=_coerce_dataframe_v608(pay.get('optimizer'));weights=_coerce_dataframe_v608(pay.get('optimizer_weights'));pm=_coerce_dataframe_v608(pay.get('premove_summary'));hl=_coerce_dataframe_v608(pay.get('hourly_summary'))
        if not opt.empty:
        ...
        if not weights.empty:
            st.markdown("#### Learned weights");st.dataframe(weights.round(3),use_container_width=True,hide_index=True)
        if not pm.empty:
        ...
        if pay.get('excel'):st.download_button("⬇️ Download Optimizer workbook",data=pay['excel'],file_name=f"AI_Stock_Hunter_V{APP_VERSION}_Optimizer_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",use_container_width=True,key="optimizer612_dl")

with tab_feedback:
    st.markdown("<div class='section'>↺ Feedback / Outcome Tracker</div>",unsafe_allow_html=True)
    st.caption("Every completed live scan is snapshotted. Due outcomes are checked at 1D / 2D / 3D / 5D. Recent resolved outcomes receive recency weighting, but small samples never auto-retrain production. Candidate changes stay in shadow until confirmed.")
    ...
    stage=pd.DataFrame();recent_summary=pd.DataFrame();recent_features=pd.DataFrame();merged=pd.DataFrame()
    ...
    excel_feedback=workbook_bytes({'Snapshots':sn,'Outcomes':oc,'Pending Summary':pending_summary,'Merged Audit':merged,'Stage Summary':stage,'Recent Weighted Summary':recent_summary,'Recent Feature Lift':recent_features},{'Tab':'Feedback','Version':APP_VERSION,'RecencyHalfLifeDays':int(fb_half_life),'Snapshots':len(sn),'EvaluatedWindows':len(oc)})
