...
APP_VERSION = "6.2.9"
APP_BUILD_ID = "V629-LARGE-UNIVERSE-FEEDBACK-SCORECARD-20260919-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • LARGE UNIVERSE • PRE-MOVE + LIVE ENTRY • FEEDBACK SCORECARD</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • 350-stock universe • Research Radar → Live Entry → Outcome Learning</div></div>""",unsafe_allow_html=True)
...
def _feedback_evaluate_due_v600(max_snapshots=30):
    ...
    try:
        con=_feedback_conn_v600(); snap=pd.read_sql_query('SELECT * FROM snapshots ORDER BY id DESC LIMIT 5000',con)
...
with tab_feedback:
    ...
    st.caption("Persistence note: V6.2.9 keeps up to 5,000 recent snapshot records in the active feedback evaluation queue. SQLite persists on the running Streamlit host; a redeploy/host reset may still clear local storage, so an external database remains the next step for permanent long-term learning.")
