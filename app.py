...
# -----------------------------------------------------------------------------
# V6.3.9.14 Decision Ranking Engine V3 — de-duplicated decision families
#
...
def _apply_decision_ranking_v63914(df,scan_mode='Production 151'):
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    def consumed_risk(r):
        ...
        # No lateness risk before ~35% of the pre-trigger move is consumed.
    ...
    def row_score(r):
        ...
        # One risk family: do not subtract overlapping lateness/chase/exit three times.
        late_chase=max(late,chase)
        risk_score=max(exit_risk,late_chase)
        ...
        if str(scan_mode) in ('Optimized 151','Discovery','OOS OPTIMIZED') and np.isfinite(optimized):
            # OOS optimizer is a bounded overlay, not another large duplicated score block.
            model_mod=float(np.clip((optimized-50.0)*0.10,-5.0,5.0))

        data_penalty=0.0 if str(r.get('DataQuality','OK'))=='OK' else 20.0
        invalid_penalty=20.0 if str(r.get('TradeStage',r.get('EntryTriggerState',''))).upper()=='INVALIDATED' else 0.0
        final=float(np.clip(core+model_mod-risk_penalty-data_penalty-invalid_penalty,0,100))
        reason=(f'Setup {setup:.1f} • Entry {entry:.1f} • Flow {money:.1f} • Evidence {evidence:.1f} • '
                f'R:R {rrq:.1f} • Risk {risk_score:.1f} (-{risk_penalty:.1f})')
        ...
        return pd.Series({
            ...
            # Compatibility/audit fields from V2. These are diagnostics only in V3;
            # they are NOT individually subtracted from the final rank.
            ...
            'DecisionRankVersion':'V3',
        })

    rank_fields=z.apply(row_score,axis=1)
    for c in rank_fields.columns:z[c]=rank_fields[c]
    z=z.sort_values(['DecisionRankScore','SetupQualityScore','EntryTimingScore'],ascending=False).reset_index(drop=True)
    z['GlobalRank']=np.arange(1,len(z)+1)
...
def _ensure_decision_ranking_v63914(df, scan_mode='Production 151'):
    """Backfill Ranking V3 into cached snapshots created by older releases.

    Streamlit cache_resource can survive a hot redeploy while the server process is
    still alive. Recompute Decision Intelligence if needed, then apply the current
    de-duplicated rank to the full snapshot before any Scanner view/filter.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    # repairs partially populated/stale ranking columns from an older cached snapshot.
    return _apply_decision_ranking_v63914(z,scan_mode)
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3. Optimized mode contributes only
            # a bounded OOS modifier; overlapping decision families are not reweighted twice.
            res=_apply_decision_ranking_v63914(res,config.get('scan_mode','Production 151'))
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.14 migration: an older cached last_completed scan may survive a code
    # redeploy and therefore have no Ranking V3 columns. Backfill and re-sort the
    ...
    if not _rank_version_ok:
        full=_ensure_decision_ranking_v63914(full,str((last.get('config') or {}).get('scan_mode','Production 151')))
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(f"<div class='top5-rankline'><b>SETUP {_setup_txt}</b> • <b>EVIDENCE {_ev_txt}</b> • Ranking V3 • de-duplicated{_bridge_html}{_price_meta}</div>",unsafe_allow_html=True)
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Ranking V3 {safe(r.get('DecisionRankScore'),1)}/100 • {r.get('DecisionRankReason','—')}")
        ...
        table=table.rename(columns={
            'Rank':'View Rank',
            'DecisionRankScore':'Ranking V3 Score','DecisionRankCore':'Ranking V3 Core','GlobalRank':'Global Rank',
        ...
        st.caption("Ranking V3: each positive decision family is weighted once — Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%. One combined Late/Chase/Exit risk penalty is then applied. Market Cycle, Catalyst, Valuation, TOP and Opportunity are context/research only.")
...
with tab_scan:
    ...
    m1,m2=st.columns(2)
    model_mode=m1.radio("Decision model",["PRODUCTION","OOS OPTIMIZED"],horizontal=True,key="decision_model_v63914")
    scan_depth=m2.radio("Scan depth",["FULL","FAST"],horizontal=True,key="scan_depth_v63914",help="FULL = hourly/15m deep analysis for every requested stock. FAST = daily prefilter first, then deep analysis only for the strongest candidates.")
    ...
    if model_mode=="OOS OPTIMIZED":
        model_horizon_choice=st.selectbox("Optimized model horizon",["AUTO BEST OOS BY MARKET","MATCH FORECAST HORIZON"],0,key="optimized_horizon_v63914",help="AUTO can use a different strongest eligible OOS horizon per market.")
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Ranking V3.")
    ...
    primary_show=st.selectbox("Show",["ALL","TOP OPPORTUNITIES","ACTIONABLE NOW","WATCHLIST","RESEARCH"],key="show_primary_v63914")
    if primary_show=="RESEARCH":
        show_mode=st.selectbox("Research view",["RESEARCH ALL","UNDERVALUED (RESEARCH)","REGULAR HIGH CONFIDENCE","REGULAR RADAR","STRONG PRE-MOVE CANDIDATE","PRE-MOVE CANDIDATE"],key="research_view_v63914")
    else:show_mode=primary_show
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63914",help="Research only. Does not change Ranking V3 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63914",help="Research only. News/earnings context is shown in Details but does not change Ranking V3 or Entry.")
    st.caption("Ranking V3 uses each decision family once: Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%, then one combined Late/Chase/Exit risk penalty. Hourly is shown as an Entry diagnostic, not a second weight. Market Cycle, Catalyst, Valuation, legacy TOP and Opportunity are research/context only.")
