...
APP_VERSION = "6.3.9.4"
APP_BUILD_ID = "V6394-DECISION-INTELLIGENCE-20260922-A"
...
# -----------------------------------------------------------------------------
# V6.3.9.4 Decision Intelligence Overlay
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(f"**Trade Entry:** :{'green' if str(r.get('TradeStage'))=='CONFIRMED ENTRY' else 'orange'}[**{r.get('TradeStage','—')}**]  •  **Movement:** {r.get('MovementStage','—')}  •  **Exit:** :{'red' if float(r.get('ExitPressure',0) or 0)>=55 else 'orange' if float(r.get('ExitPressure',0) or 0)>=35 else 'green'}[**{r.get('ExitStage','CLEAR')} {safe(r.get('ExitPressure'),0)}**]")
                ...
                di1.metric("Money Flow",f"{_mf:.0f}/100" if np.isfinite(_mf) else "—",delta=str(r.get('MoneyFlowLabel','NO DATA')),delta_color="off")
                di2.metric("Market Cycle",f"{_mc:.0f}/100" if np.isfinite(_mc) else "—",delta=str(r.get('MarketCycleStage','NO DATA')),delta_color="off")
                di3.metric("Catalyst",f"{_cat:.0f}/100" if np.isfinite(_cat) else "—",delta=str(r.get('CatalystLabel','NO DATA')),delta_color="off")
                di4.metric("Entry",f"{_entry:.0f}/100" if np.isfinite(_entry) else "—",delta=f"Hourly {safe(r.get('HourlyConfirm'),1)}",delta_color="off")
                ...
                else:st.caption(f"Money Flow: {r.get('MoneyFlowEvidence','—')} • Cycle: {r.get('MarketCycleReason','—')} • Catalyst: {r.get('CatalystEvidence','NO DATA')} / {_cer}")
                ...
                # V6.3.9.2: expose trigger lateness directly on every Top-5 card.
                _cons=pd.to_numeric(pd.Series([r.get('MoveConsumedBeforeTriggerPct',np.nan)]),errors='coerce').iloc[0]
                _mbt=pd.to_numeric(pd.Series([r.get('MoveBeforeTriggerPct',np.nan)]),errors='coerce').iloc[0]
                _st=pd.to_numeric(pd.Series([r.get('SinceTriggerPct',np.nan)]),errors='coerce').iloc[0]
                _te=str(r.get('TriggerEfficiencyLabel','RESEARCH • NO DATA'))
                _te_short=_te.split('•')[-1].strip() if '•' in _te else _te
                _cons_txt=f"{_cons:.1f}%" if np.isfinite(_cons) else '—'
                _mbt_txt=f"{_mbt:+.2f}%" if np.isfinite(_mbt) else '—'
                _st_txt=f"{_st:+.2f}%" if np.isfinite(_st) else '—'
                st.markdown(f"**Trigger timing:** **{_te_short}** • **Move consumed before trigger: {_cons_txt}** • Move before trigger {_mbt_txt} • Since trigger {_st_txt}")
                _cr=float(r.get('ChaseRiskScore',0) or 0); _sm=pd.to_numeric(pd.Series([r.get('SessionMovePct',np.nan)]),errors='coerce').iloc[0]
                ...
                _vs=pd.to_numeric(pd.Series([r.get('ValuationScore',np.nan)]),errors='coerce').iloc[0];_vd=pd.to_numeric(pd.Series([r.get('EstimatedDiscountPct',np.nan)]),errors='coerce').iloc[0];_fl=pd.to_numeric(pd.Series([r.get('FairValueLow',np.nan)]),errors='coerce').iloc[0];_fh=pd.to_numeric(pd.Series([r.get('FairValueHigh',np.nan)]),errors='coerce').iloc[0]
                if np.isfinite(_vs):
                ...
                _rr1=_num_v6394(r.get('LiveRR_T1'));_rr2=_num_v6394(r.get('LiveRR_T2'))
                m1.metric("Prediction Score",safe(r.get('Prediction'),1)); m2.metric("Hourly Confirmation",safe(r.get('HourlyConfirm'),1)); m3.metric("Evidence",str(r.get('EvidenceQuality','LOW'))); m4.metric("R:R",(f"1:{_rr1:.2f}" if np.isfinite(_rr1) else "—"),delta=(f"T2 1:{_rr2:.2f}" if np.isfinite(_rr2) else None),delta_color="off")
