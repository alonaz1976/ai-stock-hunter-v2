...
APP_VERSION = "6.3.4"
APP_BUILD_ID = "V634-MARKET-CONSENSUS-EXPLOSIVE-BENCHMARK-20260919-A"
...
def _explosive_benchmark_worker_v634(runtime,jid,cfg):
    try:
...
def _feedback_color_table_v629(df,status_col):
    ...
    def row_style(row):
        ...
        if v in ('WORKED','SUCCESS','OOS STRONG','OOS POSITIVE','FINAL STRONG','FINAL POSITIVE','PROMISING','OOS PROMISING'):style='background-color: rgba(53,212,154,.20); color: #b7f7dd; font-weight: 700'
        elif v in ('FAILED','FAILURE','WEAK'):style='background-color: rgba(255,100,124,.20); color: #ffc0ca; font-weight: 700'
...
def _feedback_workbook_v629(sheets,meta=None):
    ...
    try:
        ...
        colors={'WORKED':'C6EFCE','SUCCESS':'C6EFCE','OOS STRONG':'C6EFCE','OOS POSITIVE':'C6EFCE','FINAL STRONG':'C6EFCE','FINAL POSITIVE':'C6EFCE','PROMISING':'C6EFCE','OOS PROMISING':'C6EFCE','FAILED':'FFC7CE','FAILURE':'FFC7CE','WEAK':'FFC7CE','MIXED':'FFEB9C','OOS MIXED':'FFEB9C','FINAL MIXED':'FFEB9C'}
        for sheet,status_header in [('Indicator Summary','סטטוס'),('Stock Summary','Model Status'),('Replay Indicators','Status'),('Replay Stocks','Model Status'),('Combination Discovery','OOS Status'),('Market Consensus Combos','Status'),('Indicator Lift','Status'),('Combination Discovery','Status')]:
...
def _lab_runtimes_v603(build_id):
    ...
    names=('analyze','backtest','validate','entry','explosive','feedback','research_validator','pre_move','historical_replay','explosive_benchmark')
...
with tab_feedback:
    ...
    st.caption("V6.3.4 keeps the V3 70/30 combination audit and adds a stricter market-specific 60/20/20 test: 60% discovers combinations, the next 20% promotes only stable combinations, and the final 20% is untouched until the consensus gate is scored. Two or more family-signature-distinct combinations must agree for the main consensus candidate. Production Entry is unchanged.")
    ...
    st.caption("Research-only single-stock benchmark. It labels future +15% moves at 1D/2D/3D/5D only after the signal-date indicators are frozen. Use 1196.HK as the first benchmark, then test the discovered pattern on the wider Hong Kong universe before trusting it.")
    ...
    with xbc1:
        if st.button("▶ Run Explosive Benchmark",key="explosive_benchmark_start_v634",use_container_width=True,disabled=xb_busy):
            ok,msg=_start_lab_v603('explosive_benchmark',_explosive_benchmark_worker_v634,{'ticker':xb_ticker,'history':xb_hist,'target_pct':float(xb_target)/100.0,'horizons':(1,2,3,5)},total=4)
            if ok:st.success("Explosive Benchmark started server-side. You can leave the app and return later.");st.rerun()
            else:st.warning(msg)
    with xbc2:
        if st.button("■ Stop Benchmark",key="explosive_benchmark_stop_v634",use_container_width=True,disabled=not xb_busy or (xa and xa.get('status')=='stopping')):
