...
def add_market_and_opportunity(df):
    ...
    # V6.3.9 evidence guard: a zero-sample or explicitly weak guarded backtest
    # cannot remain a trade-grade CONFIRMED ENTRY. Keep the raw engine state for
    # audit and downgrade only the user-facing production stage.
    _btn=pd.to_numeric(z.get('BacktestN',0),errors='coerce').fillna(0)
    _evguard=z['EvidenceGuardOK'].fillna(True).astype(bool)
    z['EvidenceConfirmationGate']='OK'
    _nosample=z['TradeStage'].eq('CONFIRMED ENTRY') & _btn.le(0)
    _weakguard=z['TradeStage'].eq('CONFIRMED ENTRY') & (~_evguard)
    z.loc[_nosample,'EvidenceConfirmationGate']='NO SAMPLE — CONFIRMATION BLOCKED'
    z.loc[_weakguard,'EvidenceConfirmationGate']='WEAK EVIDENCE — CONFIRMATION BLOCKED'
    _evblock=_nosample|_weakguard
...
# -----------------------------------------------------------------------------
# V6.3.9.15 Decision Ranking Engine V3.1 — de-duplicated decision families
#
...
def _apply_decision_ranking_v63915(df,scan_mode='Production 151'):
    """Ranking V3.1: de-duplicated families plus qualification guards.
    ...
    def evidence_qualification(evidence):
        # Reliability in this model is deliberately conservative. 30+ is already
        # the upper evidence cohort in typical Scanner samples.
        if evidence>=30.0:return 'QUALIFIED',0.0,100.0,True
        if evidence>=20.0:return 'LOW EVIDENCE',5.0,55.0,False
        if evidence>=10.0:return 'VERY LOW EVIDENCE',9.0,48.0,False
        if evidence>0.0:return 'MINIMAL EVIDENCE',12.0,44.0,False
        return 'NO EVIDENCE',15.0,40.0,False
    ...
    def row_score(r):
        ...
        ev_status,ev_penalty,ev_cap,ev_eligible=evidence_qualification(evidence)
        ...
        reason=(f'Setup {setup:.1f} • Entry {entry:.1f} • Flow {money:.1f} • Evidence {evidence:.1f} '
                f'({ev_status}, -{ev_penalty:.1f}) • R:R {rrq:.1f} • Risk {risk_score:.1f} (-{risk_penalty:.1f})')
...
def _ensure_decision_ranking_v63915(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.1 into cached snapshots created by older releases.
    ...
    the Scanner is rerun under V6.3.9.15.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    return _apply_decision_ranking_v63915(z,scan_mode)
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.1. Optimized mode contributes only
            # a bounded OOS modifier; overlapping decision families are not reweighted twice.
            res=_apply_decision_ranking_v63915(res,config.get('scan_mode','Production 151'))
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # redeploy and therefore have no Ranking V3.1 columns. Backfill and re-sort the
    ...
    if not _rank_version_ok:
        full=_ensure_decision_ranking_v63915(full,str((last.get('config') or {}).get('scan_mode','Production 151')))
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(f"<div class='top5-rankline'><b>SETUP {_setup_txt}</b> • <b>EVIDENCE {_ev_txt}</b> • Ranking V3.1 • de-duplicated{_qual_html}{_bridge_html}{_price_meta}</div>",unsafe_allow_html=True)
                ...
                st.markdown(
                    ...
                    f"<div class='top5-mini'><span class='lbl'>Evidence</span><span class='val'>{_fmt6395(_evidence,0)}/100</span><span class='sub'>{_e6395(r.get('EvidenceQuality','LOW'))}</span></div>"
                ...
                _evq=str(r.get('EvidenceQualification','QUALIFIED') or 'QUALIFIED')
                if _evq!='QUALIFIED':
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>Evidence guard:</b> {_e6395(_evq)} • cannot qualify as a Top Opportunity until evidence improves.</div>",unsafe_allow_html=True)
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Ranking V3.1 {safe(r.get('DecisionRankScore'),1)}/100 • {r.get('DecisionRankReason','—')}")
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Ranking V3.1.")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.1 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.1 or Entry.")
    st.caption("Ranking V3.1 uses each decision family once: Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%. One Late/Chase/Exit risk family is applied, then evidence/timing/session-freshness qualification caps protect the Top ranks. Hourly stays an Entry diagnostic, not a second weight; Market Cycle, Catalyst, Valuation, legacy TOP and Opportunity are research/context only.")
