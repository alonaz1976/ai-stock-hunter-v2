...
APP_VERSION = "6.2.7"
APP_BUILD_ID = "V627-INDEPENDENT-FAMILIES-FRESHNESS-GUARD-20260919-A"
...
def _research_validator_universe_v624(scope, max_tickers):
    ...
    V6.2.7 adds an explicit NASDAQ scope so the Pre-Move lab can be run on the
    exact universe requested instead of using the broader US (NASDAQ + NYSE)
    bucket.
    """
    def arr(txt): return [x.strip().upper() for x in str(txt).split(',') if x.strip()]
    max_tickers=max(1,int(max_tickers))
    if scope=='NASDAQ': return arr(NASDAQ_50)[:max_tickers]
...
def _pre_move_discovery_worker_v625(runtime,jid,cfg):
    ...
    try:
        ...
        if not cand.empty:
            stage_order={'TRIGGERED':0,'ARMED':1,'BUILDING':2,'WATCH':3,'LATE / ALREADY MOVED':4}
...
with tab_optimizer:
    ...
    st.caption("Research only. The first 70% of each stock's history discovers singles/pairs; the later 30% is untouched OOS validation. V6.2.7 de-duplicates correlated signals into independent families and blocks late/already-moved candidates from ARMED/TRIGGERED. No Production Entry rule is changed.")
    ...
    if pmsnap and pmsnap.get('payload'):
        ...
        if not pmcand.empty:
            ...
            st.caption("PreMoveProbabilityPct is an empirical OOS hit rate of de-duplicated representative signals, not a guaranteed market probability. EarlySignalScore is freshness-adjusted; LATE / ALREADY MOVED cannot be ARMED/TRIGGERED.")
