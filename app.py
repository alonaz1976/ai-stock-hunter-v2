...
APP_VERSION = "6.3.6"
APP_BUILD_ID = "V635-EXPLOSIVE-INTEGRITY-UNIVERSE-RADAR-20260919-A"
...
def _explosive_current_radar_row_v636(ticker,feat,events,combos,target_pct=.15):
    ...
    score=float(stt['state_score']);score+=min(22,max(0,(analog_lift-1)*40)) if np.isfinite(analog_lift) else 0;score+=min(15,max(0,(best_lift-1)*30)) if np.isfinite(best_lift) else 0;score=float(np.clip(score,0,100))
    status='WATCH'
    if integrity=='DATA QUALITY BLOCK':status='BLOCKED — DATA QUALITY'
...
def _explosive_universe_worker_v636(runtime,jid,cfg):
    try:
        ...
        for i,t in enumerate(tickers,1):
            _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,i-1,total+2,t,'Explosive Radar V2 • first-leg + continuation analogs')
            try:
                d=fetch_ohlcv(t,history,'1d')
                if d is None or len(d)<120:raise ValueError('not enough daily history')
                f,audit=_explosive_prepare_features_v635(d,t)
        ...
        _lab_update_v603(runtime,jid,total,total+2,'Ranking','Explosive Radar V2 • ranking causal states');_lab_check_v603(runtime,jid)
        df=pd.DataFrame(rows)
        if not df.empty:
            ...
            df['_ord']=df.Status.map(order).fillna(10);df=df.sort_values(['_ord','Explosive Radar Score','Analog N'],ascending=[True,False,False]).drop(columns='_ord').reset_index(drop=True)
        counts=df.Status.value_counts().to_dict() if not df.empty else {}
        summary=pd.DataFrame([{'Scope':scope,'Requested':len(tickers),'Successful':len(df),'Skipped':len(skipped),'Target %':100*target,'History':history,'Strong Re-Acceleration':counts.get('STRONG RE-ACCELERATION CANDIDATE',0),'Re-Acceleration':counts.get('RE-ACCELERATION CANDIDATE',0),'Continuation Base':counts.get('CONTINUATION BASE CANDIDATE',0),'Strong Pre-Explosive':counts.get('STRONG PRE-EXPLOSIVE CANDIDATE',0),'Pre-Explosive':counts.get('PRE-EXPLOSIVE CANDIDATE',0),'ResearchOnly':'YES — Explosive Radar V2 is separate from regular Pre-Move +5%/3D and from Production Entry'}])
        ...
        _lab_update_v603(runtime,jid,total+1,total+2,'Excel','Explosive Radar V2 • workbook');_lab_check_v603(runtime,jid)
        excel=_feedback_workbook_v629({'Radar Summary':summary,'Current Explosive Radar V2':df,'Current State Summary':state_summary,'Skipped':pd.DataFrame(skipped)},{'Tab':'Explosive Universe Radar V2','Version':APP_VERSION,'Scope':scope,'History':history,'TargetPct':100*target,'Method':'Split-safe historical analogs with causal PRE-EXPLOSIVE / CONTINUATION BASE / RE-ACCELERATION states','RegularPreMove':'UNCHANGED — +5%/3D Pre-Move Discovery remains a separate normal-rise channel','ResearchOnly':'YES — no automatic Production promotion'})
...
def _feedback_color_table_v629(df,status_col):
    ...
    def row_style(row):
        ...
        if v in ('WORKED','SUCCESS','OOS STRONG','OOS POSITIVE','FINAL STRONG','FINAL POSITIVE','PROMISING','OOS PROMISING','STRONG EXPLOSIVE CANDIDATE','EXPLOSIVE CANDIDATE'):style='background-color: rgba(53,212,154,.20); color: #b7f7dd; font-weight: 700'
...
def _feedback_workbook_v629(sheets,meta=None):
    ...
    try:
        ...
        for sheet,status_header in [('Indicator Summary','סטטוס'),('Stock Summary','Model Status'),('Replay Indicators','Status'),('Replay Stocks','Model Status'),('Combination Discovery','OOS Status'),('Market Consensus Combos','Status'),('Indicator Lift','Status'),('Combination Discovery','Status'),('Current Explosive Radar','Status')]:
...
def _lab_runtimes_v603(build_id):
    ...
    names=('analyze','backtest','validate','entry','explosive','feedback','research_validator','pre_move','historical_replay','explosive_benchmark','explosive_universe')
...
with tab_feedback:
    ...
    st.markdown("#### 🌐 Explosive Universe Radar V2 — first leg + continuation/re-acceleration")
    st.caption("Research-only daily early-warning radar. It now looks for both a fresh first leg and a second explosive leg after a valid continuation reset. It does not replace the regular Pre-Move +5%/3D channel, and every candidate still requires Analyze/Scanner for live timing and risk.")
    ...
    if erpay:
        ...
        if isinstance(erd,pd.DataFrame) and not erd.empty:
            ...
            st.info(f"Current radar V2: {strong} STRONG • {reacc} re-acceleration • {cont} continuation-base • {pre} pre-explosive candidates. Research only; not automatic entries.")
