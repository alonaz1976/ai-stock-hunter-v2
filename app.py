...
# -----------------------------------------------------------------------------
# V6.3.9.33 Decision Ranking Engine V3.7 — current-first priority semantics
#
...
def _apply_decision_ranking_v63919(df,scan_mode='Production 151'):
    """Ranking V3.7: same de-duplicated score, with current-first trade priority semantics.
    ...
    def row_score(r):
        ...
        # only; the Emerging Score formula and Ranking V3.7 weights are unchanged.
    ...
    z['TradePriorityVersion']='V3.7-CURRENT-FIRST-UNIFIED'
...
def _ensure_decision_ranking_v63919(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.7 and decision lanes into cached snapshots.
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.7. Optimized mode contributes only
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # redeploy and therefore have no Ranking V3.7 decision-lane columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3.7').all()
    )
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            with st.container(border=True):
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Decision Score {safe(r.get('DecisionRankScore'),1)}/100 • model V3.7 • {r.get('DecisionRankReason','—')}")
        ...
        table=table.rename(columns={
            'Rank':'View Rank',
            'DecisionRankScore':'Decision Score V3.7','DecisionRankCore':'Decision Core V3.7','DecisionScoreRank':'Decision Score Rank (research)','GlobalRank':'Current Global Rank',
            'MarketRank':'Market Rank','SectorRank':'Sector Rank','TradePriorityRank':'Current Trade Priority','TradePriorityClass':'Trade Priority Class','QualifiedRank':'Qualified Opportunity Rank (Q#)','EmergingRank':'Emerging Rank (E#)','EvidenceRank':'Historical Evidence Rank (EV#)',
        })
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Decision Score V3.7.")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.7 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.7 or Entry.")
