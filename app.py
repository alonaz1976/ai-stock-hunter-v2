...
APP_VERSION = "6.3.9.24"
APP_BUILD_ID = "V63924-TASE-RECON-FEEDBACK-SIMPLIFY-20260924-A"
...
def _ensure_decision_ranking_v63919(df, scan_mode='Production 151'):
    ...
    the Scanner is rerun under V6.3.9.24.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
...
with tab_feedback:
    ...
    _fb_sn0,_fb_oc0=_feedback_frames_v600();_fb_imp=_feedback_improvement_tables_v63924(_fb_sn0,_fb_oc0,APP_VERSION)
    _fi1,_fi2,_fi3,_fi4=st.columns(4)
    _fi1.metric(f"V{APP_VERSION} resolved",int(_fb_imp.get('current_n',0)))
