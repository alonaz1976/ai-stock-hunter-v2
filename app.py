...
APP_VERSION = "6.3.2"
APP_BUILD_ID = "V632-REPLAY-FINALIZATION-PERFORMANCE-HOTFIX-20260919-A"
...
def _historical_replay_worker_v630(runtime,jid,cfg):
    ...
    try:
        ...
        final_steps=4;work_total=max(1,total+final_steps)
        rows=[];skipped=[];successful=0
        for n,t in enumerate(tickers,1):
            _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,n-1,work_total,t,'Historical Replay V2 • independent families')
            try:
        ...
        _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,total+1,work_total,'Replay database','Finalizing • saving replay events')
        _feedback_store_replay_v630(jid,rows,cfg,successful)
        _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,total+2,work_total,'Scorecards','Finalizing • calculating success / lift / calibration')
        head,inds,stocks,gates,cal=_feedback_replay_scorecards_v631(df)
        _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,total+3,work_total,'Excel','Finalizing • building workbook')
...
with tab_feedback:
    ...
    st.caption("Replay V2 reconstructs DAILY independent signal families, Freshness, Already-Moved/Extension and Distribution using only information available at each historical date. A strict-date Meta Probability uses only earlier resolved dates. Historical 15m/1h confirmations are never fabricated, and Replay remains separate from Live Feedback. V6.3.2 keeps progress below 100% until meta scoring, database save, scorecards and Excel finalization are actually finished.")
    ...
    with rr1:
        if st.button("▶ Run Historical Replay",key="historical_replay_start_v630",use_container_width=True,disabled=replay_busy):
            ...
            ok,msg=_start_lab_v603('historical_replay',_historical_replay_worker_v630,rcfg,total=len(replay_tickers)+4)
