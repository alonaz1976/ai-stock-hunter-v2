...
APP_VERSION = "6.2.5"
APP_BUILD_ID = "V625-PRE-MOVE-OOS-DISCOVERY-20260919-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • PRE-MOVE OOS DISCOVERY • CROSS-STOCK VALIDATION • TOP EXCEL</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Pre-Move Signal Discovery • 70/30 OOS Validation • Causal Research</div></div>""",unsafe_allow_html=True)
...
def _lab_runtimes_v603(build_id):
    ...
    names=('analyze','backtest','validate','entry','explosive','feedback','research_validator')
...
def _research_validator_universe_v624(scope, max_tickers):
    """Deterministic, market-balanced research universe selection."""
    def arr(txt): return [x.strip().upper() for x in str(txt).split(',') if x.strip()]
    max_tickers=max(1,int(max_tickers))
    if scope=='US': return arr(US_51)[:max_tickers]
    if scope=='HONG KONG': return arr(HK_50)[:max_tickers]
    if scope=='TEL AVIV': return arr(TASE_50)[:max_tickers]
    groups=[arr(US_51),arr(HK_50),arr(TASE_50)]; out=[]; k=0
...
def _pre_move_discovery_worker_v625(runtime,jid,cfg):
    """Cross-stock pre-move discovery with chronological 70/30 validation."""
    try:
        ...
        stock_frames=[]; skipped=[]; latest_meta=[]; total=max(1,len(tickers))
        for i,t in enumerate(tickers,1):
            ...
            try:
                ...
                f=compute_features(d).dropna(subset=['Close','High','Low']).copy()
                ...
                lr=f.iloc[-1]; latest_meta.append({'Ticker':t,'Market':market,'Price':float(lr.get('Close',np.nan)),'DailyChangePct':100*float(lr.get('ret1',np.nan)) if np.isfinite(float(lr.get('ret1',np.nan))) else np.nan})
        ...
        if not raw.empty:
            for scope,g in [('ALL',raw)]+[(m,raw[raw['Market'].astype(str)==m]) for m in sorted(raw['Market'].dropna().astype(str).unique())]:
                s=aggregate_pre_move_oos_v625(g,min_discovery_signals=max(10,min_d if scope=='ALL' else max(10,min_d//2)),min_validation_signals=max(6,min_v if scope=='ALL' else max(6,min_v//2)),min_discovery_lift=min_l)
                if not s.empty:s.insert(0,'Scope',scope);summaries.append(s)
        evidence=pd.concat(summaries,ignore_index=True) if summaries else global_summary.copy()
        # Current-candidate ranking uses only signals that passed discovery and did not fail OOS.
        ...
        candidates=[]; latest_df=pd.DataFrame(latest_meta)
        if not accepted.empty and not raw.empty:
            amap=accepted.set_index('Signal').to_dict('index')
            for t,g in raw.groupby('Ticker'):
                active=g[(g['CurrentActive']==True)&g['Signal'].isin(set(amap))].copy()
                if active.empty:continue
                vals=[]
                for _,rr in active.iterrows():
                    a=amap.get(rr['Signal'],{}); vl=float(a.get('ValidationLiftX',np.nan)); vh=float(a.get('ValidationHitRatePct',np.nan)); vn=float(a.get('ValidationSignals',0) or 0)
                    if not (np.isfinite(vl) and np.isfinite(vh)):continue
                    w=max(.05,vl-1.0)*max(1.0,math.sqrt(max(1.0,vn)))
                    vals.append((rr['Signal'],vl,vh,vn,w,a.get('OOSState','OOS MIXED')))
                if not vals:continue
                vals=sorted(vals,key=lambda x:(x[1],x[3]),reverse=True)
                wsum=sum(x[4] for x in vals); prob=sum(x[2]*x[4] for x in vals)/wsum if wsum>0 else np.nan
                best=max(x[1] for x in vals); nact=len(vals); sample=sum(x[3] for x in vals[:5])
                score=100*(.45*min(1,max(0,(best-1)/.8))+.25*min(1,math.log1p(sample)/math.log1p(120))+.30*min(1,nact/4))
                stage='TRIGGERED' if score>=70 and nact>=3 and best>=1.30 else ('ARMED' if score>=50 and nact>=2 else ('BUILDING' if score>=30 else 'WATCH'))
                meta=latest_df[latest_df['Ticker'].astype(str)==str(t)]
                m=meta.iloc[-1].to_dict() if not meta.empty else {}
                candidates.append({'Ticker':t,'Market':m.get('Market',g.iloc[0].get('Market','')),'Price':m.get('Price',np.nan),'DailyChangePct':m.get('DailyChangePct',np.nan),
                                   'EarlySignalScore':round(float(score),1),'EarlyStage':stage,'PreMoveProbabilityPct':round(float(prob),1) if np.isfinite(prob) else np.nan,
                                   'ActiveValidatedSignals':nact,'BestOOSLiftX':round(float(best),2),'OOSValidationSignalNTop5':int(sample),
                                   'StrongestPreMoveFeatures':' | '.join(x[0] for x in vals[:4]),'TargetHorizon':f'+{100*target:g}% within {horizon}D','ResearchOnly':True})
        cand=pd.DataFrame(candidates)
        if not cand.empty:cand=cand.sort_values(['EarlySignalScore','BestOOSLiftX','ActiveValidatedSignals'],ascending=False).reset_index(drop=True)
        skipped_df=pd.DataFrame(skipped)
        sheets={'Pre-Move OOS Evidence':evidence,'Current Candidates':cand,'Per-Stock Signal Audit':raw,'Skipped':skipped_df}
        excel=workbook_bytes(sheets,{'Tab':'Pre-Move OOS Discovery','Version':APP_VERSION,'Scope':cfg.get('scope'),'History':history,'RequestedStocks':len(tickers),'SuccessfulStocks':len(stock_frames),'TargetPct':100*target,'HorizonDays':horizon,'DiscoverySplit':'70%','ValidationSplit':'30%','MinDiscoverySignals':min_d,'MinValidationSignals':min_v,'MinDiscoveryLiftX':min_l,'ProductionImpact':'NONE — RESEARCH ONLY'})
        _lab_publish_v603(runtime,jid,{'evidence':evidence,'global_summary':global_summary,'candidates':cand,'raw':raw,'skipped':skipped_df,'excel':excel})
    except _LabCancelled:_lab_publish_v603(runtime,jid,None,'stopped')
    except Exception as e:_lab_fail_v603(runtime,jid,e)
...
def _render_scanner_top_excel_v624(slot, show_mode, market_filter):
    ...
    with slot.container():
        if not last or not isinstance(last.get('result'),pd.DataFrame):
            ...
            return
        full=last.get('result');res=_filter_scanner_results_v599(full,show_mode,market_filter);meta=last.get('meta') or {}
        if res is None or res.empty:
...
with tab_optimizer:
    ...
    rv_scope=rv1.selectbox("Research scope",["ALL","US","HONG KONG","TEL AVIV"],0,key="rv624_scope")
    rv_cap=rv2.selectbox("Stocks to test",[12,25,50,151],1,key="rv624_cap")
    ...
    st.caption("Research only. The first 70% of each stock's history discovers single indicators and indicator PAIRS; the later 30% is untouched OOS validation. Current candidates use only discovery-selected signals that remain positive/mixed OOS. No Production Entry rule is changed.")
    pm1,pm2,pm3,pm4=st.columns(4)
    pm_scope=pm1.selectbox("Pre-Move scope",["ALL","US","HONG KONG","TEL AVIV"],0,key="pm625_scope")
    pm_cap=pm2.selectbox("Stocks",[12,25,50,100,151],1,key="pm625_cap")
    ...
    if pmsnap and pmsnap.get('payload'):
        ...
        if not pmcand.empty:
            st.markdown("##### Current pre-move candidates — research only")
            st.dataframe(pmcand.head(30),use_container_width=True,hide_index=True)
            st.caption("PreMoveProbabilityPct is an empirical OOS hit rate of the active validated signals, not a guaranteed or calibrated market probability. EarlySignalScore summarizes OOS lift, sample and signal agreement.")
