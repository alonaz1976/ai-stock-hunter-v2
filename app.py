...
APP_VERSION = "6.3.9.17"
APP_BUILD_ID = "V63917-QUALIFIED-EMERGING-RANKS-20260922-A"
...
def _top5_current_price_v63915(ticker):
    ...
    try:
        ...
        expected=_expected_last_completed_session_v63915(market)
    except Exception:
...
def _session_freshness_v63915(frame, market):
    """Compare provider Daily data with the exchange's expected completed session."""
    expected=_expected_last_completed_session_v63915(market)
...
def _evidence_profile_v63916(row):
    """Trade-grade evidence profile using sample size + OOS lift + reliability.

    V6.3.9.16 closes the loophole where Reliability barely above 30 could be
    marked QUALIFIED even when the backtest had only a handful of signals.
    Evidence is now trade-grade only when the sample is large enough and the
    observed lift is not weak.  Low-sample setups remain visible for research
    and may stay ARMED, but they cannot become CONFIRMED/ACTIONABLE or qualify
    as TOP OPPORTUNITIES.
    """
    ...
    if n < 8:
        state='UNPROVEN'; ok=False; reason=f'Only {n} historical OOS signal(s); minimum 12 for trade-grade evidence'
    elif n < 12:
        state='PROVISIONAL — LOW SAMPLE'; ok=False; reason=f'{n} historical OOS signals; 12+ required for trade-grade evidence'
    elif not np.isfinite(lift):
        state='UNPROVEN — NO LIFT'; ok=False; reason='OOS lift is unavailable'
    elif lift < 0.75:
        state='WEAK — GUARD'; ok=False; reason=f'OOS lift {lift:.2f}x is materially below baseline'
    elif lift < 0.90:
        state='WEAK LIFT'; ok=False; reason=f'OOS lift {lift:.2f}x is below the 0.90x trade-grade floor'
    elif rel < 30.0:
        state='LOW RELIABILITY'; ok=False; reason=f'Reliability {rel:.1f}/100 is below the 30/100 trade-grade floor'
    elif n >= 20 and lift >= 1.20 and rel >= 50.0:
        state='STRONG'; ok=True; reason=f'{n} OOS signals • lift {lift:.2f}x • reliability {rel:.1f}'
    elif lift >= 1.05 and rel >= 35.0:
        state='SUPPORTIVE'; ok=True; reason=f'{n} OOS signals • lift {lift:.2f}x • reliability {rel:.1f}'
    else:
        state='MIXED / QUALIFIED'; ok=True; reason=f'{n} OOS signals • lift {lift:.2f}x • reliability {rel:.1f}'

    return {
        'EvidenceState':state,
        'EvidenceGuardOK':bool(ok),
        'EvidenceTradeGrade':bool(ok),
...
def add_market_and_opportunity(df):
    ...
    # V6.3.9.16 evidence guard: CONFIRMED ENTRY is a trade-grade state, so it
    # requires 12+ OOS signals, acceptable lift and minimum reliability. Research
    # setups with smaller/weak samples stay visible, but are downgraded to ARMED.
...
def scanner_excel_bytes(df,scan_meta=None):
    ...
    with pd.ExcelWriter(bio,engine="openpyxl") as writer:
        ...
        di_cols=[c for c in ['Ticker','Market','Sector','TradePriorityRank','QualifiedRank','EmergingRank','DecisionLane','EmergingSetupEligible','EmergingSetupScore','EmergingSetupReason','DecisionRankScore','DecisionRankCore','SetupQualityScore','EntryTimingScore','DecisionEvidenceScore','DecisionRiskScore','DecisionRankSetupQuality','DecisionRankEntryTiming','DecisionRankMoneyFlow','DecisionRankEvidence','DecisionRankRRQuality','DecisionRankRiskPenalty','DecisionRankEvidencePenalty','DecisionRankQualificationCap','DecisionRankEligible','DecisionRankQualification','EvidenceQualification','EvidenceQualificationReason','EvidenceTradeGrade','EvidenceSampleN','EvidenceLiftX','TimingQualification','DataFreshnessQualification','DecisionRankModelModifier','DecisionRankDataPenalty','DecisionRankStageModifier','DecisionRankTimingPenalty','DecisionRankChasePenalty','DecisionRankExitPenalty','DecisionRankReason','DecisionRankVersion','LegacyTradePriorityRank','LegacyGlobalRank','TopScore','OpportunityScore','TradeStage','EntryTriggerState','MoneyFlowScore','MoneyFlowLabel','MoneyFlowEvidence','MarketCycleScore','MarketCycleStage','MarketCycleReason','CatalystScore','CatalystLabel','CatalystEvidence','CatalystTopHeadline','CatalystRecentNewsCount','CatalystEventRisk','CatalystCoverage','EntryScore','LiveActionabilityScore','HourlyConfirm','MoveConsumedBeforeTriggerPct','ChaseRiskScore','ChaseRiskLabel','LiveRR_T1','LiveRR_T2','ExitPressure','ExitStage'] if c in out_df.columns]
        if di_cols:_excel_safe_df(out_df[di_cols]).to_excel(writer,index=False,sheet_name="Decision Intelligence")
        valuation_cols=[c for c in ['Ticker','Market','Sector','Price','Currency','ValuationScore','ValuationLabel','EstimatedDiscountPct','FairValueLow','FairValueMid','FairValueHigh','ValuationEvidence','ValuationArchetype','ValuationMetricCount','ValuationPeerCount','ValuationPeerScope','TrailingPE','ForwardPE','PriceToSales','PriceToBook','EVToEBITDA','EVToRevenue','FCFYieldPct','PEGRatio','RevenueGrowth','EarningsGrowth','ReturnOnEquity','ProfitMargins','ValuationMethod'] if c in out_df.columns]
...
def _valuation_peer_series_v6393(df,row_idx,metric):
    r=df.loc[row_idx];market=str(r.get('Market',''));sector=str(r.get('Sector',''))
    ...
    same_sector=df.get('Sector',pd.Series('',index=df.index)).astype(str).eq(sector)
    peers=good[same_market & same_sector].dropna()
    peer_scope='MARKET + SECTOR'
    if len(peers)<5:
        peers=good[same_market].dropna();peer_scope='MARKET'
    if len(peers)<5:
        peers=good.dropna();peer_scope='SCAN UNIVERSE'
...
def _valuation_enrich_scan_v6393(df,progress_callback=None):
    ...
    for idx,r in z.iterrows():
        price=_valuation_num_v6393(r.get('Price'));weights=_valuation_metric_weights_v6393(str(r.get('ValuationArchetype')))
        ...
        if np.isfinite(price) and price>0 and len(comps)>=2:
            ...
            fair_mid=price*wm;fair_low=price*float(np.quantile(multipliers,.25));fair_high=price*float(np.quantile(multipliers,.75))
            if fair_low>fair_high:fair_low,fair_high=fair_high,fair_low
        ...
        out.append({'ValuationScore':score,'ValuationLabel':label,'EstimatedDiscountPct':discount,'FairValueMid':fair_mid,'FairValueLow':fair_low,'FairValueHigh':fair_high,'ValuationEvidence':evidence,'ValuationMetricCount':metric_n,'ValuationPeerCount':min_peers,'ValuationPeerScope':' / '.join(dict.fromkeys(scopes)) if scopes else '—','ValuationMethod':method,'ValuationResearchOnly':True})
...
def _decision_intelligence_enrich_v6394(df):
    ...
    for _,r in z.iterrows():
        mf,mfl,mfe=_money_flow_fields_v6394(r)
        cs,cst,csr=_market_cycle_fields_v6394(r,mf)
        vals.append({'MoneyFlowScore':mf,'MoneyFlowLabel':mfl,'MoneyFlowEvidence':mfe,'MarketCycleScore':cs,'MarketCycleStage':cst,'MarketCycleReason':csr,'DecisionIntelligenceResearchOnly':True})
...
# -----------------------------------------------------------------------------
# V6.3.9.17 Decision Ranking Engine V3.3 — qualified + emerging decision lanes
#
...
def _apply_decision_ranking_v63917(df,scan_mode='Production 151'):
    """Ranking V3.3: de-duplicated score plus separate validated/emerging lanes.

    Positive information is still counted once. Evidence, lateness and freshness
    now also act as *qualification guards* so a mathematically strong but stale,
    unproven or already-consumed setup cannot sit near the top as a fresh opportunity.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    def evidence_qualification(r,evidence):
        ...
        ok=bool(p.get('EvidenceTradeGrade',False))
        ...
        if ok:
            if state=='STRONG': return state,0.0,100.0,True,p
            if state=='SUPPORTIVE': return state,0.0,100.0,True,p
            return state,1.5,100.0,True,p
        if n < 8:return state,8.0,50.0,False,p
        if n < 12:return state,5.0,58.0,False,p
        if 'WEAK — GUARD' in state:return state,12.0,42.0,False,p
        if 'WEAK LIFT' in state:return state,8.0,50.0,False,p
        if 'LOW RELIABILITY' in state:return state,7.0,52.0,False,p
        return state,9.0,48.0,False,p
    ...
    def timing_qualification(r,late,chase,exit_risk):
        ...
        if np.isfinite(consumed) and consumed>=70.0:
            eligible=False
            if retest_ok:
                status='RETEST ONLY — PRIOR MOVE CONSUMED';cap=min(cap,58.0)
            elif consumed>=100.0:
                status='LATE — MOVE FULLY CONSUMED';cap=min(cap,35.0)
            elif consumed>=85.0:
                status='VERY LATE';cap=min(cap,42.0)
            else:
                status='LATE';cap=min(cap,50.0)
    ...
    def row_score(r):
        ...
        raw_final=core+model_mod-risk_penalty-ev_penalty-data_penalty-invalid_penalty
        qualification_cap=min(ev_cap,timing_cap,data_cap,invalid_cap)
        final=float(np.clip(min(raw_final,qualification_cap),0,100))

        eligible=bool(ev_eligible and timing_eligible and data_ok and extension_ok and not invalid)
        qparts=[]
        ...
        qualification='QUALIFIED' if not qparts else ' • '.join(qparts)
        ...
        # V6.3.9.17: a second, research-only lane keeps technically strong low-sample
        # setups visible without mixing them with validated picks.  This score omits
        ...
        low_sample_only=(not ev_eligible) and (_evn < 12) and (('UNPROVEN' in ev_status) or ('PROVISIONAL' in ev_status))
        emerging_ok=bool(low_sample_only and timing_eligible and data_ok and extension_ok and (not invalid) and exit_risk < 55.0 and emerging_score >= 55.0)
        if emerging_ok:
        ...
        lane='VALIDATED PICK' if eligible else ('EMERGING SETUP' if emerging_ok else 'RESEARCH / BLOCKED')

        reason=(f'Setup {setup:.1f} • Entry {entry:.1f} • Flow {money:.1f} • Evidence {evidence:.1f} '
                f'({ev_status}, N={_evn}, Lift={_evlift_txt}, -{ev_penalty:.1f}) • R:R {rrq:.1f} • Risk {risk_score:.1f} (-{risk_penalty:.1f})')
        if qualification_cap<100:reason+=f' • qualification cap {qualification_cap:.0f}'
        ...
        return pd.Series({
            ...
            'DecisionRankVersion':'V3.3',
        })

    rank_fields=z.apply(row_score,axis=1)
    for c in rank_fields.columns:z[c]=rank_fields[c]

    # Overall rank remains the raw V3.3 score across the complete scan.  Separate
    # Qualified/Emerging ranks stop low-sample names from visually outranking a
    # trade-grade setup simply because their technical score is stronger.
    ...
    _qm=z['DecisionRankEligible'].fillna(False).astype(bool)
    ...
    _eidx=z[_em].sort_values(['EmergingSetupScore','DecisionRankScore','SetupQualityScore'],ascending=[False,False,False]).index.tolist()
    ...
    # TradePriorityRank is the UI priority: validated picks first, then emerging
    # low-sample setups, then the remaining research/blocked rows.  GlobalRank
    # still preserves the raw all-stock score for audit/comparison.
    _rest=[i for i in z.index if i not in set(_qidx) and i not in set(_eidx)]
...
def _ensure_decision_ranking_v63917(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.3 and decision lanes into cached snapshots.
    ...
    the Scanner is rerun under V6.3.9.17.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    return _apply_decision_ranking_v63917(z,scan_mode)
...
def _feedback_import_scanner_workbook_v639(uploaded):
    ...
    try:
        ...
        cfg={'horizon':horizon,'target':target_pct/100.0,'scan_mode':scan_mode,'_snapshot_ts_utc':ts_utc,'_snapshot_source':f'IMPORTED XLSX: {name}','_preserve_imported_fields':True}
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.3. Optimized mode contributes only
            # a bounded OOS modifier; overlapping decision families are not reweighted twice.
            res=_apply_decision_ranking_v63917(res,config.get('scan_mode','Production 151'))
...
def _filter_scanner_results_v599(df,show_mode,market_filter="ALL 151"):
    ...
    if show_mode=='STRONG PRE-MOVE CANDIDATE':
    ...
    elif show_mode=='VALIDATED PICKS':
        _qual=z.get('DecisionRankEligible',pd.Series(False,index=z.index)).fillna(False).astype(bool)
        z=z[_qual]
        ...
        # VALIDATED PICKS and EMERGING SETUPS separately.
    ...
    if show_mode=='VALIDATED PICKS' and 'QualifiedRank' in z.columns:
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.17 migration: an older cached last_completed scan may survive a code
    # redeploy and therefore have no Ranking V3.3 decision-lane columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3.3').all()
    )
    if not _rank_version_ok:
        full=_ensure_decision_ranking_v63917(full,str((last.get('config') or {}).get('scan_mode','Production 151')))
    ...
    if isinstance(full,pd.DataFrame) and not full.empty:
        _validated=int(full.get('DecisionRankEligible',pd.Series(False,index=full.index)).fillna(False).astype(bool).sum())
        ...
        st.caption(f"Decision lanes: VALIDATED { _validated } • EMERGING { _emerging } • ACTIONABLE NOW { _action }. Global Rank remains the raw all-stock score; Qualified Rank and Emerging Rank are separate.")
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                _qrank=_num6395(r.get('QualifiedRank',np.nan));_erank=_num6395(r.get('EmergingRank',np.nan))
                ...
                top_score=float(r.get('TopScore',0) or 0)
                opp_score=float(r.get('OpportunityScore',0) or 0)
                trade_stage=str(r.get('TradeStage','—'))
                movement=str(r.get('MovementStage','—'))
                ...
                _drs_head=f"{_drs:.1f}" if np.isfinite(_drs) else '—'

                # V6.3.9.12: refresh the displayed price for EVERY Top-5 ticker.
                # The Scanner row is a snapshot and can be stale after a long scan,
                # reconnect, or after the market closes.  A render-time quote is used
                # whenever the provider can supply a current/delayed display price.
                # Ranking/Entry/R:R remain the scan snapshot until Scanner is rerun.
                ...
                _px=np.nan
                _px_source='Current price unavailable'
                _px_ts=''
                _px_live_badge=''
                try:
                    _live_px=_top5_current_price_v63915(_logical_ticker)
                except Exception:
                    _live_px={}
                ...
                if (_live_px or {}).get('display_current') and np.isfinite(_live_val) and _live_val>0:
                    ...
                    _px_live_badge=f" • <b>PRICE {_e6395(_provider)}</b>"
                elif _bridge and _bridge.get('active_primary'):
                    # Never fall back to a cached/pre-split logical-counter price.
                    _px=np.nan
                    _px_source='Temporary-counter current quote unavailable'
                    _px_ts=''
                    _px_live_badge=' • <b>LIVE PRICE UNAVAILABLE</b>'
                ...
                st.markdown(
                    f"<div class='top5-head'><div class='top5-name'>{_rank_label} {ticker} <small>• Global #{_global_rank or rank}</small></div>"
                    f"<div class='top5-score'><small>RANK</small> {_drs_head}<span class='den'>/100</span></div>"
                    f"<div class='top5-price'><small>PRICE</small> {_px_txt}{_ccy_html}</div></div>",
                    unsafe_allow_html=True,
                )
                _bridge_html=''
                if _bridge and _bridge.get('active_primary'):
                    _bridge_html=f" • <b>TEMP DATA {_e6395(_bridge.get('temporary_symbol',''))}</b>"
                _price_meta=_px_live_badge
                if _px_ts:
                    _price_meta+=f" • {_e6395(_px_ts)}"
                else:
                    _fresh_status=str((_live_px or {}).get('freshness_status','') or '')
                    if _fresh_status:
                        _price_meta+=f" • <b>{_e6395(_fresh_status)}</b>"
                    if not np.isfinite(_px) and np.isfinite(_scan_px):
                        _price_meta+=f" • SCAN PRICE {_scan_px:.3f}"
                _setup=_num_v6394(r.get('SetupQualityScore'),np.nan);_evscore=_num_v6394(r.get('DecisionEvidenceScore',r.get('Reliability')),np.nan)
                _setup_txt=f'{_setup:.1f}' if np.isfinite(_setup) else '—';_ev_txt=f'{_evscore:.1f}' if np.isfinite(_evscore) else '—'
                ...
                _qual=str(r.get('DecisionRankQualification','QUALIFIED') or 'QUALIFIED')
                _qual_html=f" • <b>{_e6395(_lane)}</b> • QUAL {_e6395(_qual)}"
                st.markdown(f"<div class='top5-rankline'><b>SETUP {_setup_txt}</b> • <b>EVIDENCE {_ev_txt}</b> • Ranking V3.3 • de-duplicated{_qual_html}{_bridge_html}{_price_meta}</div>",unsafe_allow_html=True)
                st.markdown(status_html,unsafe_allow_html=True)
                st.markdown(
                    f"<div class='top5-status'>Trade Entry <span class='top5-pill {trade_cls}'>{_e6395(trade_stage)}</span> "
                    f"• Movement <b>{_e6395(movement)}</b> • Exit <span class='{exit_cls}'><b>{_e6395(exit_stage)} {exit_pressure:.0f}</b></span></div>",
                    unsafe_allow_html=True,
                )

                _mf=_num_v6394(r.get('MoneyFlowScore'));_mc=_num_v6394(r.get('MarketCycleScore'));_cat=_num_v6394(r.get('CatalystScore'));_entry=_num_v6394(r.get('EntryTimingScore',r.get('LiveActionabilityScore',r.get('EntryScore'))));_evidence=_num_v6394(r.get('DecisionEvidenceScore',r.get('Reliability')));_setupq=_num_v6394(r.get('SetupQualityScore'))
                ...
                _mf_txt=f"{_mf:.0f}" if np.isfinite(_mf) else '—';_mc_txt=f"{_mc:.0f}" if np.isfinite(_mc) else '—';_cat_txt=f"{_cat:.0f}" if np.isfinite(_cat) else '—';_entry_txt=f"{_entry:.0f}" if np.isfinite(_entry) else '—';_hour_txt=f"{_hour:.0f}" if np.isfinite(_hour) else '—'
                _cons_txt=f"{_cons:.0f}%" if np.isfinite(_cons) else '—'
                st.markdown(
                    "<div class='top5-grid'>"
                    f"<div class='top5-mini'><span class='lbl'>Setup</span><span class='val'>{_fmt6395(_setupq,0)}/100</span><span class='sub'>momentum / prediction</span></div>"
                    ...
                    f"<div class='top5-mini'><span class='lbl'>Hourly</span><span class='val'>{_hour_txt}</span><span class='sub'>diagnostic, not extra rank weight</span></div>"
                    f"<div class='top5-mini'><span class='lbl'>Money Flow</span><span class='val'>{_mf_txt}/100</span><span class='sub'>{_e6395(r.get('MoneyFlowLabel','NO DATA'))}</span></div>"
                    f"<div class='top5-mini'><span class='lbl'>Evidence</span><span class='val'>{_fmt6395(_evidence,0)}/100</span><span class='sub'>N {_ev_n} • Lift {_ev_lift_txt} • {_e6395(_ev_qual)}</span></div>"
                    f"<div class='top5-mini'><span class='lbl'>Move Consumed</span><span class='val {_cons_cls}'>{_cons_txt}</span><span class='sub {_cons_cls}'>{_e6395(_te_short)}</span></div>"
                    f"<div class='top5-mini'><span class='lbl'>Chase Risk</span><span class='val {_chase_cls}'>{_e6395(_chase)}</span><span class='sub'>{_cr:.0f}/100</span></div>"
                ...
                if timing_risk:
                    action=_e6395(r.get('RecommendedAction','WAIT FOR RETEST'))
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>Timing risk:</b> consumed {_cons_txt} ({_e6395(_te_short)}) • Chase {_e6395(_chase)} {_cr:.0f}/100 • {action}</div>",unsafe_allow_html=True)
                _evq=str(r.get('EvidenceQualification','UNPROVEN') or 'UNPROVEN')
                _ev_trade=bool(r.get('EvidenceTradeGrade',r.get('EvidenceGuardOK',False)))
                if not _ev_trade:
                    _ev_reason=_e6395(r.get('EvidenceQualificationReason','Insufficient trade-grade evidence'))
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>Evidence guard:</b> {_e6395(_evq)} • {_ev_reason} • not eligible for CONFIRMED/ACTIONABLE or VALIDATED PICKS.</div>",unsafe_allow_html=True)
                if not bool(r.get('SessionDataFresh',True)) or str(r.get('DataQuality','OK'))!='OK':
                    ...
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>Freshness guard:</b> {_fs} • entry plan/rank is blocked until the latest completed session is available.</div>",unsafe_allow_html=True)
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Money Flow: {r.get('MoneyFlowEvidence','—')} • Cycle: {r.get('MarketCycleReason','—')}")
                    ...
                    st.caption(f"Ranking V3.3 {safe(r.get('DecisionRankScore'),1)}/100 • {r.get('DecisionRankReason','—')}")
                    ...
                    st.caption(f"Move {safe(r.get('MoveScore'),1)} • Explosive {safe(r.get('ExplosiveScore'),1)} • Volume {r.get('VolumeContext','N/A')} • Live RVOL {safe(r.get('LiveIntradayRVOL',r.get('TimeAdjustedRVOL')))}x • Daily RVOL {safe(r.get('DailyRobustRVOL'))}x • Global #{int(r.get('GlobalRank',0) or 0)} • Qualified {('#'+str(int(r.get('QualifiedRank')))) if pd.notna(r.get('QualifiedRank',np.nan)) else '—'} • Emerging {('#'+str(int(r.get('EmergingRank')))) if pd.notna(r.get('EmergingRank',np.nan)) else '—'} • {r.get('Market','—')} #{int(r.get('MarketRank',0) or 0)} • Sector #{int(r.get('SectorRank',0) or 0)}")
        ...
        st.caption("Ranking V3.3 keeps the same de-duplicated formula: Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%. VALIDATED PICKS require all qualification guards; EMERGING SETUPS are technically strong low-sample research cases and can never become CONFIRMED/ACTIONABLE until Evidence becomes trade-grade. Global Rank remains the raw all-stock score.")
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Ranking V3.3.")
    ...
    primary_show=st.selectbox("Show",["ALL","VALIDATED PICKS","EMERGING SETUPS","ACTIONABLE NOW","WATCHLIST","RESEARCH"],key="show_primary_v63917")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.3 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.3 or Entry.")
    st.caption("Ranking V3.3 keeps the same one-time weights. The Scanner now separates VALIDATED PICKS (trade-grade Evidence + timing/data guards) from EMERGING SETUPS (strong technical setup but low-sample Evidence). Emerging setups remain research-only and cannot become CONFIRMED/ACTIONABLE until Evidence qualifies.")
...
with tab_feedback:
    ...
    st.caption("Every successfully completed live Scanner run creates snapshots immediately. Live outcomes mature only after future 1D / 2D / 3D / 5D trading sessions. V6.3.9 adds Early/Mid/Late ARMED forward validation plus Scanner-XLSX recovery, while keeping Historical Replay separate from live Feedback.")
