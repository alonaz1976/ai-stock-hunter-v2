...
APP_VERSION = "6.3.0"
APP_BUILD_ID = "V630-PERSISTENT-FEEDBACK-HISTORICAL-REPLAY-20260919-A"
...
def _feedback_conn_v600():
    ...
    if 'candidate' not in replay_existing:
        try:con.execute('ALTER TABLE replay_events ADD COLUMN candidate INTEGER')
        except Exception:pass
    con.commit()
    return con
...
def _feedback_replay_active_indicators_v630(r):
    """Causal daily indicators saved with Historical Replay rows.

    These use only information available at the replay signal close. They are
    intentionally simpler than live intraday confirmations and are never mixed
    into the live Feedback success percentage.
    """
    ...
    rv=fin('robust_volume_ratio');va=fin('vol_accel');cmf=fin('cmf20');obv=fin('obv_slope5');ad=fin('ad_slope5');rs=fin('rs20');cl=fin('close_location');adx=fin('adx14')
    states=[
        ...
        ('Squeeze release',bool(r.get('squeeze_release',False))),
        ('Fresh transitions >= 2',fin('fresh_transition_count',0)>=2),
    ]
    return [name for name,on in states if bool(on)]


def _feedback_store_replay_v630(run_id,rows,cfg,successful_tickers):
    con=_feedback_conn_v600()
    try:
        con.execute('DELETE FROM replay_events WHERE run_id=?',(str(run_id),))
        vals=[]
        for r in rows:
            vals.append((str(run_id),str(r.get('Ticker','')),str(r.get('Market','')),str(r.get('SignalDate','')),r.get('Price'),r.get('QuantScore'),r.get('EarlyScore'),r.get('EntryScore'),r.get('ATRPct'),r.get('Target1'),r.get('Invalidation'),r.get('EndReturn'),r.get('MFE'),r.get('MAE'),str(r.get('FirstEvent','')),str(r.get('CleanOutcome','')),str(r.get('Indicators','')),int(bool(r.get('Candidate',False)))))
        if vals:
            con.executemany('INSERT OR REPLACE INTO replay_events(run_id,ticker,market,signal_date,price,quant_score,early_score,entry_score,atr_pct,target1,invalidation,end_return,max_favorable,max_adverse,first_event,clean_outcome,indicators,candidate) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',vals)
        ...
        # Keep replay research useful without allowing repeated large runs to make
        # the persistent DB grow forever. Live snapshots/outcomes are NOT pruned.
        ...
        for old_id in old_runs:
            con.execute('DELETE FROM replay_events WHERE run_id=?',(str(old_id),));con.execute('DELETE FROM replay_runs WHERE run_id=?',(str(old_id),))
        con.commit()
    finally:con.close()
...
def _feedback_replay_scorecards_v630(events):
    ...
    if e.empty:return pd.DataFrame(),pd.DataFrame(),pd.DataFrame()
    cand=e[pd.to_numeric(e.get('candidate'),errors='coerce').fillna(0).eq(1)].copy()
    clean=cand[cand.get('clean_outcome','').astype(str).isin(['WIN','LOSS'])].copy() if not cand.empty else pd.DataFrame()
    if not clean.empty:clean['Win']=clean['clean_outcome'].astype(str).eq('WIN').astype(float)
    wins=int((clean.get('clean_outcome',pd.Series(dtype=str))=='WIN').sum()) if not clean.empty else 0;losses=int((clean.get('clean_outcome',pd.Series(dtype=str))=='LOSS').sum()) if not clean.empty else 0
    resolved=wins+losses;rate=100*wins/resolved if resolved else np.nan
    headline=pd.DataFrame([{'Historical Replay Success %':rate,'Candidate Rows':len(cand),'Clean Resolved':resolved,'Wins':wins,'Losses':losses,'Open/Ambiguous':max(0,len(cand)-resolved),'Definition':'Daily research candidate; fixed target vs ATR-based invalidation. Separate from Live Feedback.'}])
    indicators=[]
    base=float(clean['Win'].mean()) if not clean.empty else np.nan
    if not clean.empty and 'indicators' in clean:
        names=sorted({z.strip() for txt in clean['indicators'].fillna('').astype(str) for z in txt.split(' | ') if z.strip()})
        for name in names:
            mask=clean['indicators'].fillna('').astype(str).str.split(' | ').apply(lambda arr:name in arr)
            g=clean[mask];n=len(g)
            if n<5:continue
            ...
            indicators.append({'Indicator':name,'Resolved':n,'Success Rate %':100*hr,'Replay Baseline %':100*base if np.isfinite(base) else np.nan,'Lift x':lift,'Delta pp':delta,'Status':status})
    ind=pd.DataFrame(indicators)
    if not ind.empty:
        order={'WORKED':0,'MIXED':1,'FAILED':2,'LOW SAMPLE':3};ind['_o']=ind.Status.map(order).fillna(9);ind=ind.sort_values(['_o','Lift x','Resolved'],ascending=[True,False,False]).drop(columns=['_o'])
    stocks=[]
    if not cand.empty:
        for t,g in cand.groupby('ticker'):
            gc=g[g['clean_outcome'].isin(['WIN','LOSS'])];n=len(gc);w=int((gc['clean_outcome']=='WIN').sum());l=n-w;hr=100*w/n if n else np.nan
            status='LOW SAMPLE / WAITING' if n<5 else ('SUCCESS' if hr>=55 else ('FAILURE' if hr<=45 else 'MIXED'))
            stocks.append({'Ticker':str(t),'Candidate Rows':len(g),'Resolved':n,'Wins':w,'Losses':l,'Success Rate %':hr,'Avg MFE %':100*pd.to_numeric(g.get('max_favorable'),errors='coerce').mean(),'Avg MAE %':100*pd.to_numeric(g.get('max_adverse'),errors='coerce').mean(),'Model Status':status})
    return headline,ind,pd.DataFrame(stocks)
...
def _feedback_workbook_v629(sheets,meta=None):
    ...
    try:
        ...
        colors={'WORKED':'C6EFCE','SUCCESS':'C6EFCE','FAILED':'FFC7CE','FAILURE':'FFC7CE','MIXED':'FFEB9C'}
        for sheet,status_header in [('Indicator Summary','סטטוס'),('Stock Summary','Model Status')]:
...
def _lab_runtimes_v603(build_id):
    ...
    names=('analyze','backtest','validate','entry','explosive','feedback','research_validator','pre_move')
...
def _historical_replay_worker_v630(runtime,jid,cfg):
    """Causal daily replay for research statistics only.

    It deliberately does NOT pretend to reconstruct unavailable historical 15m/1h
    live confirmations. Replay rows are stored in separate tables and never enter
    the headline Live Feedback success percentage.
    """
    try:
        ...
        for n,t in enumerate(tickers,1):
            _lab_check_v603(runtime,jid);_lab_update_v603(runtime,jid,n-1,total,t,'Historical daily replay')
            try:
                ...
                # Conservative anti-lookahead guard: exclude the newest provider row,
                # because it may still be provisional on an exchange that is open now.
                if len(f)>1:f=f.iloc[:-1].copy()
                if len(f)<90+h:raise ValueError('Insufficient completed daily history after feature warm-up')
                successful+=1;start=70;last=len(f)-h
                for i in range(start,last,step):
                    r=f.iloc[i];p0=float(r.get('Close',np.nan))
                    if not np.isfinite(p0) or p0<=0:continue
                    q=float(score_row(r,0)[0]) if callable(score_row) else np.nan
                    es=float(early_score_row(r)[0]) if callable(early_score_row) else np.nan
                    ent=float(_entry_score_from_row(r,q,es))
                    atrp=float(r.get('atr_pct',np.nan)) if pd.notna(r.get('atr_pct',np.nan)) else np.nan
                    stop_pct=float(np.clip(1.20*(atrp/100.0),.02,.06)) if np.isfinite(atrp) else .03
                    ...
                    active=_feedback_replay_active_indicators_v630(r)
                    candidate=bool(ent>=55 and (es>=50 or q>=60))
                    try:sigdate=str(pd.Timestamp(f.index[i]).date())
                    except Exception:sigdate=str(f.index[i])
                    rows.append({'Ticker':str(t),'Market':_feedback_market_key_v612('',str(t)),'SignalDate':sigdate,'Price':p0,'QuantScore':q,'EarlyScore':es,'EntryScore':ent,'ATRPct':atrp,'Target1':t1,'Invalidation':inv,'EndReturn':endret,'MFE':mfe,'MAE':mae,'FirstEvent':first,'CleanOutcome':outcome,'Indicators':' | '.join(active),'Candidate':candidate})
            except Exception as e:skipped.append({'Ticker':str(t),'Reason':f'{type(e).__name__}: {e}'})
            _lab_update_v603(runtime,jid,n,total,t,'Historical daily replay')
        _feedback_store_replay_v630(jid,rows,cfg,successful)
        df=pd.DataFrame(rows);head,inds,stocks=_feedback_replay_scorecards_v630(df.rename(columns={'CleanOutcome':'clean_outcome','Indicators':'indicators','Candidate':'candidate','Ticker':'ticker','MFE':'max_favorable','MAE':'max_adverse'})) if not df.empty else (pd.DataFrame(),pd.DataFrame(),pd.DataFrame())
        excel=workbook_bytes({'Replay Success':head,'Replay Indicators':inds,'Replay Stocks':stocks,'Replay Events':df,'Skipped':pd.DataFrame(skipped)},{'Tab':'Historical Replay','Version':APP_VERSION,'Scope':cfg.get('scope'),'History':history,'TargetPct':100*target,'HorizonDays':h,'SampleEverySessions':step,'RequestedStocks':total,'SuccessfulStocks':successful,'ResearchOnly':'YES — daily replay is never mixed with Live Feedback'})
        _lab_publish_v603(runtime,jid,{'events':df,'headline':head,'indicators':inds,'stocks':stocks,'skipped':pd.DataFrame(skipped),'excel':excel,'successful':successful})
    except _LabCancelled:_lab_publish_v603(runtime,jid,None,'stopped')
    except Exception as e:_lab_fail_v603(runtime,jid,e)
...
with tab_feedback:
    ...
    st.markdown("#### 🕰️ Historical Replay — immediate research statistics")
    st.caption("Replay reconstructs DAILY indicators using only information available at each historical date, then checks the following sessions. It is research-only and is never mixed with the headline Live Feedback success rate because old 15m/1h live confirmations cannot be reconstructed reliably at scale.")
    ...
    with rr1:
        if st.button("▶ Run Historical Replay",key="historical_replay_start_v630",use_container_width=True,disabled=replay_busy):
            ...
            ok,msg=_start_lab_v603('historical_replay',_historical_replay_worker_v630,rcfg,total=len(replay_tickers))
    ...
    replay_head,replay_ind,replay_stocks=_feedback_replay_scorecards_v630(replay_events)
    ...
    if not replay_head.empty:
        ...
        rm1.metric("Replay success %",f"{float(rh['Historical Replay Success %']):.1f}%" if pd.notna(rh['Historical Replay Success %']) else "—")
        rm2.metric("Replay candidates",int(rh['Candidate Rows']));rm3.metric("Clean resolved",int(rh['Clean Resolved']));rm4.metric("Open / ambiguous",int(rh['Open/Ambiguous']))
        st.caption("Replay success is a DAILY research benchmark using the replay candidate rule and ATR-based invalidation. It is not the Live Scanner success percentage.")
    ...
    if not replay_events.empty:
        replay_excel=workbook_bytes({'Replay Success':replay_head,'Replay Indicators':replay_ind,'Replay Stocks':replay_stocks,'Replay Events':replay_events},{'Tab':'Historical Replay','Version':APP_VERSION,'ResearchOnly':'YES — separate from Live Feedback'})
