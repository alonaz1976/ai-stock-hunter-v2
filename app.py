...
APP_VERSION = "6.0.4"
APP_BUILD_ID = "V604-LIVE-PRICE-SPLIT-HOTFIX-20260917-A"
...
fetch_live_intraday_snapshot = getattr(qe, "fetch_live_intraday_snapshot", lambda ticker,daily=None: {"price":np.nan,"prev_close":np.nan,"change_pct":np.nan,"timestamp_label":"—","source":"unavailable","fresh":False,"same_session_date":False,"age_minutes":np.nan,"split_ratio":np.nan,"split_date":None,"split_detected":False})
...
def _v544_scan_universe_dynamic(tickers,daily_period='6mo',use_hourly=True,hourly_period='1mo',horizon=5,target_pct=.03,min_turnover=0,buy_threshold=66,prefilter_top=30,progress_callback=None):
    ...
    for deep_i,(pre,ticker,f,ds) in enumerate(stage,1):
        ...
        try:
            ...
            try:
                if m15 is not None and len(m15): intraday_price=float(m15['Close'].iloc[-1])
                elif h is not None and len(h): intraday_price=float(h['Close'].iloc[-1])
            except Exception: intraday_price=np.nan
...
def add_market_and_opportunity(df):
    ...
    live_ok=z['PreviousSessionTrigger'] & z['MarketPhase'].eq('OPEN') & (pd.to_numeric(z['ExitPressure'],errors='coerce').fillna(0)<55) & _live_price_ok & _live_data_ok & _live_plan_ok
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        _lab_update_v603(runtime,jid,1,8,t,'Fresh 5m price + corporate action check')
        ...
        d,h,m15,ca_report=normalize_cross_timeframes(
            d,h,m15,
            anchor_price=live_snapshot.get('price',np.nan),
        ...
        # V6.0.4: display/session price comes from the freshest 5m provider bar,
        # not from the last Daily row.  Closed-bar Daily features remain unchanged.
        lp=float(live_snapshot.get('price',np.nan)) if np.isfinite(float(live_snapshot.get('price',np.nan))) else np.nan
        if np.isfinite(lp): latest_live['price']=lp
        phase_now=_market_phase(MARKET_MAP.get(t,'CUSTOM'))
        fresh_change=bool(live_snapshot.get('fresh')) or phase_now!='OPEN'
...
with tab1:
    ...
    if snap and snap.get('payload'):
        ...
        price_fresh=bool(live_snapshot.get('fresh'));price_label='Intraday price' if np.isfinite(float(live_snapshot.get('price',np.nan))) else 'Last price'
        st.markdown("<div class='section'>Decision cockpit</div>",unsafe_allow_html=True);p1,p2,p3,p4=st.columns([1.35,1,1,1]);p1.metric(price_label,f"{cur:,.3f} {currency}" if np.isfinite(cur) else '—',delta=f"{ch:+.2f}%" if np.isfinite(ch) else None);p2.metric("TOP SCORE",f"{decision.get('TopScore',0):.1f}");p3.metric("Opportunity",f"{decision.get('OpportunityScore',0):.1f}");p4.metric("Exit Pressure",f"{decision.get('ExitPressure',0):.0f}",delta=decision.get('ExitStage','CLEAR'))
        quote_stamp=live_snapshot.get('timestamp_label','—');quote_age=live_snapshot.get('age_minutes',np.nan);age_txt=f" • age {float(quote_age):.0f}m" if np.isfinite(float(quote_age)) else ''
        st.caption(f"{phase} • {decision.get('Market','—')} • price: 5m intraday provider feed @ {quote_stamp}{age_txt} • provider data may be delayed")
        if phase=='OPEN' and not price_fresh:st.error("STALE DATA — current-session price is not fresh enough. Live day-change is suppressed and LIVE TRIGGER / Actionable Now must not be trusted until a fresh intraday bar arrives.")
