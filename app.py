...
APP_VERSION = "6.3.9.33"
APP_BUILD_ID = "V63931-DECISION-HK-FRESHNESS-20260925-A"
...
# -----------------------------------------------------------------------------
# V6.3.9.33 Decision Ranking Engine V3.6 — current-first priority semantics
#
...
def _apply_decision_ranking_v63919(df,scan_mode='Production 151'):
    """Ranking V3.6: same de-duplicated score, with current-first trade priority semantics.
    ...
    def row_score(r):
        ...
        # only; the Emerging Score formula and Ranking V3.6 weights are unchanged.
    ...
    z['TradePriorityVersion']='V3.6-CURRENT-FIRST'
...
def _ensure_decision_ranking_v63919(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.6 and decision lanes into cached snapshots.
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.6. Optimized mode contributes only
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # redeploy and therefore have no Ranking V3.6 decision-lane columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3.6').all()
    )
        ...
        for _,r in topcards.iterrows():
            with st.container(border=True):
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Decision Score {safe(r.get('DecisionRankScore'),1)}/100 • model V3.6 • {r.get('DecisionRankReason','—')}")
                    ...
                    st.caption(f"Move {safe(r.get('MoveScore'),1)} • Explosive {safe(r.get('ExplosiveScore'),1)} • Volume {r.get('VolumeContext','N/A')} • Live RVOL {safe(r.get('LiveIntradayRVOL',r.get('TimeAdjustedRVOL')))}x • Daily RVOL {safe(r.get('DailyRobustRVOL'))}x • Global #{int(r.get('GlobalRank',0) or 0)} • Qualified {('#'+str(int(r.get('QualifiedRank')))) if pd.notna(r.get('QualifiedRank',np.nan)) else '—'} • Emerging {('#'+str(int(r.get('EmergingRank')))) if pd.notna(r.get('EmergingRank',np.nan)) else '—'} • Historical EV Rank {('#'+str(int(r.get('EvidenceRank')))) if pd.notna(r.get('EvidenceRank',np.nan)) else '—'} • {r.get('Market','—')} #{int(r.get('MarketRank',0) or 0)} • Sector #{int(r.get('SectorRank',0) or 0)}")
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Decision Score V3.6.")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.6 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.6 or Entry.")
