...
# -----------------------------------------------------------------------------
# V6.3.9.18 Decision Ranking Engine V3.4 — validated opportunity semantics
#
...
def _apply_decision_ranking_v63918(df,scan_mode='Production 151'):
    """Ranking V3.4: de-duplicated score plus separate validated/emerging lanes.
    ...
    def row_score(r):
        ...
        # V6.3.9.18 separates two different questions:
        ...
        evidence_validated=bool(ev_eligible)
        ...
        validated_opportunity=bool(guard_eligible and current_opportunity_ok)
        ...
        # V6.3.9.18: the research-only Emerging lane keeps technically strong low-sample
...
def _ensure_decision_ranking_v63918(df, scan_mode='Production 151'):
    """Backfill current Ranking V3.4 and decision lanes into cached snapshots.
    ...
    the Scanner is rerun under V6.3.9.18.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    return _apply_decision_ranking_v63918(z,scan_mode)
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # de-duplicated Decision Ranking V3.4. Optimized mode contributes only
            # a bounded OOS modifier; overlapping decision families are not reweighted twice.
            res=_apply_decision_ranking_v63918(res,config.get('scan_mode','Production 151'))
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.18 migration: an older cached last_completed scan may survive a code
    # redeploy and therefore have no Ranking V3.4 decision-lane columns. Backfill and re-sort the
    ...
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V3.4').all()
    )
    if not _rank_version_ok:
        full=_ensure_decision_ranking_v63918(full,str((last.get('config') or {}).get('scan_mode','Production 151')))
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(f"<div class='top5-rankline'><b>SETUP {_setup_txt}</b> • <b>EVIDENCE {_ev_txt}</b> • Ranking V3.4 • de-duplicated{_qual_html}{_bridge_html}{_price_meta}</div>",unsafe_allow_html=True)
                ...
                if not _ev_trade:
                    ...
                    st.markdown(f"<div class='top5-alert'>⚠️ <b>Evidence guard:</b> {_e6395(_evq)} • {_ev_reason} • not eligible for CONFIRMED/ACTIONABLE or a VALIDATED OPPORTUNITY.</div>",unsafe_allow_html=True)
                if _ev_trade and not bool(r.get('ValidatedOpportunityEligible',r.get('DecisionRankEligible',False))):
                    ...
                    st.markdown(f"<div class='top5-alert'>ℹ️ <b>Current opportunity guard:</b> Evidence is validated, but {_opp_reason}. This remains Evidence Validated / WAIT, not a Q-ranked opportunity.</div>",unsafe_allow_html=True)
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Ranking V3.4 {safe(r.get('DecisionRankScore'),1)}/100 • {r.get('DecisionRankReason','—')}")
        ...
        table=table.rename(columns={
            'Rank':'View Rank',
            'DecisionRankScore':'Ranking V3.4 Score','DecisionRankCore':'Ranking V3.4 Core','GlobalRank':'Global Rank',
        ...
        st.caption("Ranking V3.4 keeps the same de-duplicated formula: Setup 35% + Entry Timing 25% + Money Flow 15% + Evidence 15% + R:R 10%. VALIDATED OPPORTUNITIES require trade-grade Evidence plus Setup ≥55, Entry ≥55, Ranking ≥55 and a WATCH/ARMED/CONFIRMED current stage; EVIDENCE VALIDATED is historical proof only; EMERGING SETUPS are technically strong low-sample research cases and can never become CONFIRMED/ACTIONABLE until Evidence becomes trade-grade. Global Rank remains the raw all-stock score.")
...
with tab_scan:
    ...
    if model_mode=="OOS OPTIMIZED":
        ...
        if model.get('scopes'):
            st.caption(f"Model Registry: {len(registry.get('models',[]))} saved optimization run(s). Only SHADOW ELIGIBLE OOS models for +{float(ta):g}% / {hlabel} may modify Ranking V3.4.")
    ...
    with st.expander("Research overlays (optional)",expanded=False):
        valuation_overlay=st.checkbox("Valuation / underpricing overlay",value=True,key="valuation_overlay_v63915",help="Research only. Does not change Ranking V3.4 or Entry gates.")
        catalyst_overlay=st.checkbox(f"Catalyst overlay for Top {_CATALYST_TOP_N_V6394}",value=False,key="catalyst_overlay_v63915",help="Research only. News/earnings context is shown in Details but does not change Ranking V3.4 or Entry.")
    st.caption("Ranking V3.4 keeps the same one-time weights. The Scanner now separates VALIDATED OPPORTUNITIES (trade-grade Evidence + strong current Setup/Entry + clean timing/data guards), EMERGING SETUPS (strong now but low-sample Evidence), and EVIDENCE VALIDATED names that are currently WAIT/BLOCKED. Emerging setups remain research-only and cannot become CONFIRMED/ACTIONABLE until Evidence qualifies.")
