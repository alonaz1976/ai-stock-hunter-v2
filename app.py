...
APP_VERSION = "6.1.1"
APP_BUILD_ID = "V611-ADAPTIVE-ENTRY-LEARNING-20260917-A"
...
resolve_current_market_snapshot = getattr(qe, "resolve_current_market_snapshot", lambda ticker,daily=None,hourly=None,m15=None,primary=None,market_open=False: dict(primary or {}))
...
def _v544_scan_universe_dynamic(tickers,daily_period='6mo',use_hourly=True,hourly_period='1mo',horizon=5,target_pct=.03,min_turnover=0,buy_threshold=66,prefilter_top=30,progress_callback=None):
    ...
    for deep_i,(pre,ticker,f,ds) in enumerate(stage,1):
        ...
        try:
            ...
            phase_scan=_market_phase(MARKET_MAP.get(ticker,'CUSTOM'))
            ...
            ent=entry_timing(f,dq,de,hourly_feat=hfeat,m15_feat=m15feat,current_price=display_price,market_regime=market_regime)
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • DIRECTIONAL VOLUME • EXIT PRESSURE • FEEDBACK LOOP • 151 STOCKS</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Decision Intelligence • Direction-aware Volume • Trade + Exit Signals • Outcome Learning</div></div>""",unsafe_allow_html=True)
...
MARKET_MAP={**{t:'NASDAQ' for t in NASDAQ_50.split(',')},'ITT':'NYSE',**{t:'HONG KONG' for t in HK_50.split(',')},**{t:'TEL AVIV' for t in TASE_50.split(',')}}
SECTOR_MAP={}
...
def _market_phase(market):
    ...
    cfg={
        ...
        'NYSE':('America/New_York',time(4,0),time(9,30),time(16,0)),
    ...
    if pre<=t<op:return 'PRE-MARKET' if market in ('NASDAQ','NYSE') else 'PRE-OPEN'
    if op<=t<cl:return 'OPEN'
    if market in ('NASDAQ','NYSE') and cl<=t<time(20,0): return 'AFTER-MARKET'
    return 'CLOSED'
...
def _market_regime_v610(ticker):
    ...
    t=str(ticker).upper()
    bench='SPY' if t in set(US_51.split(',')) else ('^HSI' if t.endswith('.HK') else ('^TA125.TA' if t.endswith('.TA') else None))
...
def add_market_and_opportunity(df):
    ...
    nas_pm=z[(z['Market'].isin(['NASDAQ','NYSE'])) & (z['MarketPhase'].eq('PRE-MARKET'))]['Ticker'].tolist()
    ...
    nas_ah=z[(z['Market'].isin(['NASDAQ','NYSE'])) & (z['MarketPhase'].eq('AFTER-MARKET'))]['Ticker'].tolist()
    ...
    priority={'CONFIRMED ENTRY':6,'ARMED':5,'WATCH':4,'WAIT':3,'TOO LATE / CHASE':2,'INVALIDATED':1}
    ...
    z['ActionableNow']=(((z['MarketPhase'].eq('OPEN')) & z['LiveStage'].eq('CONFIRMED ENTRY — LIVE')) | ((z['MarketPhase'].eq('PRE-MARKET')) & z['PMConfirmation'].eq('CONFIRMED') & z['TradeStage'].eq('CONFIRMED ENTRY')) | ((z['MarketPhase'].eq('AFTER-MARKET')) & z['AHConfirmation'].eq('CONFIRMED') & z['TradeStage'].eq('CONFIRMED ENTRY'))) & (pd.to_numeric(z['ExitPressure'],errors='coerce').fillna(0)<55) & _plan_ok & _data_ok & _price_fresh
...
def _entry_research_gate_v610(r,quant_score,entry_score):
    ...
    dv=directional_volume_row(r);flow=bool(dv.get('bullish',0)>=.22 and dv.get('bullish',0)>dv.get('bearish',0))
    nochase=bool((not np.isfinite(e20) or (p-e20)/a<=1.65) and (not np.isfinite(rs) or rs<=76) and mom3<=.16)
    gate=bool(setup and fresh and flow and nochase and xp<45)
    return {'TriggerEligible':gate,'DailySetup':setup,'FreshSignal':fresh,'VolumeFlow':flow,'NoChase':nochase,'ExitPressureAtSignal':xp}
...
def _entry_optimizer_v611(rows, min_signals=80, seed=611):
    """Tune a DAILY shadow-entry mix on folds 1-3 and report fold-4 out-of-sample.

    This optimizer never auto-promotes production weights. It is intentionally a
    shadow candidate so fresh/live feedback can confirm or reject it later.
    """
    z=pd.DataFrame(rows)
    if z.empty:return pd.DataFrame(),pd.DataFrame()
    z=z[z.get('Mode','').astype(str).eq('Dynamic')].copy() if 'Mode' in z else z.copy()
    if z.empty:return pd.DataFrame(),pd.DataFrame()
    features=['DailySetup','FreshSignal','VolumeFlow','NoChase','EntryNorm','QuantNorm','ExitSafety']
    ...
    candidates=[]
    # Transparent anchors + deterministic random simplex candidates.
    anchors=[np.array([.18,.22,.22,.06,.14,.10,.08]),np.array([.15,.25,.25,.03,.14,.10,.08]),np.array([.20,.20,.25,.00,.15,.12,.08])]
    candidates.extend(anchors)
    candidates.extend(list(rng.dirichlet(np.array([2.0,2.6,2.6,.8,1.8,1.5,1.2]),size=420)))
    thresholds=np.arange(.52,.83,.03)
    scopes=[('ALL',z)]
    if 'Market' in z:
        us=z[z.Market.astype(str).isin(['NASDAQ','NYSE'])].copy()
        if len(us)>=250:scopes.append(('US',us))
        for m,g in z.groupby('Market'):
            if len(g)>=250:scopes.append((str(m),g.copy()))
    best_rows=[];weight_rows=[]
    for scope,g in scopes:
        train=g[g.Fold.astype(int)<=3].copy(); test=g[g.Fold.astype(int)==4].copy()
        if len(train)<200 or len(test)<40:continue
        baseline_train=float(train.Hit.mean()); baseline_test=float(test.Hit.mean())
        X=train[features].to_numpy(float); Xt=test[features].to_numpy(float)
        best=None
        for w in candidates:
            sc=X@w
            for th in thresholds:
                mask=sc>=th; ns=int(mask.sum())
                if ns<int(min_signals):continue
                hr=float(train.loc[mask,'Hit'].mean()); lift=hr/baseline_train if baseline_train>0 else np.nan
                fold_l=[]
                for _,fg in train.assign(_score=sc).groupby('Fold'):
                    fm=fg._score>=th
                    if int(fm.sum())<12:continue
                    fb=float(fg.Hit.mean()); fh=float(fg.loc[fm,'Hit'].mean()); fold_l.append(fh/fb if fb>0 else np.nan)
                fold_l=[x for x in fold_l if np.isfinite(x)]
                if len(fold_l)<2:continue
                pos=np.mean(np.array(fold_l)>1.0); med=float(np.median(fold_l)); mfe=float(train.loc[mask,'MFE'].mean()); mae=float(train.loc[mask,'MAE'].mean())
                sample=min(1.0,np.sqrt(ns/max(float(min_signals)*3,1)))
                objective=(med-1.0)*1.8 + (pos-.5)*.45 + (mfe-max(0,-mae))*.15 + sample*.08
                rec=(objective,med,pos,ns,th,w,hr)
                if best is None or rec[0]>best[0]:best=rec
        if best is None:continue
        objective,med,pos,ns,th,w,train_hr=best;ts=Xt@w;tmask=ts>=th;tn=int(tmask.sum())
        test_hr=float(test.loc[tmask,'Hit'].mean()) if tn else np.nan; test_lift=test_hr/baseline_test if tn and baseline_test>0 else np.nan
        best_rows.append({'Scope':scope,'Train signals':ns,'Train hit rate %':100*train_hr,'Train baseline %':100*baseline_train,'Train median fold lift':med,'Train positive folds %':100*pos,'Shadow threshold':float(th),'OOS Fold4 signals':tn,'OOS Fold4 hit rate %':100*test_hr if np.isfinite(test_hr) else np.nan,'OOS Fold4 baseline %':100*baseline_test,'OOS Fold4 lift':test_lift,'Verdict':'PROMOTE TO FEEDBACK SHADOW' if tn>=20 and np.isfinite(test_lift) and test_lift>1.03 else 'KEEP RESEARCH-ONLY'})
        for name,val in zip(features,w):weight_rows.append({'Scope':scope,'Feature':name,'Learned weight %':100*float(val),'Shadow threshold':float(th)})
    return pd.DataFrame(best_rows),pd.DataFrame(weight_rows)
...
def _feedback_conn_v600():
    ...
    for col,typ in [('entry_state','TEXT'),('confirmation_pct','REAL'),('daily_setup','INTEGER'),('fresh_signal','INTEGER'),('hourly_entry','INTEGER'),('volume_flow','INTEGER'),('no_chase','INTEGER'),('market_regime','TEXT'),('market_regime_ok','INTEGER')]:
...
def _feedback_store_scan_v600(scan_id,df,config):
    ...
    try:
        ...
        for _,r in df.iterrows():
            ...
            vals=(scan_id,ts,str(r.get('Ticker','')),str(r.get('Market','')),num(r.get('Price')),num(r.get('TopScore')),num(r.get('OpportunityScore')),str(r.get('TradeStage','')),str(r.get('MovementStage','')),num(r.get('Prediction')),num(r.get('MoveScore')),num(r.get('ExplosiveScore')),num(r.get('EntryScore')),num(r.get('HourlyConfirm')),num(r.get('ExitPressure')),int(r.get('GlobalRank')) if pd.notna(r.get('GlobalRank')) else None,int(r.get('MarketRank')) if pd.notna(r.get('MarketRank')) else None,str(r.get('SessionStatus','')),num(r.get('EntryLow')),num(r.get('EntryHigh')),num(r.get('BreakoutTrigger')),num(r.get('Invalidation')),num(r.get('Target1')),num(r.get('Target2')),int(config.get('horizon',5)),str(r.get('EntryTriggerState',r.get('EntryStatus',''))),num(r.get('EntryConfirmationPct')),bit(r.get('DailySetupCheck')),bit(r.get('FreshSignalCheck')),bit(r.get('HourlyEntryCheck')),bit(r.get('VolumeFlowCheck')),bit(r.get('NoChaseCheck')),str(r.get('MarketRegime','NEUTRAL')),bit(r.get('MarketRegimeCheck')))
            con.execute("""INSERT OR REPLACE INTO snapshots(scan_id,ts_utc,ticker,market,price,top_score,opportunity,trade_stage,movement_stage,prediction,move_score,explosive,entry_score,hourly,exit_pressure,global_rank,market_rank,session_status,entry_low,entry_high,trigger,invalidation,target1,target2,horizon_days,entry_state,confirmation_pct,daily_setup,fresh_signal,hourly_entry,volume_flow,no_chase,market_regime,market_regime_ok) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",vals)
    ...
    try:
        ...
        try:
            ...
            for _,ar in amb.iterrows():
                try:
                    scan_date=pd.Timestamp(str(ar.ts_utc)).date(); d0=fetch_ohlcv(str(ar.ticker),'1mo','1d')
                    if d0 is None or d0.empty:continue
                    z0=d0.copy(); future0=z0[np.array([x>scan_date for x in pd.to_datetime(z0.index).date])].head(int(ar.horizon))
...
def _feedback_learning_v611(merged, half_life_days=60):
    ...
    feature_rows=[]
    feature_map={'daily_setup':'Daily Setup','fresh_signal':'Fresh Signal','hourly_entry':'Hourly / 15m','volume_flow':'Volume / Flow','no_chase':'No-Chase','market_regime_ok':'Market Regime'}
...
def _filter_scanner_results_v599(df,show_mode,market_filter="ALL 151"):
    ...
    if show_mode=='CONFIRMED ENTRY':
        ...
        z=z[z.get('EntryTriggerState',z.OpportunityStage).isin(['WATCH','ARMED'])]
...
def _scanner_stage_guide_v599():
    with st.expander("ℹ️ Setup progression — stage guide", expanded=False):
        st.markdown(
            "**WAIT** → **WATCH** → **ARMED** → **CONFIRMED ENTRY** → **TOO LATE / CHASE** or **INVALIDATED**"
        )
        st.caption(
        ...
        st.caption(
            "TOO LATE / CHASE = the setup may still be bullish but price is too extended for a disciplined entry • "
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    if res is None or res.empty:
        ...
        st.caption("Ranking: Trade Priority = stage first, then TOP Score • Global Rank = quality rank across all successfully analyzed stocks from the 151 requested • Market Rank = exchange rank • Sector Rank = sector rank inside that market.")
...
def _lab_runtime_v603(name):
    return _lab_runtimes_v603(APP_BUILD_ID)[name]
...
def _analyze_worker_v603(runtime,jid,cfg):
    try:
        ...
        h=confirmed_intraday_bars(fetch_ohlcv(t,'1mo','1h'));m15=confirmed_intraday_bars(fetch_ohlcv(t,'1mo','15m'))
        ...
        phase_now=_market_phase(MARKET_MAP.get(t,'CUSTOM'))
        live_snapshot=resolve_current_market_snapshot(t,d,h,m15,live_snapshot,market_open=(phase_now=='OPEN'))
        ...
        current_entry_price=float(live_snapshot.get('price',np.nan)) if bool(live_snapshot.get('display_current')) and np.isfinite(float(live_snapshot.get('price',np.nan))) else np.nan
        ent=entry_timing(f,qscore,escore,hourly_feat=hfeat,m15_feat=m15feat,current_price=current_entry_price,market_regime=_market_regime_v610(t))
        if str(ca_report.get('data_quality','OK'))!='OK':
        ...
        if np.isfinite(lp): latest_live['price']=lp
        ...
        if fresh_change and np.isfinite(float(live_snapshot.get('change_pct',np.nan))):
        ...
        row=pd.DataFrame([{'Ticker':t,'Prediction':dyn.get('final_prediction',0),'EntryScore':ent.get('entry_score',0),'EntryStatus':ent.get('status','WAIT'),'EntryTriggerState':ent.get('trigger_state',ent.get('status','WAIT')),'EntryConfirmationPct':ent.get('confirmation_pct',0),'DailySetupCheck':ent.get('daily_setup',False),'FreshSignalCheck':ent.get('fresh_signal',False),'HourlyEntryCheck':ent.get('hourly_entry_ok',False),'VolumeFlowCheck':ent.get('volume_flow_ok',False),'NoChaseCheck':ent.get('no_chase',False),'MarketRegime':ent.get('market_regime','NEUTRAL'),'EntryWhyNow':ent.get('why_now',''),'EntryMissingChecks':ent.get('missing_checks',''),'MoveScore':timing.get('move_score',0),'ExplosiveScore':ex.get('score',0),'Accel1D':timing.get('accel_1d',0),'Accel2D':timing.get('accel_2d',0),'Accel3D':timing.get('accel_3d',0),'HourlyConfirm':timing.get('hourly_confirmation',ex.get('hourly_confirmation',0)),'CalibrationConfidence':bt.get('confidence',0),'BacktestN':bt.get('n',0),'SignalLift':1.0,'ExitPressure':timing.get('exit_pressure',latest_live.get('exit_pressure',0)),'ExitStage':timing.get('exit_stage',latest_live.get('exit_stage','CLEAR')),'TimingStage':timing.get('timing_stage','—'),'PlanValid':ent.get('plan_valid',False),'EntryLow':ent.get('zone_low'),'EntryHigh':ent.get('zone_high'),'BreakoutTrigger':ent.get('trigger'),'Invalidation':ent.get('invalidation'),'Target1':ent.get('target1'),'Target2':ent.get('target2'),'Price':latest_live.get('price'),'VolumeContext':latest_live.get('volume_context','N/A'),'LiveIntradayRVOL':(hfeat.iloc[-1].get('time_adjusted_rvol',np.nan) if hfeat is not None and len(hfeat) else np.nan),'DailyRobustRVOL':daily_lr.get('robust_volume_ratio',np.nan),'TimeAdjustedRVOL':(hfeat.iloc[-1].get('time_adjusted_rvol',np.nan) if hfeat is not None and len(hfeat) else daily_lr.get('robust_volume_ratio',np.nan)),'RSI14':latest_live.get('rsi14',np.nan),'SplitAdjusted':ca_report.get('split_adjusted',False),'DataQuality':ca_report.get('data_quality','OK'),'LivePriceFresh':bool(live_snapshot.get('trade_fresh')) if phase_now=='OPEN' else True}]);decision=add_market_and_opportunity(row).iloc[0].to_dict();_lab_check_v603(runtime,jid)
        ...
        payload={'ticker':t,'history':hist,'horizon':horizon,'target':target,'f':f,'latest':latest,'dyn':dyn,'hfeat':hfeat,'ent':ent,'latest_live':latest_live,'live_snapshot':live_snapshot,'ex':ex,'timing':timing,'decision':decision,'ca_report':ca_report,'bt':bt,'buy_threshold':buy_threshold,'auto_table':auto_table,'comp':comp,'ec':ec,'cmp':cmp,'compare_df':compare_df,'ev':ev,'early_cal':early_cal,'quant_cal':quant_cal,'cal_events':cal_events,'excel':excel}
...
def _entry_payload_v603(rows,audit,skipped,cfg,partial=False,premove_rows=None,hourly_rows=None):
    summary,gate,market_gate=_aggregate_entry_validation(rows);opt,opt_weights=_entry_optimizer_v611(rows);pm=pd.DataFrame(premove_rows or []);hl=pd.DataFrame(hourly_rows or []);pm_summary=_aggregate_lift_rows_v611(pm,['Indicator','Days Before','Target','Horizon'],'Baseline %') if not pm.empty else pd.DataFrame();pm_market=_aggregate_lift_rows_v611(pm,['Market','Indicator','Days Before','Target','Horizon'],'Baseline %') if not pm.empty else pd.DataFrame();hl_summary=_aggregate_lift_rows_v611(hl,['Signal','Target','Horizon'],'Matched Baseline %') if not hl.empty else pd.DataFrame();hl_market=_aggregate_lift_rows_v611(hl,['Market','Signal','Target','Horizon'],'Matched Baseline %') if not hl.empty else pd.DataFrame();sheets={'Entry Summary':summary,'Entry Trigger V3 Gate':gate,'Entry Gate by Market':market_gate,'Entry Optimizer Shadow':opt,'Optimizer Weights':opt_weights,'Pre-Move Lift Summary':pm_summary,'Pre-Move by Market':pm_market,'Pre-Move Indicator Lift':pm,'Hourly Lift Summary':hl_summary,'Hourly Lift by Market':hl_market,'Hourly Matched Lift':hl,'Raw Observations':pd.DataFrame(rows),'Stock Audit':pd.DataFrame(audit)}
    if skipped:sheets['Skipped']=pd.DataFrame(skipped,columns=['Ticker','Reason'])
    excel=workbook_bytes(sheets,{'Tab':'Entry Validation','Market':cfg.get('market','CUSTOM'),'History':cfg['history'],'TargetPct':cfg['target'],'HorizonDays':cfg['horizon'],'MinTrainEvents':cfg['min_events'],'Status':'PARTIAL / STOPPED' if partial else 'COMPLETED','Note':'V6.1.1 shadow optimizer learns on WF folds 1-3 and reports fold-4 OOS. No automatic production promotion.'})
    return {'summary':summary,'gate':gate,'market_gate':market_gate,'optimizer':opt,'optimizer_weights':opt_weights,'premove':pm,'premove_summary':pm_summary,'premove_market':pm_market,'hourly_lift':hl,'hourly_summary':hl_summary,'hourly_market':hl_market,'audit':pd.DataFrame(audit),'skipped':pd.DataFrame(skipped,columns=['Ticker','Reason']) if skipped else pd.DataFrame(),'excel':excel,'partial':partial,'observations':len(rows)//2}


def _entry_worker_v603(runtime,jid,cfg):
    rows=[];audit=[];skipped=[];premove_rows=[];hourly_rows=[];completed=0;folds=[(.40,.55),(.55,.70),(.70,.85),(.85,1.00)];total=len(cfg['tickers'])
    try:
        for idx,t in enumerate(cfg['tickers'],1):
                ...
                try:
                    if callable(pre_move_indicator_lift):
                        ...
                        if pm is not None and not pm.empty:
                            pm=pm.copy();pm.insert(0,'Market',MARKET_MAP.get(t,'CUSTOM'));pm.insert(0,'Ticker',t);premove_rows.extend(pm.to_dict('records'))
                except Exception:pass
                try:
                    hraw=fetch_ohlcv(t,'6mo','1h')
                    if hraw is not None and len(hraw)>=80 and callable(hourly_lift_study):
                        hf=compute_features(hraw,True);hs=hourly_lift_study(hf,targets=(1,2,3),horizon_bars=(1,2,4,8),min_signals=10)
                        if hs is not None and not hs.empty:
                            hs=hs.copy();hs.insert(0,'Market',MARKET_MAP.get(t,'CUSTOM'));hs.insert(0,'Ticker',t);hourly_rows.extend(hs.to_dict('records'))
                except Exception:pass
                for fold,(train_end,test_end) in enumerate(folds,1):
                    _lab_check_v603(runtime,jid);i1=max(80,int(len(f)*train_end));i2=min(len(f),int(len(f)*test_end))
            ...
            completed=idx;_lab_update_v603(runtime,jid,completed,total,t,'Ticker complete')
        _lab_check_v603(runtime,jid);_lab_publish_v603(runtime,jid,_entry_payload_v603(rows,audit,skipped,cfg,False,premove_rows,hourly_rows))
    except _LabCancelled:
        try:payload=_entry_payload_v603(rows,audit,skipped,cfg,True,premove_rows,hourly_rows)
        except Exception:payload=None
        _lab_publish_v603(runtime,jid,payload,'stopped')
    except Exception as e:_lab_fail_v603(runtime,jid,e)
...
with tab1:
    ...
    if snap and snap.get('payload'):
        pay=snap['payload'];t=pay['ticker'];f=pay['f'];dyn=pay['dyn'];ent=pay['ent'];latest_live=pay['latest_live'];live_snapshot=pay.get('live_snapshot',{});ex=pay['ex'];timing=pay['timing'];decision=pay['decision'];ca_report=pay['ca_report'];bt=pay['bt'];buy_threshold=pay['buy_threshold'];comp=pay['comp'];ec=pay['ec'];cmp=pay['cmp'];compare_df=pay['compare_df'];ev=pay['ev'];early_cal=pay['early_cal'];quant_cal=pay['quant_cal'];target_run=pay['target'];horizon_run=pay['horizon']
        ...
        if display_current and np.isfinite(live_px):cur=live_px;price_label='Current price' if trade_fresh else 'Current / delayed price'
        ...
        st.markdown("<div class='section'>Decision cockpit</div>",unsafe_allow_html=True);p1,p2,p3,p4=st.columns([1.35,1,1,1]);p1.metric(price_label,f"{cur:,.3f} {currency}" if np.isfinite(cur) else 'UNAVAILABLE',delta=f"{ch:+.2f}%" if display_current and np.isfinite(ch) else None);p2.metric("TOP SCORE",f"{decision.get('TopScore',0):.1f}");p3.metric("Opportunity",f"{decision.get('OpportunityScore',0):.1f}");p4.metric("Exit Pressure",f"{decision.get('ExitPressure',0):.0f}",delta=decision.get('ExitStage','CLEAR'))
        ...
        st.markdown(f"### Entry: :{'green' if ent.get('trigger_state')=='CONFIRMED ENTRY' else 'orange'}[{ent.get('trigger_state',ent.get('status','WAIT'))}] · Movement: {decision.get('MovementStage','—')}")
        ...
        if bool(ent.get('plan_valid',False)) and ca_report.get('data_quality')=='OK':
            z1,z2,z3,z4=st.columns(4);z1.metric("Entry zone",f"{ent['zone_low']:.3f} – {ent['zone_high']:.3f}");z2.metric("Trade trigger",f"> {ent['trigger']:.3f}");z3.metric("Invalidation",f"< {ent['invalidation']:.3f}");z4.metric("Targets",f"{ent['target1']:.3f} / {ent['target2']:.3f}");st.caption(f"Entry Trigger V3: {ent.get('status','—')} • Quant is supporting evidence only; fresh + hourly + volume/flow + no-chase gates control confirmation.")
...
with tab2:
    ...
    universe_mode=st.radio("Market filter",["ALL 151","US 51","HONG KONG 50","TEL AVIV 50","Custom"],horizontal=True)
    if universe_mode == "ALL 151": default_universe=VALIDATION_151
    elif universe_mode == "US 51": default_universe=US_51
    ...
    with st.expander("Advanced scanner settings",expanded=False):
        st.caption("Fixed market views still scan all 151 so Global Rank is genuinely global. The selected market only changes what is displayed.")
    ...
    scan_universe=VALIDATION_151 if universe_mode!="Custom" else tickers
    prefilter_top=151 if universe_mode!="Custom" else min(151,max(10,len([x for x in tickers.split(',') if x.strip()])))
    show_mode=st.selectbox("Show",["ALL","CONFIRMED ENTRY","ARMED","TOP OPPORTUNITIES","ACTIONABLE NOW","WATCHLIST"],index=0)
    ...
    with b1:
        if st.button("▶ Start Scan",key="scanner_start_v601",use_container_width=True,disabled=scanner_busy):
            ...
            if ok:
                st.success("Scan started server-side. Global Rank will be calculated across the complete 151-stock universe.")
                st.rerun()
            else:st.warning(msg)
    with b2:
...
with tab4:
    ...
    st.markdown(f"<div class='section'>V{APP_VERSION} Liquidity / validation notes</div>",unsafe_allow_html=True)
...
with tab5:
    ...
    st.caption("Validates whether the causal Daily Setup + Fresh Signal + Volume/Flow + No-Chase gate improves unseen outcomes. Live confirmation still additionally requires Hourly/15m timing.")
    entry_market=st.radio("Entry validation market",["ALL 151","US 51","HONG KONG 50","TEL AVIV 50"],horizontal=True,key="entry_v_market")
    ...
    entry_universe={"ALL 151":VALIDATION_151,"US 51":US_51,"HONG KONG 50":HK_50,"TEL AVIV 50":TASE_50}[entry_market]
    entry_tickers=[x.strip() for x in entry_universe.split(',') if x.strip()]
    ...
    with c1:
        if st.button("▶ Start Entry Validation",key="entry603_start",use_container_width=True,disabled=busy):
            cfg={'tickers':entry_tickers,'market':entry_market,'history':ehist,'target':float(etarget),'horizon':int(ehorizon),'min_events':int(emin)};ok,msg=_start_lab_v603('entry',_entry_worker_v603,cfg,total=len(entry_tickers))
    ...
    if snap and snap.get('payload'):
        pay=snap['payload'];summary=pay.get('summary',pd.DataFrame());gate=pay.get('gate',pd.DataFrame());market_gate=pay.get('market_gate',pd.DataFrame());audit=pay.get('audit',pd.DataFrame());sk=pay.get('skipped',pd.DataFrame())
        ...
        st.markdown("#### Entry Trigger V3 daily proxy gate — Static vs Dynamic")
        if gate is None or gate.empty:st.info("No rows passed the causal Entry Trigger V3 daily proxy gate in the validated test blocks.")
        ...
        if market_gate is not None and not market_gate.empty:
            st.markdown("#### Entry Trigger V3 — behavior by market")
            st.dataframe(market_gate.round(2),use_container_width=True,hide_index=True)
            st.caption("Market rows are diagnostic. V6.1.1 only proposes market-specific shadow weights after out-of-sample confirmation.")
        opt=pay.get('optimizer',pd.DataFrame());ow=pay.get('optimizer_weights',pd.DataFrame())
        if opt is not None and not opt.empty:
            st.markdown("#### 🧠 Entry Optimizer — shadow weights learned from history");st.dataframe(opt.round(3),use_container_width=True,hide_index=True)
            st.caption("Weights are tuned on Walk-Forward folds 1–3 and judged on unseen Fold 4. They are NOT auto-promoted to production.")
            if ow is not None and not ow.empty:st.dataframe(ow.round(2),use_container_width=True,hide_index=True)
        pm=pay.get('premove_summary',pd.DataFrame())
        if pm is not None and not pm.empty:
            st.markdown("#### ⏪ Pre-Move indicator lift — what appeared 1–3 days before")
            st.dataframe(pm.head(80).round(2),use_container_width=True,hide_index=True)
        hl=pay.get('hourly_summary',pd.DataFrame())
        if hl is not None and not hl.empty:
            st.markdown("#### ⏱ Hourly lift vs same-hour baseline")
            st.dataframe(hl.head(80).round(2),use_container_width=True,hide_index=True)
        if audit is not None and not audit.empty:st.markdown("#### Stock / fold audit");st.dataframe(audit,use_container_width=True,hide_index=True)
        if sk is not None and not sk.empty:
            with st.expander(f"Skipped stocks ({len(sk)})"):st.dataframe(sk,use_container_width=True,hide_index=True)
...
with tab6:
    ...
    if snap and snap.get('payload'):
        ...
        if ewf is not None and not ewf.empty:
            ...
            if {'Target','Horizon'}.issubset(ewf.columns):
                p10=ewf[(ewf.Target=='+10%') & (ewf.Horizon=='3D')];p15=ewf[(ewf.Target=='+15%') & (ewf.Horizon=='3D')]
                e1,e2=st.columns(2)
                if not p10.empty:
                    r=p10.iloc[0];e1.metric("Primary explosive: +10% / 3D",f"{r['Median Lift']:.2f}x lift" if np.isfinite(r['Median Lift']) else '—',delta=f"Positive folds {r['Positive Folds']}")
                if not p15.empty:
                    r=p15.iloc[0];e2.metric("Extreme explosive: +15% / 3D",f"{r['Median Lift']:.2f}x lift" if np.isfinite(r['Median Lift']) else '—',delta=f"Positive folds {r['Positive Folds']}")
...
with tab7:
    st.markdown("<div class='section'>🧪 Adaptive 151-Stock Research</div>",unsafe_allow_html=True)
    ...
    r_market=st.radio("Research market",["ALL 151","US 51","HONG KONG 50","TEL AVIV 50"],horizontal=True,key="r_market")
    ...
    with rb1:
        if st.button("▶ Start Research",key="run_602_research",use_container_width=True,disabled=research_busy):
            if not all(callable(x) for x in [historical_signal_timeline,threshold_optimization,adaptive_target_horizon_v2,atr_target_validation,pre_move_study,topk_daily_validation]):
                ...
                _ru={'ALL 151':VALIDATION_151,'US 51':US_51,'HONG KONG 50':HK_50,'TEL AVIV 50':TASE_50}[r_market]
...
with tab8:
    ...
    st.caption("Every completed live scan is snapshotted. Due outcomes are checked at 1D / 3D / 5D. Recent resolved outcomes receive recency weighting, but small samples never auto-retrain production. Candidate changes stay in shadow until confirmed.")
    fb_half_life=st.selectbox("Recent-feedback half-life (days)",[30,60,90],1,key="feedback_half_life_v611",help="Lower = recent outcomes adapt faster; higher = more stable. This changes analytics only, never production weights automatically.")
    ...
    if sn.empty:
        ...
        st.markdown("#### Recent evaluated snapshots");showcols=['ticker','market','horizon','trade_stage','entry_state','confirmation_pct','daily_setup','fresh_signal','hourly_entry','volume_flow','no_chase','market_regime','movement_stage','top_score','opportunity','entry_score','hourly','exit_pressure','global_rank','End Return %','MFE %','MAE %','first_event','Clean Outcome'];st.dataframe(merged[[c for c in showcols if c in merged]].head(100),use_container_width=True,hide_index=True)
        excel_feedback=workbook_bytes({'Snapshots':sn,'Outcomes':oc,'Stage Summary':stage,'Recent Weighted Summary':recent_summary,'Recent Feature Lift':recent_features},{'Tab':'Feedback','Version':APP_VERSION,'RecencyHalfLifeDays':int(fb_half_life)})
