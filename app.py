...
APP_VERSION = "6.1.3"
APP_BUILD_ID = "V613-CHASE-EXTENSION-GUARD-20260918-B"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • MODEL REGISTRY • HOURLY OOS • DISCOVERY • FEEDBACK</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Best Eligible OOS Model • Hourly Timing • Live Feedback</div></div>""",unsafe_allow_html=True)
...
def _import_optimizer_workbook_v613(file_obj):
    """Import a V6.1.2/V6.1.3 Optimizer workbook into the registry.
...
with tab_analyze:
    ...
    if snap and snap.get('payload'):
        ...
        _cr=float(ent.get('chase_risk_score',0) or 0);_sm=float(ent.get('session_move_pct',np.nan));_st=float(ent.get('since_trigger_pct',np.nan));_va=float(ent.get('vwap_distance_atr',np.nan));_tp=float(ent.get('target1_progress_pct',np.nan))
        _risk_line=f"Chase Risk {ent.get('chase_risk_label','LOW')} ({_cr:.0f}/100) • Session {f'{_sm:+.1f}%' if np.isfinite(_sm) else '—'} • Since original trigger {f'{_st:+.1f}%' if np.isfinite(_st) else '—'} • VWAP distance {f'{_va:.2f} ATR' if np.isfinite(_va) else '—'} • Volume {ent.get('volume_trend','NO DATA')}"
        ...
        if bool(ent.get('plan_valid',False)) and ca_report.get('data_quality')=='OK':
            z1,z2,z3,z4=st.columns(4);z1.metric("Entry zone",f"{ent['zone_low']:.3f} – {ent['zone_high']:.3f}");z2.metric("Trade trigger",f"> {ent['trigger']:.3f}");z3.metric("Invalidation",f"< {ent['invalidation']:.3f}");z4.metric("Targets",f"{ent['target1']:.3f} / {ent['target2']:.3f}");st.caption(f"Entry Trigger V3: {ent.get('status','—')} • Entry zone anchored to original fresh trigger ({ent.get('trigger_anchor_timeframe','—')} @ {ent.get('trigger_anchor_price',np.nan):.3f} when available) • Chase/Extension Guard can hard-block a late entry even at 6/6 gates.")
...
with tab_optimizer:
    ...
    with st.expander("Import prior V6.1.2 Optimizer workbooks into Model Registry"):
        ...
        reg_now=_load_model_registry_v613();st.caption(f"Registry currently stores {len(reg_now.get('models',[]))} optimization run(s). Imported V6.1.2 daily models can be reused immediately; hourly OOS models are learned only by V6.1.3 runs.")
