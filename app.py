...
APP_VERSION = "6.3.9.7"
APP_BUILD_ID = "V6397-RANKING-V2-HOTFIX-20260922-A"
...
@media (max-width: 700px){
  ...
  .top5-name{font-size:1.12rem}.top5-score{font-size:1.25rem}.top5-opp{font-size:.88rem}
}
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # V6.3.9.7: final Top-5 / Scanner order uses the transparent
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    ...
                    f"<div class='top5-score'><small>TOP</small> {top_score:.1f}</div>"
                    f"<div class='top5-opp'><small>OPP</small> {opp_score:.1f}</div></div>",
                    unsafe_allow_html=True,
                )
                _drs=_num_v6394(r.get('DecisionRankScore'),np.nan)
                _drs_txt=f"{_drs:.1f}/100" if np.isfinite(_drs) else '—'
                st.markdown(f"<div class='top5-rankline'>Ranking V2 <b>{_drs_txt}</b> • timing-aware</div>",unsafe_allow_html=True)
