...
APP_VERSION = "6.1.9"
APP_BUILD_ID = "V619-POST-SPIKE-CONTINUATION-LAB-20260919-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • POST-SPIKE STATE • CONTINUATION LAB • ORIGIN OOS AUDIT</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Post-Spike Quality • Continuation Reset Research • Causal Origin Validation • OOS Evidence</div></div>""",unsafe_allow_html=True)
...
with tab_analyze:
    ...
    if snap and snap.get('payload'):
        ...
        if bool(ent.get('plan_valid',False)) and ca_report.get('data_quality')=='OK':
            ...
            if ent.get('continuation_base_candidate',False):st.info(f"Continuation Lab (research only): base {safe(ent.get('continuation_base_low'),2)}–{safe(ent.get('continuation_base_high'),2)} • {int(ent.get('continuation_base_bars',0) or 0)} bars • range {safe(ent.get('continuation_base_range_pct'),2)}% • volume dry-up {safe(ent.get('continuation_volume_dryup_ratio'),2)}x • breakout audit {safe(ent.get('continuation_breakout_trigger'),2)}")
