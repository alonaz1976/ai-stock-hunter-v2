...
APP_VERSION = "6.2.4"
APP_BUILD_ID = "V624-CROSS-STOCK-BOOTSTRAP-TOP-EXCEL-20260919-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • CROSS-STOCK BOOTSTRAP • TOP EXCEL • NO-OVERFIT GUARD</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Adaptive Entry Intelligence • Cross-Stock Matched Evidence • Session-Cluster Bootstrap • Causal Validation</div></div>""",unsafe_allow_html=True)
    ...
    try:
        ...
        for i,t in enumerate(tickers,1):
            ...
            try:
                ...
                w=base[base['OpeningBarOrdinal'].astype(str)=='WEIGHTED'].iloc[-1] if isinstance(base,pd.DataFrame) and not base.empty and (base['OpeningBarOrdinal'].astype(str)=='WEIGHTED').any() else None
                clean_pairs=pairs[np.isfinite(pd.to_numeric(pairs.get('SignalWin'),errors='coerce'))] if isinstance(pairs,pd.DataFrame) and not pairs.empty else pd.DataFrame()
                ...
                stock_rows.append({'Ticker':t,'Market':market,'15mBars':len(f15),'MatchedSignals':len(pairs) if isinstance(pairs,pd.DataFrame) else 0,
                                   'CleanMatchedSignals':len(clean_pairs),'SignalHitRatePct':100*pd.to_numeric(clean_pairs.get('SignalWin'),errors='coerce').mean() if len(clean_pairs) else np.nan,
                                   'MatchedBaselineHitRatePct':float(w.get('BaselineHitRatePct',np.nan)) if w is not None else np.nan,
                                   'MatchedHitRateLiftX':float(w.get('HitRateLiftX',np.nan)) if w is not None else np.nan,
                                   'MatchedExpectancyDeltaPct':float(w.get('ExpectancyDeltaPct',np.nan)) if w is not None else np.nan,
...
def _render_optimizer_top_excel_v624(slot):
    ...
    with slot.container():
        ...
        if not available:
            st.caption('⬇️ Excel downloads will appear here after a completed Optimizer or Cross-Stock Research run.')
            return
        cols=st.columns(len(available))
        for c,(label,data) in zip(cols,available):
            with c:st.download_button(f'⬇️ {label} Excel',data=data,file_name=f"AI_Stock_Hunter_V{APP_VERSION}_{label.replace(' ','_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',use_container_width=True,key=f"top_opt_{label}_v624")
...
with tab_optimizer:
    ...
    _render_optimizer_top_excel_v624(optimizer_excel_top)
