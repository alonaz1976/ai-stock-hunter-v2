...
with tab_feedback:
    ...
    st.caption("V6.3.5 research-only benchmark. Prices are split/corporate-action normalized before indicators and future +15% labels. Unexplained giant gaps are excluded, and overlapping hits are separated into independent explosive legs after a cool-off/reset. Use 1196.HK as a benchmark, then validate the same idea across a wider universe.")
    ...
    with xbc1:
        if st.button("▶ Run Explosive Benchmark",key="explosive_benchmark_start_v635",use_container_width=True,disabled=xb_busy):
            ok,msg=_start_lab_v603('explosive_benchmark',_explosive_benchmark_worker_v635,{'ticker':xb_ticker,'history':xb_hist,'target_pct':float(xb_target)/100.0,'horizons':(1,2,3,5)},total=5)
            if ok:st.success("Explosive Benchmark started server-side. You can leave the app and return later.");st.rerun()
            else:st.warning(msg)
    with xbc2:
        if st.button("■ Stop Benchmark",key="explosive_benchmark_stop_v635",use_container_width=True,disabled=not xb_busy or (xa and xa.get('status')=='stopping')):
    ...
    st.markdown("#### 🌐 Explosive Universe Radar — find the next +15% setup across markets")
    st.caption("Research-only daily early-warning radar. It compares today's completed-session fingerprint with split-safe historical +15% analogs and OOS-promising combinations. A candidate is not a trade trigger; use Analyze/Scanner for live timing and risk.")
    ...
    with erc1:
        if st.button("▶ Run Explosive Universe Radar",key="explosive_radar_start_v635",use_container_width=True,disabled=er_busy):
            ok,msg=_start_lab_v603('explosive_universe',_explosive_universe_worker_v635,{'scope':er_scope,'max_tickers':int(er_count),'history':er_hist,'target_pct':float(er_target)/100.0},total=max(1,int(er_count)+2))
            if ok:st.success("Explosive Universe Radar started server-side. You can leave the app and return later.");st.rerun()
            else:st.warning(msg)
    with erc2:
        if st.button("■ Stop Explosive Radar",key="explosive_radar_stop_v635",use_container_width=True,disabled=not er_busy or (era and era.get('status')=='stopping')):
    ...
    if erpay:
        ...
        if isinstance(erd,pd.DataFrame) and not erd.empty:
            strong=int(erd.Status.eq('STRONG EXPLOSIVE CANDIDATE').sum());cand=int(erd.Status.eq('EXPLOSIVE CANDIDATE').sum())
            st.info(f"Current radar: {strong} STRONG + {cand} EXPLOSIVE candidates. These are early-warning research candidates, not automatic entries.")
            st.dataframe(_feedback_color_table_v629(erd.round(3),'Status'),use_container_width=True,hide_index=True,height=480)
        if erpay.get('excel'):
            st.download_button("⬇️ Download Explosive Universe Radar Excel",data=erpay['excel'],file_name=f"AI_Stock_Hunter_V{APP_VERSION}_Explosive_Radar_{er_scope.replace(' ','_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',use_container_width=True,key='explosive_radar_excel_v635')
