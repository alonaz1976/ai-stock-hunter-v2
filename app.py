...
APP_VERSION = "6.3.9.13"
APP_BUILD_ID = "V63913-OFFICIAL-CLOSE-PRICE-FALLBACK-20260922-A"
...
def _top5_current_price_v63913(ticker):
    """Resolve the Top-5 display price without presenting a stale scan snapshot as current.

    Priority:
      1) current/delayed quote from the live resolver;
      2) when the market is CLOSED, today's official/latest daily close;
      3) otherwise no current price (the UI may show the old scan value only as SCAN PRICE).

    This display refresh never changes Ranking/Entry/R:R; rerun Scanner for that.
    """
    ...
    try:
        ...
        phase=str((_market_status_detail_v612(market) or {}).get('phase','UNKNOWN')).upper()
    except Exception:
        phase='UNKNOWN'
    # After the cash session closes, prefer the latest daily close over a provider
    # quote endpoint that can remain on an earlier intraday print.  This fixes cases
    # such as 2202.HK showing 2.500 even though the official 22-Sep close is 2.440.
    if phase=='CLOSED' and fetch_ohlcv is not None:
        try:
            d=fetch_ohlcv(logical,period='5d',interval='1d')
                ...
                if not dd.empty:
                    ...
                    if np.isfinite(px) and px>0:
                        ...
                        return {
                            ...
                            'provider_symbol':provider,'timestamp_label':idx.strftime('%Y-%m-%d')+' close',
                            'source':'Latest official/daily close','quote_status':'OFFICIAL/LAST CLOSE',
                            'same_session_date':True,'age_minutes':np.nan
                        }
        except Exception:
            pass
    if q.get('display_current'):
        return q
    return q
...
# -----------------------------------------------------------------------------
# V6.3.9.7 Decision Ranking Engine V2 hotfix
# Final Scanner rank is no longer a hard Entry-stage ordering (ARMED > WAIT).
# It is a transparent, timing-aware composite of the same fields visible on the
# Top-5 cards. Entry state is only a small modifier, so a mediocre ARMED setup
# cannot automatically outrank a materially stronger WAIT setup. Catalyst stays
# out of the score because it is fetched only for a subset of candidates; using
# it here would create a coverage/selection bias. Production Entry gates, targets
# and stops are unchanged.
# -----------------------------------------------------------------------------

def _apply_decision_ranking_v6397(df,scan_mode='Production 151'):
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    # Preserve the pre-V2 ranks for audit/debugging when available.
    ...
    def consumed_penalty(r):
        try:v=float(r.get('MoveConsumedBeforeTriggerPct',np.nan))
        except Exception:v=np.nan
        if not np.isfinite(v):return 0.0
        # No penalty in the early zone; then progressively penalize lateness.
        if v<=40:return 0.0
        if v<=70:return (v-40.0)/30.0*4.0
        return min(16.0,4.0+(v-70.0)/30.0*12.0)
    def chase_penalty(r):
        try:v=float(r.get('ChaseRiskScore',np.nan))
        except Exception:v=np.nan
        if not np.isfinite(v):return 0.0
        return min(12.0,max(0.0,v-40.0)*0.20)
    def exit_penalty(r):
        try:v=float(r.get('ExitPressure',np.nan))
        except Exception:v=np.nan
        if not np.isfinite(v):return 0.0
        return min(22.0,max(0.0,v-30.0)*0.32)
    def stage_modifier(r):
        stage=str(r.get('TradeStage',r.get('EntryTriggerState','WAIT')) or 'WAIT').upper()
        return {'CONFIRMED ENTRY':4.0,'ARMED':2.5,'WATCH':1.0,'WAIT':0.0,'EXTENDED — DO NOT CHASE':-4.0,'TOO LATE / CHASE':-8.0,'INVALIDATED':-15.0}.get(stage,0.0)
    def row_score(r):
        top=n(r,'TopScore',0);entry=n(r,'LiveActionabilityScore',n(r,'EntryScore',50));hour=n(r,'HourlyConfirm',50);money=n(r,'MoneyFlowScore',50);cycle=n(r,'MarketCycleScore',50);rrq=rr_quality(r)
        # TOP already contains Opportunity + Prediction + Reliability, so they
        # are intentionally not added again here (avoids double counting).
        core=.40*top+.20*entry+.10*hour+.10*money+.08*cycle+.12*rrq
        optimized=np.nan
        try:optimized=float(r.get('OptimizedScore',np.nan))
        except Exception:pass
        if str(scan_mode) in ('Optimized 151','Discovery') and np.isfinite(optimized):
            core=.72*core+.28*float(np.clip(optimized,0,100))
        sp=stage_modifier(r);tp=consumed_penalty(r);cp=chase_penalty(r);ep=exit_penalty(r)
        dq=0.0 if str(r.get('DataQuality','OK'))=='OK' else 20.0
        eg=0.0 if bool(r.get('EvidenceGuardOK',True)) else 8.0
        final=float(np.clip(core+sp-tp-cp-ep-dq-eg,0,100))
        reason=f'Core {core:.1f} • stage {sp:+.1f} • late -{tp:.1f} • chase -{cp:.1f} • exit -{ep:.1f}'
        if dq:reason+=f' • data -{dq:.0f}'
        if eg:reason+=f' • evidence -{eg:.0f}'
        return pd.Series({'DecisionRankScore':round(final,1),'DecisionRankCore':round(core,1),'DecisionRankRRQuality':round(rrq,1),'DecisionRankStageModifier':round(sp,1),'DecisionRankTimingPenalty':round(tp,1),'DecisionRankChasePenalty':round(cp,1),'DecisionRankExitPenalty':round(ep,1),'DecisionRankReason':reason,'DecisionRankVersion':'V2'})

    rank_fields=z.apply(row_score,axis=1)
    for c in rank_fields.columns:z[c]=rank_fields[c]
    z=z.sort_values(['DecisionRankScore','TopScore','OpportunityScore'],ascending=False).reset_index(drop=True)
    z['GlobalRank']=np.arange(1,len(z)+1)
...
def _ensure_decision_ranking_v6397(df, scan_mode='Production 151'):
    """Backfill Ranking V2 into snapshots created before V6.3.9.6.

    Streamlit cache_resource can survive a hot redeploy while the server process is
    still alive. That means the new UI can receive an older last_completed DataFrame
    that has no DecisionRankScore column. Recompute the deterministic Decision
    Intelligence fields if needed, then rank the entire snapshot before any view
    filtering so the visible #1–#5 and Ranking V2 value always agree.
    """
    if df is None or not isinstance(df,pd.DataFrame) or df.empty:return df
    z=df.copy()
    ...
    # Always recompute the rank when called: it is cheap, deterministic, and also
    # repairs partially populated/stale V2 columns from an older cached snapshot.
    return _apply_decision_ranking_v6397(z,scan_mode)
...
def _scanner_worker_v599(runtime,job_id,config):
    ...
    try:
        ...
        if isinstance(res,pd.DataFrame) and not res.empty:
            ...
            # timing-aware Decision Ranking V2. Optimized modes retain their
            # learned score as a bounded blend instead of bypassing live timing.
            res=_apply_decision_ranking_v6397(res,config.get('scan_mode','Production 151'))
...
def _render_scanner_results_v599(show_mode,market_filter="ALL 151"):
    ...
    # V6.3.9.7 hotfix: an older cached last_completed scan may survive a code
    # redeploy and therefore have no Ranking V2 columns. Backfill and re-sort the
    # FULL snapshot before filtering. This fixes cards showing `Ranking V2 —` and
    # prevents legacy ARMED-first ordering from leaking into the new UI.
    _rank_version_ok=(
        ...
        full['DecisionRankVersion'].astype(str).eq('V2').all()
    )
    if not _rank_version_ok:
        full=_ensure_decision_ranking_v6397(full,str((last.get('config') or {}).get('scan_mode','Production 151')))
    ...
    if res is None or res.empty:
        ...
        for _,r in topcards.iterrows():
            ...
            with st.container(border=True):
                ...
                _px_live_badge=''
                try:
                    _live_px=_top5_current_price_v63913(_logical_ticker)
                except Exception:
                    _live_px={}
                ...
                elif not np.isfinite(_px) and np.isfinite(_scan_px):
                    _price_meta+=f" • SCAN PRICE {_scan_px:.3f}"
                st.markdown(f"<div class='top5-rankline'><b>TOP {top_score:.1f}</b> • <b>OPP {opp_score:.1f}</b> • Ranking V2 • timing-aware{_bridge_html}{_price_meta}</div>",unsafe_allow_html=True)
                ...
                _mf=_num_v6394(r.get('MoneyFlowScore'));_mc=_num_v6394(r.get('MarketCycleScore'));_cat=_num_v6394(r.get('CatalystScore'));_entry=_num_v6394(r.get('LiveActionabilityScore',r.get('EntryScore')))
                ...
                st.markdown(
                    ...
                    f"<div class='top5-mini'><span class='lbl'>Market Cycle</span><span class='val'>{_e6395(r.get('MarketCycleStage','NO DATA'))}</span><span class='sub'>Score {_mc_txt}/100</span></div>"
                    f"<div class='top5-mini'><span class='lbl'>Catalyst</span><span class='val'>{_cat_txt}/100</span><span class='sub'>{_e6395(r.get('CatalystLabel','NO DATA'))}</span></div>"
                    ...
                    f"<div class='top5-mini'><span class='lbl'>Hourly</span><span class='val'>{_hour_txt}</span><span class='sub'>confirmation</span></div>"
                ...
                with st.expander("Details / Why?",expanded=False):
                    ...
                    st.caption(f"Ranking V2 {safe(r.get('DecisionRankScore'),1)}/100 • {r.get('DecisionRankReason','—')}")
        ...
        st.caption("Ranking V2: Trade Priority = timing-aware Decision Rank across the full scan • Global Rank = the same full-universe order • Market Rank = exchange rank • Sector Rank = sector rank inside that market. Entry state is only a small modifier; late/chase/exit risks can reduce rank.")
...
with tab_scan:
    ...
    show_mode=st.selectbox("Show",["ALL","UNDERVALUED (RESEARCH)","REGULAR HIGH CONFIDENCE","REGULAR RADAR","STRONG PRE-MOVE CANDIDATE","PRE-MOVE CANDIDATE","CONFIRMED ENTRY","OPTIMIZED CONFIRMED","OPTIMIZED ARMED","ARMED","EXTENDED / RETEST","TOP OPPORTUNITIES","ACTIONABLE NOW","WATCHLIST"],key="show_v613")
    valuation_overlay=st.checkbox("Enable valuation overlay (research-only)",value=True,key="valuation_overlay_v6394",help="Fetches cached fundamentals and compares valuation multiples with peers. First run can take longer; results never change Entry/ARMED/CONFIRMED.")
    catalyst_overlay=st.checkbox(f"Enable catalyst overlay for Top {_CATALYST_TOP_N_V6394} (research-only)",value=True,key="catalyst_overlay_v6394",help="Checks cached recent Yahoo headlines and earnings proximity only for the highest-priority candidates. It never changes ranking or Entry.")
    st.caption("Decision Intelligence: Money Flow + Market Cycle are computed for every analyzed stock; Catalyst is fetched for top candidates. Ranking V2 uses TOP + Entry + Hourly + Money Flow + Market Cycle + R:R, then applies explicit late/chase/exit penalties. Entry state is only a small modifier; Catalyst and Valuation remain research-only and do not affect rank.")
