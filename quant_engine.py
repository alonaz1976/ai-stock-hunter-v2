...
ENGINE_BUILD_ID = "V600-DIRECTIONAL-VOLUME-EXIT-FEEDBACK-20260916-A"
...
def normalize_cross_timeframes(daily, hourly=None, m15=None):
    """Normalize 1h/15m price scale to daily when a split-like scale mismatch exists.

    auto_adjust usually handles splits, but providers can temporarily return mixed
    adjustment scales across intervals. Only clear multiplicative mismatches are
    adjusted. Returns normalized frames plus a corporate-action/data-quality report.
    """
    ...
    report={'split_adjusted':False,'data_quality':'OK','details':[]}
    if d is None or d.empty:return d,h,m,report
    dp=float(d['Close'].iloc[-1])
    def fix(frame,label):
        nonlocal report
        if frame is None or frame.empty:return frame
        ip=float(frame['Close'].iloc[-1])
        if not (np.isfinite(dp) and np.isfinite(ip) and dp>0 and ip>0):return frame
        ratio=dp/ip
        mismatch=abs(ratio-1.0)
        if mismatch<=0.12:return frame
        # split-like factors: 2:1, 3:1, 4:1, 5:1, 10:1 and reverse splits.
        ...
        if err<=0.10:
            ...
            for c in ['Open','High','Low','Close']:
                if c in z:z[c]=pd.to_numeric(z[c],errors='coerce')*ratio
            report['split_adjusted']=True
            report['details'].append(f'{label} rescaled x{ratio:.4g}')
            return z
        report['data_quality']='MISMATCH'
        report['details'].append(f'{label} scale mismatch {ratio:.3f}x')
        return frame
    h=fix(h,'1H'); m=fix(m,'15m')
    return d,h,m,report
...
def exit_pressure_row(r):
    ...
    stage='EXIT TRIGGER' if score>=72 else 'EXIT ARMED' if score>=55 else 'EXIT WATCH' if score>=35 else 'CLEAR'
...
def entry_timing(feat, quant_score, early_score):
    ...
    elif score>=72 and quant_score>=66 and xp<45:status='ENTER ZONE'
