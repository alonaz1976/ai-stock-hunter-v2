...
ENGINE_BUILD_ID = "V605-STALE-QUOTE-HARD-GATE-20260917-A"
...
def fetch_live_intraday_snapshot(ticker: str, daily=None):
    """Fetch a current-session provider quote without ever promoting stale bars.

    V6.0.5 tries 1-minute first and then 5-minute data.  A bar from a prior local
    market date is retained only as diagnostic metadata (`stale_*`); it is never
    returned as `price`.  This directly prevents a pre-split 10.85 print from
    being shown as today's intraday quote several sessions later.
    """
    ...
    try:
        ...
        best_stale=None
        attempts=[('1m','1d',15.0),('5m','5d',35.0)]
        for interval,period,max_age in attempts:
            try:
                ...
                # Hard gate: yesterday/last week can never be today's live price.
                if not (same and age<=max_age):
                    continue
                aligned=_align_current_quote_to_daily(raw_price,daily,ticker,out.get('split_ratio',np.nan))
                if aligned.get('scale_suspect'):
                    out['scale_suspect']=True;out['details'].extend(aligned.get('details',[]))
                    continue
                price=float(aligned['price'])
                ...
                out.update({'price':price,'prev_close':prev_close,'change_pct':change,
                            ...
                            'source':f'{interval} current-session provider feed','fresh':True,
                            'same_session_date':True,'age_minutes':age,
                            'quote_split_adjusted':bool(aligned.get('adjusted',False))})
                out['details'].extend(aligned.get('details',[]))
                return out
            except Exception:
                continue
...
def resolve_current_market_snapshot(ticker: str, daily=None, hourly=None, m15=None,
    ...
    """Choose a trustworthy display/session price after frame normalization.

    Priority while the market is open:
      1) verified current-session 1m/5m quote from `primary`
      2) current-session confirmed 15m bar
      3) current-session confirmed 1H bar
      4) no live price (Daily close is exposed only as `fallback_price`)

    Outside an open session, the latest Daily close is a valid display fallback.
    """
    p=dict(primary or {})
    ...
    candidates=[]
    if p.get('fresh') and np.isfinite(_finite(p.get('price'),np.nan)) and not p.get('scale_suspect'):
        candidates.append(('primary',p))
    ms=frame_price_snapshot(ticker,m15,45.0); ms['source']='15m current-session confirmed bar'
    hs=frame_price_snapshot(ticker,hourly,110.0); hs['source']='1H current-session confirmed bar'
    if ms.get('fresh'):candidates.append(('15m',ms))
    if hs.get('fresh'):candidates.append(('1H',hs))

    if candidates:
        _,sel=candidates[0]
        price=_finite(sel.get('price'),np.nan)
        # The frames have already been normalized before this resolver is called.
        ...
        base.update({'price':price,'prev_close':prev_close,'change_pct':change,
                     ...
                     'source':sel.get('source','current-session intraday'),'fresh':True,
                     'same_session_date':True,'age_minutes':sel.get('age_minutes',np.nan)})
        return base

    # Never promote an older intraday bar while the market is open.
    if market_open:
        base['source']='LIVE PRICE UNAVAILABLE — using last official close only as reference'
        return base

    # Closed/pre-open: the latest official Daily close is safe as a non-live display value.
    if np.isfinite(daily_last) and daily_last>0:
        d=_canonical_ohlcv(daily,ticker=ticker)
        prev=np.nan
        ...
        base.update({'price':daily_last,'prev_close':prev,'change_pct':change,
                     ...
                     'source':'latest official Daily close','fresh':True,
