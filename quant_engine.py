...
ENGINE_BUILD_ID = "V614-TRIGGER-ANCHOR-AUDIT-20260918-A"
...
def _transition_candidates_v614(frame, bars, timeframe):
    ...
    for pos,(idx,row) in enumerate(f.iterrows()):
        ...
        out.append({
            'price':float(px),'timestamp':str(idx),'ts_sort':ts,'date':local_date,
...
def _intraday_setup_anchor_v614(frame,bars,timeframe):
    ...
    if today:
        ...
        try:
            ...
            after=[c for c in today if posmap.get(next((ix for ix in session.index if str(ix)==c['timestamp']),None),-1)>reset]
        except Exception:
            after=today
        ...
        chosen=min(usable,key=lambda x:x.get('ts_sort',pd.Timestamp.max.tz_localize('UTC')))
    ...
    chosen=max(cands,key=lambda x:x.get('ts_sort',pd.Timestamp.min.tz_localize('UTC')))
...
def _original_trigger_anchor_v614(daily, hourly=None, m15=None):
    ...
    if current:
        chosen=min(current,key=lambda x:x.get('ts_sort',pd.Timestamp.max.tz_localize('UTC')))
        # Prefer 15m when timestamps are effectively the same.
        if len(current)>1:
            t0=chosen.get('ts_sort')
            close15=[x for x in current if x.get('timeframe')=='15m' and pd.notna(x.get('ts_sort')) and pd.notna(t0) and abs((x['ts_sort']-t0).total_seconds())<=3600]
            if close15:chosen=min(close15,key=lambda x:x['ts_sort'])
        return {k:v for k,v in chosen.items() if not str(k).startswith('_')}
    if intraday:
        chosen=max(intraday,key=lambda x:x.get('ts_sort',pd.Timestamp.min.tz_localize('UTC')))
        return {k:v for k,v in chosen.items() if not str(k).startswith('_')}
...
def _extension_risk_v614(f,r,p,a,zone_low,zone_high,entry_mid,target1,trigger_anchor=None,hourly_feat=None,m15_feat=None,previous_close=None):
    ...
    score=(25*ramp(sr,.03,.10)+20*ramp(st,.02,.08)+15*ramp(max(0,dvwap) if np.isfinite(dvwap) else 0,.75,2.0)+10*ramp(max(0,de9,de20),.80,2.0)+10*ramp(gp,.02,.07)+10*ramp(tp,.25,.85)+10*ramp(sa,.70,1.80))
...
def entry_timing(feat, quant_score, early_score, hourly_feat=None, m15_feat=None, current_price=None, market_regime='NEUTRAL', previous_close=None):
    ...
    extension=_extension_risk_v614(f,r,p,a,zone_low,zone_high,entry_mid,target1,trigger_anchor,hourly_feat,m15_feat,previous_close)
    ...
    return {
        ...
        'open_move_pct':extension.get('open_move_pct',np.nan),'gap_pct':extension.get('gap_pct',np.nan),'since_trigger_pct':extension.get('since_trigger_pct',np.nan),'vwap_distance_atr':extension.get('vwap_distance_atr',np.nan),'ema9_distance_atr':extension.get('ema9_distance_atr',np.nan),'ema20_distance_atr':extension.get('ema20_distance_atr',np.nan),'target1_progress_pct':extension.get('target1_progress_pct',np.nan),
        'volume_trend':extension.get('volume_trend','NO DATA'),'volume_trend_timeframe':extension.get('volume_trend_timeframe','—'),
        'trigger_anchor_price':_finite((trigger_anchor or {}).get('price',np.nan)),'trigger_anchor_time':anchor_ts,'trigger_anchor_timeframe':anchor_tf,'trigger_anchor_signals':anchor_signals,'trigger_anchor_quality':anchor_quality,'trigger_anchor_age_bars':anchor_age,'trigger_anchor_selection':anchor_selection,
