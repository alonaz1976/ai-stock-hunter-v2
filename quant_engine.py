...
ENGINE_VERSION = "6.1.9"
ENGINE_BUILD_ID = "V619-POST-SPIKE-CONTINUATION-LAB-20260919-A"
...
def _post_spike_state_v619(f, hourly=None, m15=None, previous_close=None, current_price=None, momentum_state=None):
    """Describe whether a large move is holding constructively or failing.

    Research/audit only in V6.1.9. It never clears the Chase/Extension Guard.
    The purpose is to distinguish healthy high retention from actual distribution
    after an explosive day, so WAIT FOR RETEST does not imply the setup is bearish.
    """
    ...
    p=_finite(current_price,_finite(s.iloc[-1].get('Close',np.nan)))
    ...
    last=s.iloc[-1]
    vw=_finite(last.get('vwap',np.nan)); e20=_finite(last.get('ema20',np.nan)); e9=_finite(last.get('ema9',np.nan))
    bear=_finite(last.get('bearish_volume_evidence',np.nan),0); dist=_finite(last.get('distribution_persistence3',np.nan),0)
    structure=bool((not np.isfinite(vw) or p>=vw) and (not np.isfinite(e20) or p>=e20) and (not np.isfinite(e9) or p>=e9-.15*_finite(last.get('atr14',np.nan),0)))
    mlab=str((momentum_state or {}).get('label','NO DATA')).upper()
    if np.isfinite(retention) and retention<.50:
        state='FAILED SPIKE'
    elif (np.isfinite(retention) and retention<.68) or (not structure and (bear>=.12 or dist>=1)):
...
def _continuation_base_v619(m15, previous_close=None, current_price=None):
    ...
    breakout=bh+.10*iatr if np.isfinite(iatr) and iatr>0 else bh*1.0015
    status='RESEARCH • BASE FORMING' if candidate else ('RESEARCH • HOLDING HIGH' if hold and np.isfinite(move) and move>=.05 else 'RESEARCH • NO BASE')
...
def _carryover_extension_v617(f,p,a):
    """Persist extension risk into the next session so the guard does not reset at midnight.

    The check is deliberately conservative and only looks at the immediately prior
    completed daily bar.  A prior explosive session remains a veto while most of
    that move is still retained.  This prevents a +10% day from becoming a fresh
    low-risk entry merely because the next session starts at +0.2%.
    """
    out={'active':False,'prior_session_move_pct':np.nan,'prior_session_move_atr':np.nan,
         ...
         'carryover_peak_close':np.nan,'hard':False,'reason':''}
    if f is None or not isinstance(f,pd.DataFrame) or len(f)<3 or not np.isfinite(p) or p<=0:
        return out
    q=f.dropna(subset=['Close'])
    if len(q)<3:return out
    # The last row may be the current/incomplete session.  The immediately prior
    # row is therefore the completed session whose extension can carry forward.
    prev=q.iloc[-2]; base=q.iloc[-3]
    ...
    display_retention_pct=min(100.0,max(0.0,raw_retention_pct)) if np.isfinite(raw_retention_pct) and active else np.nan
    ...
    if active:
        ...
        if np.isfinite(beyond_peak_pct) and beyond_peak_pct>0.25:
            reason+=f' • +{beyond_peak_pct:.1f}% above prior close'
    out.update({'active':active,'hard':hard,'prior_session_move_pct':100*prior_move,
                'prior_session_move_atr':prior_atr,
                'carryover_retention_pct':display_retention_pct,
...
def _extension_risk_v614(f,r,p,a,zone_low,zone_high,entry_mid,target1,trigger_anchor=None,hourly_feat=None,m15_feat=None,previous_close=None,invalidation=None,target2=None):
    ...
    carry=_carryover_extension_v617(f,p,a)
    ...
    return {
        ...
        'carryover_raw_retention_pct':carry.get('carryover_raw_retention_pct',np.nan),'carryover_extension_beyond_peak_pct':carry.get('carryover_extension_beyond_peak_pct',np.nan),
...
def entry_timing(feat, quant_score, early_score, hourly_feat=None, m15_feat=None, current_price=None, market_regime='NEUTRAL', previous_close=None):
    ...
    post_spike=_post_spike_state_v619(f,hourly_feat,m15_feat,previous_close,p,momentum_state)
    ...
    extension=_extension_risk_v614(f,r,p,a,zone_low,zone_high,entry_mid,target1,trigger_anchor,hourly_feat,m15_feat,previous_close,invalid,target2)
    ...
    elif too_late:state='TOO LATE / CHASE'
    elif critical and combined>=62:state='CONFIRMED ENTRY'
    ...
    if np.isfinite(move_consumed_before_trigger_pct):
        trigger_efficiency_label='RESEARCH • EARLY' if move_consumed_before_trigger_pct<=45 else ('RESEARCH • MID' if move_consumed_before_trigger_pct<=70 else 'RESEARCH • LATE')
    else:
        trigger_efficiency_label='RESEARCH • NO DATA'
    ...
    if not extension_guard_ok and np.isfinite(_finite(retest_lo,np.nan)) and np.isfinite(_finite(retest_hi,np.nan)):
    ...
    return {
        ...
        'carryover_raw_retention_pct':extension.get('carryover_raw_retention_pct',np.nan),'carryover_extension_beyond_peak_pct':extension.get('carryover_extension_beyond_peak_pct',np.nan),
        ...
        'move_consumed_before_trigger_pct':move_consumed_before_trigger_pct,'move_consumed_before_setup_origin_pct':move_consumed_before_origin_pct,'trigger_efficiency_label':trigger_efficiency_label,
