...
ENGINE_VERSION = "6.1.3"
ENGINE_BUILD_ID = "V613-CHASE-EXTENSION-GUARD-20260918-B"
...
def fetch_live_intraday_snapshot(ticker: str, daily=None):
    ...
    try:
        ...
        for interval,period,max_age,trade_age in attempts:
            try:
                raw=yf.download(str(ticker).upper(),period=period,interval=interval,prepost=True,
                ...
                out.update({'price':price,'prev_close':prev_close,'change_pct':change,
                            ...
                            'source':f'Yahoo {interval} current-session feed','fresh':True,
        ...
        tv=_fetch_tradingview_display_quote(ticker,daily,out.get('split_ratio',np.nan))
...
def _transition_anchor_v613(frame, bars, timeframe):
    """Earliest hard bullish transition inside the still-fresh lookback window.

    The anchor is intentionally transition-based instead of current-state based so
    a late scanner refresh cannot silently move the entry zone up to the current
    price after the trade has already run.
    """
    ...
    for idx,row in f.iterrows():
        ...
        if labels:
            ...
            try:
                ts=pd.Timestamp(idx)
                if ts.tzinfo is None:ts_sort=ts.tz_localize('UTC')
                else:ts_sort=ts.tz_convert('UTC')
            except Exception:
                ts_sort=pd.Timestamp('2262-01-01',tz='UTC')
            return {
                'price':float(px),'timestamp':str(idx),'ts_sort':ts_sort,'timeframe':timeframe,
                'signals':labels,'vwap':_finite(row.get('vwap',np.nan)),'ema9':_finite(row.get('ema9',np.nan)),
                'ema20':_finite(row.get('ema20',np.nan)),'atr14':_finite(row.get('atr14',np.nan)),
            }
    return None


def _original_trigger_anchor_v613(daily, hourly=None, m15=None):
    candidates=[]
    for frame,bars,tf in ((daily,4,'1D'),(hourly,6,'1H'),(m15,8,'15m')):
        x=_transition_anchor_v613(frame,bars,tf)
        if x:candidates.append(x)
    if not candidates:return None
    # Oldest still-fresh hard transition = original trigger for the current setup.
    # This is conservative by design: later VWAP reclaims must not reset the clock.
    return min(candidates,key=lambda x:x.get('ts_sort'))


def _volume_trend_v613(hourly=None,m15=None):
    """Describe whether current intraday participation is accelerating or fading."""
    for frame,tf in ((m15,'15m'),(hourly,'1H')):
        ...
        if np.isfinite(latest) and np.isfinite(base) and base>0:
            ratio=latest/base
            if ratio>=1.08 or (np.isfinite(va) and va>=1.08):return {'label':'ACCELERATING','timeframe':tf,'latest_rvol':latest,'prior_rvol':base,'ratio':ratio}
            if ratio<=0.82 or (np.isfinite(va) and va<0.90):return {'label':'FADING','timeframe':tf,'latest_rvol':latest,'prior_rvol':base,'ratio':ratio}
            return {'label':'STABLE','timeframe':tf,'latest_rvol':latest,'prior_rvol':base,'ratio':ratio}
        if np.isfinite(va):
            return {'label':'ACCELERATING' if va>=1.08 else ('FADING' if va<0.90 else 'STABLE'),'timeframe':tf,'latest_rvol':latest,'prior_rvol':base,'ratio':np.nan}
    return {'label':'NO DATA','timeframe':'—','latest_rvol':np.nan,'prior_rvol':np.nan,'ratio':np.nan}


def _extension_risk_v613(f,r,p,a,entry_mid,target1,trigger_anchor=None,hourly_feat=None,m15_feat=None,previous_close=None):
    """Independent hard veto for late entries after a move has already happened.

    This guard intentionally sits *outside* the six classic entry gates. A setup
    can therefore display 6/6 while still becoming EXTENDED — DO NOT CHASE.
    """
    ...
    if not np.isfinite(prev) or prev<=0:
        # If current price is near the latest daily close, that row is probably the
        # current session and the previous row is the proper baseline. If the live
        # price is materially away from the latest daily close, treat that close as
        # the previous official close (common while the daily feed is still stale).
        if np.isfinite(close) and close>0 and abs(p/close-1.0)>0.015:
            prev=close
        elif f is not None and len(f)>=2:
            prev=_finite(f.iloc[-2].get('Close',np.nan))
    ...
    vt=_volume_trend_v613(hourly_feat,m15_feat)

    def ramp(x,lo,hi):
        if not np.isfinite(x):return 0.0
        return float(np.clip((x-lo)/max(hi-lo,1e-9),0,1))
    # Positive extension only; a sharp decline is handled by invalidation/exit logic.
    ...
    score=(30*ramp(sr,.03,.10)+25*ramp(st,.02,.08)+15*ramp(max(0,dvwap) if np.isfinite(dvwap) else 0,.75,2.0)+10*ramp(max(0,de9,de20),.80,2.0)+10*ramp(gp,.02,.07)+10*ramp(tp,.25,.85))
    if vt.get('label')=='FADING' and sr>=.04:score+=5
    score=float(np.clip(score,0,100))

    hard=bool(
        sr>=.10 or st>=.085 or vext>=2.20 or tp>=.85 or
        (sr>=.075 and (gp>=.045 or vext>=1.25))
    )
    ...
    return {
        ...
        'volume_trend':vt.get('label','NO DATA'),'volume_trend_timeframe':vt.get('timeframe','—'),
    }


def entry_timing(feat, quant_score, early_score, hourly_feat=None, m15_feat=None, current_price=None, market_regime='NEUTRAL', previous_close=None):
    """V6.1.3 Entry Trigger Engine V3 + Chase/Extension Guard.

    The six classic evidence gates remain visible. A seventh independent hard
    veto prevents CONFIRMED ENTRY when the stock is already extended, even when
    the setup itself is 6/6. Entry geometry is anchored to the original fresh
    transition instead of being regenerated around a late current price.
    """
    f=feat.dropna(subset=['Close']) if feat is not None else pd.DataFrame()
    if f.empty:
        return {'entry_score':0.0,'status':'WAIT','trigger_state':'WAIT','zone_low':np.nan,'zone_high':np.nan,'trigger':np.nan,'invalidation':np.nan,'target1':np.nan,'target2':np.nan,'plan_valid':False,'plan_reason':'No data','why_now':'No data','missing_checks':'Daily Setup','confirmed_conditions':0,'total_conditions':6,'chase_risk_score':0.0,'chase_risk_label':'LOW','extension_guard_ok':True}
    r=f.iloc[-1]
    ...
    trigger_anchor=_original_trigger_anchor_v613(f,hourly_feat,m15_feat)
    ...
    fresh_h=_recent_transition_evidence(hourly_feat,6)
    fresh_m=_recent_transition_evidence(m15_feat,8)
    ...
    # Legacy No-Chase remains one of the six explainable gates. The stronger
    # extension veto below is deliberately independent, so 6/6 can still be
    # displayed while the final action becomes DO NOT CHASE.
    ...
    extension=_extension_risk_v613(f,r,p,a,entry_mid,target1,trigger_anchor,hourly_feat,m15_feat,previous_close)
    ...
    critical=all(checks.values()) and plan_valid and xp<45 and extension_guard_ok
    ...
    if invalidated:
        state='INVALIDATED'
    elif not extension_guard_ok:
        state='EXTENDED — DO NOT CHASE'
    elif too_late:
        state='TOO LATE / CHASE'
    elif critical and combined>=62:
        state='CONFIRMED ENTRY'
    elif plan_valid and daily_setup and fresh_signal and xp<55 and confirmed>=4:
        state='ARMED'
    elif plan_valid and daily_setup and xp<55:
        state='WATCH'
    else:
        state='WAIT'
    ...
    if state=='EXTENDED — DO NOT CHASE':
        why.extend(extension.get('reasons',[])[:4]);why.append('WAIT FOR RETEST')
    else:
    ...
    anchor_ts=(trigger_anchor or {}).get('timestamp','—');anchor_tf=(trigger_anchor or {}).get('timeframe','—');anchor_signals=' • '.join((trigger_anchor or {}).get('signals',[])) if trigger_anchor else 'None'
    return {
        'entry_score':round(float(_clamp(combined)),1),'setup_entry_score_pre_chase':round(pre_chase_combined,1),'status':state,'trigger_state':state,
        ...
        'why_now':why_now,'missing_checks':' • '.join(missing) if missing else 'None',
        'fresh_evidence':' • '.join(fresh_labels[:6]) if fresh_labels else 'None',
        'extension_guard_ok':bool(extension_guard_ok),'chase_risk_score':extension.get('score',0.0),'chase_risk_label':extension.get('label','LOW'),
        'chase_reasons':' • '.join(extension.get('reasons',[])) if extension.get('reasons') else 'None',
        'session_move_pct':extension.get('session_move_pct',np.nan),'open_move_pct':extension.get('open_move_pct',np.nan),'gap_pct':extension.get('gap_pct',np.nan),
        'since_trigger_pct':extension.get('since_trigger_pct',np.nan),'vwap_distance_atr':extension.get('vwap_distance_atr',np.nan),'ema9_distance_atr':extension.get('ema9_distance_atr',np.nan),'ema20_distance_atr':extension.get('ema20_distance_atr',np.nan),'target1_progress_pct':extension.get('target1_progress_pct',np.nan),
        'volume_trend':extension.get('volume_trend','NO DATA'),'volume_trend_timeframe':extension.get('volume_trend_timeframe','—'),
        'trigger_anchor_price':_finite((trigger_anchor or {}).get('price',np.nan)),'trigger_anchor_time':anchor_ts,'trigger_anchor_timeframe':anchor_tf,'trigger_anchor_signals':anchor_signals,
        'recommended_action':'WAIT FOR RETEST' if not extension_guard_ok else ('DO NOT ENTER' if state in ('INVALIDATED','TOO LATE / CHASE') else ('ENTRY CONDITIONS CONFIRMED' if state=='CONFIRMED ENTRY' else state)),
    }
