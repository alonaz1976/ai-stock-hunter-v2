...
ENGINE_VERSION = "6.1.7"
ENGINE_BUILD_ID = "V617-LIVE-RR-CARRYOVER-GUARD-20260919-A"
...
def _carryover_extension_v617(f,p,a):
    ...
    out={'active':False,'prior_session_move_pct':np.nan,'prior_session_move_atr':np.nan,
         'carryover_retention_pct':np.nan,'carryover_base_price':np.nan,
    ...
    reason=''
    if active:
        reason=f'prior session +{100*prior_move:.1f}% • {100*retention_clip:.0f}% of move retained'
    out.update({'active':active,'hard':hard,'prior_session_move_pct':100*prior_move,
                'prior_session_move_atr':prior_atr,
                'carryover_retention_pct':100*retention_clip if np.isfinite(retention_clip) else np.nan,
