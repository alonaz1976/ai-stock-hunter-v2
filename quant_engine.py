...
ENGINE_BUILD_ID = "V611-ADAPTIVE-ENTRY-LEARNING-20260917-A"
...
def entry_timing(feat, quant_score, early_score, hourly_feat=None, m15_feat=None, current_price=None, market_regime='NEUTRAL'):
    """V6.1 Entry Trigger Engine V3.

    A high score is no longer sufficient for entry. The state machine requires:
    Daily Setup -> Fresh Signal -> Hourly/15m Timing -> Volume/Flow -> No-Chase.
    States: WAIT / WATCH / ARMED / CONFIRMED ENTRY / TOO LATE / INVALIDATED.
    """
    f=feat.dropna(subset=['Close']) if feat is not None else pd.DataFrame()
    if f.empty:
        return {'entry_score':0.0,'status':'WAIT','trigger_state':'WAIT','zone_low':np.nan,'zone_high':np.nan,'trigger':np.nan,'invalidation':np.nan,'target1':np.nan,'target2':np.nan,'plan_valid':False,'plan_reason':'No data','why_now':'No data','missing_checks':'Daily Setup','confirmed_conditions':0,'total_conditions':6}
    r=f.iloc[-1]
    ...
    cmf=_finite((short_r if short_r is not None else r).get('cmf20',np.nan),0)
    volume_flow_ok=bool((dv_short['bullish']>=0.22 and dv_short['bullish']>dv_short['bearish']) or (dv_daily['bullish']>=0.24 and cmf>=-0.02))
    ...
    critical=all(checks.values()) and plan_valid and xp<45

    # Blend score is descriptive; state remains evidence-gated.
    ...
    return {
        'entry_score':round(float(_clamp(combined)),1),'status':state,'trigger_state':state,
        ...
        'volume_flow_ok':volume_flow_ok,'no_chase':no_chase,'market_regime':regime,'market_regime_ok':market_regime_ok,
